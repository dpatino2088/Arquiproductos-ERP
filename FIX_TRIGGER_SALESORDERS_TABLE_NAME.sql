...
-- ====================================================
-- This script fixes the on_quote_approved_create_operational_docs function
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        ELSIF EXISTS (
            ...
            -- FIXED: Use 'SalesOrders' (plural) as table name
    ...
    LOOP
        ...
        IF NOT FOUND THEN
            ...
            RAISE NOTICE '✅ Created SaleOrderLine for QuoteLine %', v_quote_line_record.id;
        ELSE
            RAISE NOTICE '⏭️  SaleOrderLine % already exists for QuoteLine %', v_sale_order_line_id, v_quote_line_record.id;
        END IF;
    END LOOP;
    
    RAISE NOTICE '✅ Completed processing Quote % → SaleOrder %', NEW.id, v_sale_order_id;
...
'Automatically creates a SalesOrder with status=Draft when a Quote is approved. Does NOT create BOM, Manufacturing Order, or OrderList.';

-- ====================================================
-- STEP 3: Verify the function and trigger
-- ====================================================

DO $$
BEGIN
    RAISE NOTICE '✅ Function on_quote_approved_create_operational_docs updated to use "SalesOrders" (plural)';
    RAISE NOTICE '✅ Trigger trg_on_quote_approved_create_operational_docs created/updated';
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '✅ Trigger verification: EXISTS and ACTIVE';
    ELSE
        RAISE WARNING '⚠️ Trigger verification: NOT FOUND';
    END IF;
END;
$$;
