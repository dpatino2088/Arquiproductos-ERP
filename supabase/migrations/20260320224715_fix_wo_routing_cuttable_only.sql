...
-- 1. Update WorkCenters routing rules
UPDATE "WorkCenters"
SET routing_rule = '{"category_parent_names": ["Profiles"], "require_linear": true}'::jsonb,
    updated_at = now()
...
-- Re-enable PICK station as Hardware & Parts
UPDATE "WorkCenters"
SET is_active = true,
    routing_rule = '{"is_pick": true}'::jsonb,
    name = 'Hardware & Parts',
    updated_at = now()
WHERE code = 'PICK';

-- 2. Replace the function with corrected routing logic
...
DECLARE
  ...
  v_is_pick      boolean;
  v_routed_bils  uuid[] := ARRAY[]::uuid[];  -- track which BOM lines were routed to cutting/pick
BEGIN
  ...
  -- Process work centers in sequence order (cutting stations first, then pick, then assembly)
  FOR v_wc IN
  ...
  LOOP
    ...
    -- PICK station: gets items NOT already routed to cutting stations
    ...
    -- CUTTING stations: category-based + optional require_linear filter
    ...
    LOOP
      ...
      -- Apply require_linear filter: only linear/area items for cutting stations
...
DO $$ BEGIN RAISE NOTICE 'WO routing fixed: CUT-PROFILE only linear items, PICK gets remaining parts'; END $$;;
