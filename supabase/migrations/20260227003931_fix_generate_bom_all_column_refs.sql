...
BEGIN
    ...
    LOOP
        ...
        IF v_bom_instance_id IS NULL THEN v_warnings := v_warnings || format('Failed to create BOMInstance for SaleOrderLine %s', v_sales_order_line.id); CONTINUE; END IF;
        ...
        LOOP
            IF v_bom_component.component_role = 'fabric' THEN
                ...
                IF NOT FOUND THEN v_warnings := v_warnings || format('Fabric CatalogItem %s does NOT match SaleOrderLine %s', v_bom_component.component_item_id, v_sales_order_line.id); CONTINUE; END IF;
            ELSE
            ...
            ELSE v_unit_cost_exw := COALESCE(CAST(v_catalog_item.cost_exw AS numeric(12,4)), 0);
            END IF;
