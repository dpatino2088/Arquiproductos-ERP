-- ============================================================================
-- Heat Seal (automatic fabric rotation) + Bottom Bar Wrap (forrado)
-- Adds heatseal_price_per_m and bottom_bar_wrap_pct to FabricRules.
-- Rewrites compute_fabric_pricing_from_rule with auto-rotation/heatseal/wrap.
-- Updates build_bom_preview_snapshot to pass RollSpecs flags + config and to
-- respect condition_key / condition_value on BOM components.
-- ============================================================================

BEGIN;

-- ── 1. Schema: new columns on FabricRules ──────────────────────────────────

ALTER TABLE public."FabricRules"
  ADD COLUMN IF NOT EXISTS heatseal_price_per_m numeric DEFAULT 0,
  ADD COLUMN IF NOT EXISTS bottom_bar_wrap_pct  numeric DEFAULT 0;

COMMENT ON COLUMN public."FabricRules".heatseal_price_per_m IS
  'Cost per linear meter of heat-seal splice when fabric is rotated and needs welding.';
COMMENT ON COLUMN public."FabricRules".bottom_bar_wrap_pct IS
  'Percentage surcharge on fabric cost when the bottom bar is wrapped (forrado). 0.08 = 8%.';


-- ── 2. Drop old signature so we can add new return columns ─────────────────

DROP FUNCTION IF EXISTS public.compute_fabric_pricing_from_rule(
  uuid, uuid, text, numeric, numeric, numeric, numeric, jsonb
);


-- ── 3. Rewrite compute_fabric_pricing_from_rule ────────────────────────────
--    New params: p_can_rotate, p_is_weldable, p_bottom_bar_wrapped
--    New return cols: is_rotated, heatseal_seams, heatseal_cost,
--                     bottom_bar_wrap_cost
...
BEGIN
    ...
    ELSE
      ...
      -- ── AUTO-ROTATION for ROLLER_DROPS ──
      IF COALESCE(v_r.formula_code, '') = 'ROLLER_DROPS'
         AND p_roll_width_m IS NOT NULL AND p_roll_width_m > 0
      THEN
          IF v_weff > p_roll_width_m AND p_can_rotate THEN
              v_rotated := true;
              -- Rotated: roll width covers height, unroll for width
              v_drops := CEIL(v_heff / p_roll_width_m);
              v_area := v_drops * v_weff * p_roll_width_m;

              -- Heat seal if height exceeds roll width after rotation
              IF v_heff > p_roll_width_m AND p_is_weldable THEN
...
COMMENT ON FUNCTION public.compute_fabric_pricing_from_rule IS
  'Fabric consumption engine with auto-rotation, heat-seal costing, and '
  'bottom-bar-wrap surcharge. Auto-detects when rotation is needed '
  '(fabric_cut_width > roll_width AND can_rotate). Returns is_rotated, '
  'heatseal_seams, heatseal_cost, bottom_bar_wrap_cost for full auditability.';


-- ── 4. Rewrite build_bom_preview_snapshot ──────────────────────────────────
--    - Reads can_rotate / is_weldable from CatalogItemRollSpecs
--    - Reads bottom_bar_wrapped from config_snapshot
--    - Passes those to compute_fabric_pricing_from_rule
--    - Includes rotation/heatseal/wrap in fabric_calc output
--    - Applies condition_key / condition_value filtering on BOM components
...
BEGIN
  ...
  -- Read bottom_bar_wrapped from config
  ...
  IF v_cp.roll_catalog_item_id IS NOT NULL THEN
    ...
    -- Read can_rotate / is_weldable from CatalogItemRollSpecs
    BEGIN
  ...
  IF p_bom_template_id IS NOT NULL THEN
    ...
    LOOP
      ...
      -- ── condition_key / condition_value filtering ──
      ...
      FOR v_child IN
        ...
        ORDER BY bc.sort_order ASC
      LOOP
        -- condition filtering for children too
