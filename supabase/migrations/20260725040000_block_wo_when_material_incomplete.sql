-- ============================================================================
-- Block Work Order generation when MO materials are incomplete
-- Keeps current routing logic (cuttable + pick), adds readiness guard.
-- ============================================================================
...
BEGIN
  ...
  -- Guardrail: do not allow WO generation until materials are covered.
  v_readiness := public.get_mo_material_readiness(p_mo_id);
  v_has_shortage := COALESCE((v_readiness->>'has_shortage')::boolean, true);
  IF v_has_shortage THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error', 'Materials incomplete. Cannot generate Work Orders until Material Demand is resolved.',
  ...
  LOOP
    ...
    -- PICK station: gets lines not routed to cutting stations.
