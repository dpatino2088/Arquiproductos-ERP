-- Fix intermediate / joint deductors: quantity must follow N-1 panels, not BOM qty_value.
-- BOM qty for these lines can reflect purchasing (sets/meters) and must not inflate cut math.
  ...
  LOOP
    ...
    FOR v_deductor IN
      SELECT bc2.id, bc2.component_role, ci2.sku,
             ...
             (
               lower(COALESCE(bc2.component_role, '')) LIKE '%intermediate%'
    ...
    LOOP
        ...
        CASE
          WHEN COALESCE(bc_ch.delta_mode, 'subtract') = 'add' THEN
            -(CASE WHEN v_axis = 'height'
              ...
              END) * (CASE WHEN COALESCE(bc_ch.cut_delta_scope, 'per_item') = 'per_side' THEN 2 ELSE 1 END) * COALESCE(bc_ch.qty_value, 1)
      ...
      IF v_combined_d <> 0 OR v_deductor.delta_mode = 'info' THEN
        v_deductions := v_deductions || jsonb_build_object(
          ...
          'delta',         v_combined_d,
          'qty',           CASE WHEN v_deductor.is_intermediate THEN v_joint_qty ELSE v_deductor.qty END,
          'total',         CASE
                             WHEN v_deductor.is_intermediate THEN v_combined_d * v_joint_qty
    ...
    FOR v_child_ded IN
      SELECT bc3.component_role, ci3.sku,
             ...
             (
               lower(COALESCE(bc3.component_role, '')) LIKE '%intermediate%'
    ...
    LOOP
      ...
      IF v_child_ded.delta <> 0 OR v_child_ded.delta_mode = 'info' THEN
        ...
        v_deductions := v_deductions || jsonb_build_object(
          ...
          'qty',           CASE WHEN v_child_ded.is_intermediate THEN v_joint_qty ELSE v_child_ded.qty END,
          'total',         CASE
                             WHEN v_child_ded.is_intermediate THEN v_child_ded.delta * v_joint_qty
