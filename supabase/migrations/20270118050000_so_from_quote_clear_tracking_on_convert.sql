-- When converting a quote to SO, clear Quotes.tracking_status.
-- Check constraint quotes_tracking_status_only_when_approved requires
-- tracking_status IS NULL whenever status <> 'approved'. SO insert mirrors
-- tracking onto the quote first, so convert must clear it.
