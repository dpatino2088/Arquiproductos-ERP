-- Phase 3c: unify SINGLE-PANEL cut geometry to the core engine.
--
-- Until now, single-panel component lines wrote cut_length_mm from the snapshot
-- qty (the legacy cascade geometry), which diverges from compute_cut_breakdown_core
-- by a few mm (motor/edge/intermediate handling). Multi-panel already used the core
-- via per-panel splits; fabric already uses FabricRules. This closes the remaining
-- gap so production cut instructions come from the core for every product.
--
-- Cost protection: cut_length_mm does NOT drive cost (qty does). We only overwrite
-- the cut instruction with the core geometry and keep qty / cost / msrp frozen from
-- the quoted snapshot. A reconciliation guard keeps the quoted cut (and warns) when
-- core vs quoted geometry diverge beyond tolerance, so nothing silently shifts.
...
BEGIN
    ...
    LOOP
        ...
        -- ============================================================
        -- Unify cut geometry to the core for EVERY cuttable role.
        -- Multi-panel  -> split into per-panel lines (cost prorated from frozen line).
        -- Single-panel -> overwrite cut_length_mm with core geometry (qty/cost frozen).
        -- Both paths reconcile against the quoted geometry before changing anything.
        -- ============================================================
        FOR v_role IN
            SELECT replace(k, '_width_mm', '')
            FROM jsonb_object_keys(v_dim_outputs) AS k
            WHERE k LIKE '%\_width\_mm' AND k NOT LIKE 'finished%'
        LOOP
            ...
            IF v_panel_cuts IS NOT NULL AND jsonb_typeof(v_panel_cuts) = 'array' AND jsonb_array_length(v_panel_cuts) > 1 THEN
                ...
                END LOOP;
            ELSE
                -- ---------- SINGLE-PANEL CORE CUT OVERRIDE ----------
                -- Only single-piece roles (exactly one 'm' line) are overwritten,
                -- to avoid corrupting multi-piece roles (side_channel, brush, chain)
                -- whose cut_length_mm stores combined length, not per-piece geometry.
                v_core_mm := (v_dim_outputs->>(v_role || '_width_mm'))::numeric;
                IF v_core_mm IS NOT NULL AND v_core_mm > 0 THEN
                    SELECT count(*) INTO v_line_cnt FROM public."BOMInstanceLines"
                     WHERE bom_instance_id = v_bi_id AND part_role = v_role AND deleted = false
                       AND COALESCE(uom,'') = 'm' AND panel_index IS NULL;
                    IF v_line_cnt = 1 THEN
                        ...
                        LOOP
                            ...
                            IF ABS(v_core_mm - v_quoted_cut_mm) > v_recon_tol THEN
                                v_recon_skips := v_recon_skips + 1;
                                v_warn := v_warn || format(
                                    'Role %s single-panel: core cut %smm <> quoted %smm (SOL %s); kept quoted cut',
                                    v_role, round(v_core_mm, 1), round(v_quoted_cut_mm, 1), v_sol.id);
                                CONTINUE;
                            END IF;
                            UPDATE public."BOMInstanceLines"
                               SET cut_length_mm = ROUND(v_core_mm, 1), updated_at = now()
