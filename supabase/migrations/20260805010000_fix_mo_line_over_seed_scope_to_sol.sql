-- Fix: generate_bom_for_manufacturing_order auto-created ManufacturingOrderLines
-- for ALL SaleOrderLines in the same SalesOrder, instead of only the specific
-- sales_order_line_id that the MO is linked to.
--
-- Root cause: WHERE sol.sales_order_id = v_mo.sales_order_id (all SO lines)
-- Fix:        AND (v_mo.sales_order_line_id IS NULL OR sol.id = v_mo.sales_order_line_id)
--
-- This caused each MO to get BOM materials from OTHER products on the same SO,
-- leading to wrong fabric allocations (e.g., MO for "VX Screen" also getting
-- "Sunset Blackout" fabric allocated).
...
BEGIN
    ...
    -- FIX: scope to MO's specific sales_order_line_id when set
    ...
    LOOP
        ...
        -- Re-cut linear profiles per panel (tube / bottom_bar)
        ...
        -- Split fabric per panel for multi-panel products
