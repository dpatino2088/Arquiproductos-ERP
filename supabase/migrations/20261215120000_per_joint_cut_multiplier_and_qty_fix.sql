-- Joint / per-joint cut multiplier fixes:
-- 1) qty_type = per_joint must use N-1 panel multiplier even when component_role text does not
--    contain 'intermediate' (e.g. support sets catalogued as Intermediate Bracket category).
-- 2) qty_value = 0 must not zero-out deductions (COALESCE does not replace 0).
...
BEGIN
  ...
  LOOP
    ...
    -- Optional cuttables explicitly disabled in config
    ...
    -- External deductors (top-level affecting this cuttable)
    FOR v_deductor IN
      SELECT bc2.id, bc2.component_role, ci2.sku,
       ...
       WHERE bc2.bom_template_id = p_bom_template_id
         ...
         AND v_role = ANY(string_to_array(COALESCE(bc2.affects_role, ''), ','))
    ...
    LOOP
      ...
      -- optional part explicitly disabled
      IF v_config_snap ->> (v_deductor.component_role || '_item_id') = 'NONE' THEN
        CONTINUE;
      END IF;

      -- N-1: intermediates absent for single panel
      ...
      WHERE bc_ch.parent_component_id = v_deductor.id
        ...
        AND v_role = ANY(string_to_array(COALESCE(bc_ch.affects_role, ''), ','));
...
IS 'Core DB cut breakdown calculator used for reliable preview/instance parity.';
