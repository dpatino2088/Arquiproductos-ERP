...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- total = (self_delta * parent_qty) + children_sum
  ...
  IF p_bom_template_id IS NOT NULL THEN
    ...
    v_topo_resolved := '{}'; LOOP v_topo_progress := false;
      FOR v_topo_item IN SELECT * FROM jsonb_array_elements(v_topo_all) LOOP
        ...
        -- Parent-level only (parent_component_id IS NULL), with children aggregated
        -- Axis-aware delta, conditional/intermediate filtering
        SELECT COALESCE(SUM(
