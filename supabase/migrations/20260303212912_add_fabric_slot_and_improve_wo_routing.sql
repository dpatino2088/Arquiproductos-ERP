...
SELECT DISTINCT ON (bc.organization_id, bc.bom_template_id)
  bc.organization_id,
  bc.bom_template_id,
  'fabric',
  true,
  NULL,
  1,
  'Auto-added: fabric slot was missing from template slots'
FROM "BOMComponents" bc
WHERE bc.component_role = 'fabric'
  AND bc.deleted = false
  AND NOT EXISTS (
    SELECT 1 FROM "BOMTemplateSlots" bts
    WHERE bts.organization_id = bc.organization_id
      AND bts.bom_template_id = bc.bom_template_id
      AND bts.item_role = 'fabric'
  )
ORDER BY bc.organization_id, bc.bom_template_id, bc.created_at DESC;
