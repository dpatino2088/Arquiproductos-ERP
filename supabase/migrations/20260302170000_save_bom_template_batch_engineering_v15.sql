-- Update save_bom_template_batch to persist engineering v1.5 columns:
-- engineering_delta_source, engineering_attr_key, engineering_scope, engineering_source_role
...
BEGIN
  ...
  -- First pass: parents (parent_component_id is null)
  ...
  LOOP
    ...
    IF v_comp_id IS NOT NULL THEN
      UPDATE "BOMComponents" SET
        ...
        engineering_delta_source = COALESCE(v_comp ->> 'engineering_delta_source', 'fixed'),
        engineering_attr_key     = v_comp ->> 'engineering_attr_key',
        engineering_scope        = COALESCE(v_comp ->> 'engineering_scope', 'total'),
        engineering_source_role  = v_comp ->> 'engineering_source_role',
    ...
    ELSE
      INSERT INTO "BOMComponents" (
        ...
        COALESCE(v_comp ->> 'engineering_delta_source', 'fixed'),
        v_comp ->> 'engineering_attr_key',
        COALESCE(v_comp ->> 'engineering_scope', 'total'),
        v_comp ->> 'engineering_source_role',
  ...
  -- Second pass: children (have parent_component_id or parent_temp_id)
  FOR v_comp IN SELECT jsonb_array_elements(p_components_upsert)
  LOOP
    v_temp_id := v_comp ->> 'temp_id';
    v_comp_id := CASE
