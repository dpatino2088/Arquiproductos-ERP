-- Update compute_system_dimensions and build_bom_preview_snapshot
-- to use affected_by_roles instead of affects_role for deduction queries.
-- This eliminates double-counting of conditional/mutually-exclusive components.

BEGIN;

-- ============================================================================
-- 1. compute_system_dimensions: load affected_by_roles from the cuttable
--    component row, then match on component_role = ANY(affected_by_roles)
--    instead of affects_role = '<role>'. Also filter by delta_mode = 'subtract'.
-- ============================================================================
...
BEGIN
  ...
  -- ===== TUBE =====
  SELECT bc.id, bc.affected_by_roles
  INTO v_target_comp_id, v_affected_roles
  ...
  v_affected_roles := COALESCE(v_affected_roles, '{}');
    ...
    SELECT
      ...
      + COALESCE((
        ...
        WHERE cbc.parent_component_id = bc.id AND cbc.deleted = false
          AND COALESCE(cbc.delta_mode, 'subtract') = 'subtract'
      ), 0) as delta_val
    FROM "BOMComponents" bc
    JOIN "CatalogItems" ci ON ci.id = bc.component_item_id
    WHERE bc.bom_template_id = v_cp.bt_id
      AND bc.deleted = false
      AND bc.component_role = ANY(v_affected_roles)
      AND bc.parent_component_id IS NULL
      AND COALESCE(bc.qty_type::text, 'fixed') != 'per_joint'
      AND COALESCE(bc.delta_mode, 'subtract') = 'subtract'
    UNION ALL
    SELECT
...
-- ============================================================================
-- 2. build_bom_preview_snapshot: update the cascade pre-pass to load
--    affected_by_roles for each cuttable and use
--    component_role = ANY(affected_by_roles) instead of affects_role
-- ============================================================================
...
BEGIN
  ...
  -- Now uses affected_by_roles instead of affects_role
  IF p_bom_template_id IS NOT NULL THEN
    ...
    LOOP
      v_topo_all := v_topo_all || jsonb_build_object(
        ...
        'affected_by_roles', COALESCE(v_cascade.affected_by_roles, '{}')
      );
    END LOOP;
    ...
    LOOP
      ...
      LOOP
        ...
        -- Load affected_by_roles from the topo item
        v_affected_roles := COALESCE(
          ARRAY(SELECT jsonb_array_elements_text(v_topo_item->'affected_by_roles')),
          '{}'
        );

        -- Sum deductions from components whose role is in affected_by_roles
        SELECT COALESCE(SUM(
