...
BEGIN
  ...
  -- Step 1: Auto-create ManufacturingOrderLines if none exist
  ...
  FOR v_sol IN
    SELECT mol.id as mol_id, mol.sales_order_line_id, mol.organization_id,
           mol.configured_product_id as mol_cp_id, mol.quantity as mol_qty,
           sol.product_type, sol.product_type_id, sol.width_m, sol.height_m,
           sol.quote_line_id, sol.quantity as sol_qty,
           sol.configured_product_id as sol_cp_id,
           cp.bom_template_id as cp_bom_template_id,
           cp.width_mm as cp_width_mm, cp.height_mm as cp_height_mm
    FROM "ManufacturingOrderLines" mol
    JOIN "SaleOrderLines" sol ON sol.id = mol.sales_order_line_id
    LEFT JOIN "ConfiguredProducts" cp ON cp.id = COALESCE(mol.configured_product_id, sol.configured_product_id)
  ...
  LOOP
    -- Priority: ConfiguredProduct.bom_template_id > product_type_id lookup > product_type code lookup
    v_bom_template_id := v_sol.cp_bom_template_id;

    IF v_bom_template_id IS NULL AND v_sol.product_type_id IS NOT NULL THEN
    ...
    IF v_bom_template_id IS NULL THEN
      v_warnings := array_append(v_warnings, 'No BOM Template for line: ' || COALESCE(v_sol.product_type, 'NULL'));
      CONTINUE;
    END IF;
    ...
    -- Use ConfiguredProduct dimensions (mm) if available, else SaleOrderLine dimensions (m->mm)
    v_width_mm := COALESCE(v_sol.cp_width_mm, COALESCE(v_sol.width_m, 0) * 1000);
    v_height_mm := COALESCE(v_sol.cp_height_mm, COALESCE(v_sol.height_m, 0) * 1000);

    FOR v_bc IN
      SELECT bc.component_item_id, bc.component_role, bc.qty_type, bc.qty_value,
             bc.qty_delta_mm, bc.uom, bc.waste_pct, bc.cut_axis, bc.cut_delta_mm, bc.qty_min
    ...
    LOOP
      ...
      IF COALESCE(v_bc.waste_pct, 0) > 0 THEN v_qty := v_qty * (1 + v_bc.waste_pct / 100); END IF;
      IF COALESCE(v_bc.qty_min, 0) > 0 AND v_qty < v_bc.qty_min THEN v_qty := v_bc.qty_min; END IF;
      v_qty := v_qty * COALESCE(v_sol.sol_qty, 1);

      v_cut_len := NULL;
      IF v_bc.cut_axis = 'width' THEN v_cut_len := v_width_mm + COALESCE(v_bc.cut_delta_mm, 0);
      ELSIF v_bc.cut_axis = 'height' THEN v_cut_len := v_height_mm + COALESCE(v_bc.cut_delta_mm, 0);
      END IF;
