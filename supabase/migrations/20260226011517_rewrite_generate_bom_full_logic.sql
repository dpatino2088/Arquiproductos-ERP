...
BEGIN
  ...
  LOOP
    -- Resolve BOMTemplate from product_type or product_type_id
    ...
    IF v_sol.product_type_id IS NOT NULL THEN
      ...
      AND bt.is_active = true AND bt.deleted = false
    ...
    -- Convert dimensions from meters to mm for calculations
    v_width_mm := COALESCE(v_sol.width_m, 0) * 1000;
    v_height_mm := COALESCE(v_sol.height_m, 0) * 1000;

    -- Process BOMComponents for this template
    FOR v_bc IN
      SELECT bc.id, bc.component_item_id, bc.component_role, bc.qty_type, bc.qty_value,
             ...
             bc.qty_spacing_mm, bc.qty_min, bc.component_scope, bc.type_per_unit
    ...
    LOOP
      ...
      -- Multiply by line quantity (number of units)
      ...
      -- Get cost from CatalogItems
      v_unit_cost := 0;
      SELECT cost_exw INTO v_unit_cost FROM "CatalogItems" WHERE id = v_bc.component_item_id AND deleted = false;
