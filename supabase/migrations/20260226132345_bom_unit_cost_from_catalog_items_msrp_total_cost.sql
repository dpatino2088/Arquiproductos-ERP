-- BOM unit cost: use CatalogItemsMSRP.total_cost (cost_exw + shipping + import_tax) with fallback to CatalogItems.cost_exw
...
BEGIN
    ...
    END IF;
    FOR v_mo_line IN SELECT mol.sales_order_line_id FROM public."ManufacturingOrderLines" mol WHERE mol.manufacturing_order_id = p_manufacturing_order_id AND mol.deleted = false AND mol.archived = false ORDER BY mol.created_at ASC
    LOOP
        ...
        v_bom_total_cost := 0;
        v_created_lines := 0;
        SELECT sol.id, sol.product_type, sol.collection_name, sol.variant_name, sol.width_m, sol.height_m, sol.area, sol.drive_type, sol.cassette, sol.cassette_type, sol.side_channel, sol.side_channel_type, sol.hardware_color, sol.bottom_rail_type, sol.quote_line_id INTO v_sales_order_line FROM "SalesOrderLines" sol WHERE sol.id = v_mo_line.sales_order_line_id AND sol.deleted = false;
        IF NOT FOUND THEN
...
COMMENT ON FUNCTION public.generate_bom_for_manufacturing_order IS 'Generates BOM for a ManufacturingOrder. Unit cost from CatalogItemsMSRP.total_cost (cost_exw+shipping+import_tax) with fallback to CatalogItems.cost_exw.';
