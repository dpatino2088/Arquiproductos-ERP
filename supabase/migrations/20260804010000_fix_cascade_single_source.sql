-- Fix cascade resolution to be a single source of truth between:
--   1) compute_template_cut_breakdown  (template display)
--   2) build_bom_preview_snapshot      (MO/WO production)
--
-- Bugs fixed:
--   A) Axis: cascade always used delta_x_mm; now uses delta_y_mm for height-axis cuttables
--   B) Conditional: both motors were deducted simultaneously; now filtered by config
--   C) Intermediate: connectors counted even for single-panel; now skipped when panel_count <= 1
--   D) Children aggregation: cascade had no parent_component_id IS NULL filter + children subquery
--   E) Math: template breakdown used (self+children)*qty; corrected to (self*qty)+children
--
-- Uses affects_role (text, comma-separated on deductors) to match cuttables.

BEGIN;

-- ============================================================================
-- 1. compute_template_cut_breakdown
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- External deductions: parent-level deductors whose affects_role contains this cuttable's role
    FOR v_deductor IN
      ...
      WHERE bc2.bom_template_id = p_bom_template_id
        ...
        AND bc2.affects_role IS NOT NULL
        AND bc2.affects_role != ''
        AND v_role = ANY(string_to_array(bc2.affects_role, ','))
      ORDER BY bc2.sort_order
    LOOP
      -- Sum children of this deductor that also target this cuttable (or have no explicit affects_role)
      SELECT COALESCE(SUM(
        (CASE WHEN v_axis = 'height'
      ...
      -- FIX E: total = (self_delta * parent_qty) + children_sum
...
BEGIN
  ...
  -- Uses affects_role (on deductors) + axis-aware deltas + conditional/intermediate filtering
  IF p_bom_template_id IS NOT NULL THEN
    ...
    v_topo_resolved := '{}'; LOOP v_topo_progress := false;
      FOR v_topo_item IN SELECT * FROM jsonb_array_elements(v_topo_all) LOOP
        ...
        -- Determine axis for this cuttable
        ...
        -- External deductions: deductors whose affects_role contains this cascade_role
        -- Parent-level only, with children aggregated, axis-aware, conditional/intermediate filtering
        SELECT COALESCE(SUM(
        ...
        -- Own children of the cuttable (axis-aware)
    ...
    END LOOP;
    -- Fallback for unresolved (circular deps)
    FOR v_topo_item IN SELECT * FROM jsonb_array_elements(v_topo_all) LOOP
