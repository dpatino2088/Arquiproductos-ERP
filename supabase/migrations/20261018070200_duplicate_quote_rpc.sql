-- RPC: duplicate_quote(p_quote_id, p_mode, p_recalculate)
-- Two modes:
--   'copy'    -> brand new QT-XXXXX number, no parent link
--   'version' -> new _V<N> number, linked to original via parent_quote_id / root_quote_id,
--                the previous editable version (draft/sent) is marked as 'superseded'.
-- p_recalculate = true marks cloned lines with pricing_locked=false and last_priced_at=NULL
-- so the UI can prompt the user to re-open each line against the current catalog.
...
GRANT EXECUTE ON FUNCTION public.duplicate_quote(uuid, text, boolean) TO authenticated;

COMMENT ON FUNCTION public.duplicate_quote(uuid, text, boolean) IS
  'Duplicate a Quote. mode=copy -> brand new QT number. mode=version -> appends _V2, _V3, ... and links parent_quote_id/root_quote_id. When p_recalculate=true, cloned lines are marked with pricing_locked=false and last_priced_at=NULL so UI can prompt fresh pricing.';
