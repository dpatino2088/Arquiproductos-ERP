...
BEGIN
  ...
  -- Keep current primary only if it still belongs to customer and is active.
  ...
  -- Fallback: oldest related active contact.
  SELECT c.id
...
-- Backfill existing customers where primary is null/invalid.
DO $$
DECLARE
  v_customer_id uuid;
BEGIN
