...
BEGIN
    ...
    FROM "CostSettings" WHERE organization_id = v_mo.organization_id AND is_active = true LIMIT 1;
    IF FOUND THEN
    ...
    LOOP
        ...
        IF v_sol.product_type IS NULL OR TRIM(v_sol.product_type) = '' THEN
            v_warnings := v_warnings || format('SOL %s has NULL product_type', v_sol.id); CONTINUE;
        END IF;
        SELECT bt.id INTO v_bom_template_id FROM "BOMTemplates" bt INNER JOIN "ProductTypes" pt ON pt.id = bt.product_type_id WHERE pt.code = v_sol.product_type AND bt.active = true AND bt.deleted = false ORDER BY bt.created_at DESC LIMIT 1;
        ...
        IF v_bom_instance_id IS NULL THEN v_warnings := v_warnings || format('Failed BOMInstance for SOL %s', v_sol.id); CONTINUE; END IF;
        ...
        IF v_snapshot_items IS NOT NULL THEN
            ...
            LOOP
                ...
                IF v_unit_cost_exw > 0 THEN
                    ...
                    BEGIN
                        IF v_cost_from_msrp THEN v_ucwt := v_unit_cost_exw;
                        ELSE v_ucwt := v_unit_cost_exw * (1 + v_shipping_pct + v_import_tax_pct); END IF;
                    ...
                    END;
                ELSE v_unit_msrp := 0; v_total_msrp := 0;
                END IF;
                ...
                IF v_item ? 'children' AND jsonb_array_length(COALESCE(v_item->'children', '[]'::jsonb)) > 0 THEN
                    ...
                    LOOP
                        ...
                        IF v_unit_cost_exw > 0 THEN
                            ...
                            BEGIN
                                IF v_cost_from_msrp THEN v_ccwt := v_unit_cost_exw;
                                ELSE v_ccwt := v_unit_cost_exw * (1 + v_shipping_pct + v_import_tax_pct); END IF;
                                v_cmsi := v_ccwt / NULLIF(1 - v_min_margin_pct, 0);
                                v_unit_msrp := COALESCE(v_cmsi / NULLIF(1 - v_max_discount_pct, 0), 0);
                                v_total_msrp := v_unit_msrp * v_item_qty;
                            END;
                        ELSE v_unit_msrp := 0; v_total_msrp := 0;
                        END IF;
            ...
            END LOOP;
        ELSE
            v_warnings := v_warnings || format('No snapshot for SOL %s; using legacy BOMComponents', v_sol.id);
            v_fabric_count := 0;
            ...
            LOOP
                ...
                ELSIF v_bom_component.qty_type = 'per_width' THEN
                    ...
                    v_calculated_qty := v_sol.width_m * COALESCE(v_bom_component.qty_value, 1);
                ELSIF v_bom_component.qty_type = 'per_height' THEN
                    IF v_sol.height_m IS NULL THEN CONTINUE; END IF;
                    v_calculated_qty := v_sol.height_m * COALESCE(v_bom_component.qty_value, 1);
                ELSIF v_bom_component.qty_type = 'per_area' THEN
                    IF v_sol.width_m IS NULL OR v_sol.height_m IS NULL THEN CONTINUE; END IF;
                    v_calculated_qty := v_sol.width_m * v_sol.height_m * COALESCE(v_bom_component.qty_value, 1);
                ...
                IF v_unit_cost_exw > 0 THEN
                    ...
                    BEGIN
                        IF v_cost_from_msrp THEN v_lcwt := v_unit_cost_exw;
                        ELSE v_lcwt := v_unit_cost_exw * (1 + v_shipping_pct + v_import_tax_pct); END IF;
                        v_lcmsi := v_lcwt / NULLIF(1 - v_min_margin_pct, 0);
                        v_unit_msrp := COALESCE(v_lcmsi / NULLIF(1 - v_max_discount_pct, 0), 0);
                        v_total_msrp := v_unit_msrp * v_calculated_qty;
                    END;
                ELSE v_unit_msrp := 0; v_total_msrp := 0; END IF;
                v_bom_total_cost := v_bom_total_cost + v_total_cost_exw;

                INSERT INTO "BOMInstanceLines" (organization_id, bom_instance_id, resolved_part_id, part_role, qty, uom, unit_cost_exw, total_cost_exw, unit_msrp, total_msrp, cut_length_mm, deleted, created_at, updated_at)
                VALUES (v_mo.organization_id, v_bom_instance_id, v_catalog_item.id, v_bom_component.component_role, v_calculated_qty, v_normalized_uom, COALESCE(v_unit_cost_exw,0)::numeric(12,4), COALESCE(v_total_cost_exw,0)::numeric(12,4), COALESCE(v_unit_msrp,0)::numeric(12,4), COALESCE(v_total_msrp,0)::numeric(12,4), CASE WHEN v_bom_component.qty_type = 'per_width' OR v_bom_component.component_role IN ('tube','bottom_bar','bottom_rail','bottom_rail_profile') THEN COALESCE(v_sol.width_m, 0) * 1000.0 ELSE NULL END, false, now(), now());
                v_created_lines := v_created_lines + 1; v_total_bom_lines := v_total_bom_lines + 1;
            END LOOP;
        END IF;
