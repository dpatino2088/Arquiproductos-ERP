-- Fix receive_purchase_order: convert received qty to internal meters based on the
-- LINE's actual unit, instead of assuming roll items are always received in rolls.
-- Previously a roll item line in 'yd' (e.g. 14 yd) was treated as 14 ROLLS and
-- multiplied by roll length (~384 m). Now: roll->x meters/roll, yd/ft/in/m->direct.
-- Non-roll linear items also convert yd/ft/in->m even when uppu<=1.
...
BEGIN
  ...
  LOOP
    SELECT
      ...
      COALESCE(ci.measure_basis, 'unit') AS measure_basis
    INTO v_pol
    ...
    -- Roll items: stock is linear meters. Convert by the LINE's actual unit.
    IF v_pol.catalog_item_id IS NOT NULL AND COALESCE(v_pol.is_roll_snapshot, false) = true THEN
      ...
      IF v_po_unit_norm = 'roll' THEN
        ...
        END IF;
      ELSIF v_po_unit_norm IN ('m', 'meter', 'meters', 'metre', 'metres') THEN
        v_inventory_delta := v_new_received;
      ELSIF v_po_unit_norm IN ('yd', 'yard', 'yards') THEN
        v_inventory_delta := v_new_received * 0.9144;
      ELSIF v_po_unit_norm IN ('ft', 'foot', 'feet') THEN
        v_inventory_delta := v_new_received * 0.3048;
      ELSIF v_po_unit_norm IN ('in', 'inch', 'inches') THEN
        v_inventory_delta := v_new_received * 0.0254;
      ELSE
        IF v_length_m_per_purchase IS NOT NULL AND v_length_m_per_purchase > 0 THEN
          v_inventory_delta := v_new_received * v_length_m_per_purchase;
        END IF;
    ...
    -- Non-roll items
    ELSIF v_pol.catalog_item_id IS NOT NULL THEN
      -- Packaging multiplier (box/pack/set -> stock unit) when units differ
      IF v_uppu > 1 AND v_po_unit_norm != v_stock_unit_norm THEN
        v_inventory_delta := v_new_received * v_uppu;
      END IF;
      -- Linear unit-system conversion to meters when stock is meters
      IF v_stock_unit_norm = 'm' THEN
        IF v_po_unit_norm IN ('yd', 'yard', 'yards') THEN
    ...
    IF v_pol.catalog_item_id IS NOT NULL THEN
      INSERT INTO "InventoryMovementLines" (
        ...
        v_inventory_delta,
        v_inventory_unit,
        format('Received %s %s -> %s %s',
               v_new_received,
               COALESCE(v_pol.unit, 'ea'),
