...
AS $function$
DECLARE
    v_mo RECORD; v_mo_line RECORD; v_sol RECORD; v_bi_id uuid; v_bt_id uuid; v_bc RECORD; v_ci RECORD; v_fc integer := 0; v_qty numeric; v_uom text; v_cat text;
    v_ucx numeric(12,4) := 0; v_tcx numeric(12,4) := 0; v_um numeric(12,4) := 0; v_tm numeric(12,4) := 0;
    v_btc numeric(12,4) := 0; v_blc numeric(12,4) := 0; v_btcl numeric(12,4) := 0; v_bm numeric(12,4) := 0;
    v_cs RECORD; v_sp numeric(8,4) := 0; v_ip numeric(8,4) := 0; v_mm numeric(8,4) := 35.0; v_md numeric(8,4) := 65.0; v_lp numeric(8,4) := 0;
    v_cl integer := 0; v_err text[] := ARRAY[]::text[]; v_warn text[] := ARRAY[]::text[]; v_fp jsonb;
...
BEGIN
    ...
    LOOP
        ...
        SELECT sol.id, sol.product_type, sol.collection_name, sol.variant_name, sol.width_m, sol.height_m, sol.area, ql.drive_type, ql.cassette, ql.side_channel, sol.hardware_color, sol.quote_line_id INTO v_sol FROM "SaleOrderLines" sol LEFT JOIN "QuoteLines" ql ON ql.id = sol.quote_line_id WHERE sol.id = v_mo_line.sales_order_line_id AND sol.deleted = false;
        ...
        v_tbi := v_tbi + 1;
        FOR v_bc IN SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value, bc.qty_formula_code, bc.qty_formula_params, bc.uom FROM "BOMComponents" bc WHERE bc.bom_template_id = v_bt_id AND bc.deleted = false AND bc.component_item_id IS NOT NULL ORDER BY bc.created_at
        LOOP
            ...
            END IF;
            IF v_bc.qty_formula_code = 'CHAIN_HEIGHT_FACTOR' THEN
                v_fp := v_bc.qty_formula_params;
                IF v_fp IS NULL OR (v_fp->>'height_factor') IS NULL OR (v_fp->>'mult') IS NULL OR v_sol.height_m IS NULL THEN CONTINUE; END IF;
                v_qty := v_sol.height_m * COALESCE((v_fp->>'height_factor')::numeric, 0.75) * COALESCE((v_fp->>'mult')::numeric, 2);
            ELSIF v_bc.qty_type = 'per_width' THEN
