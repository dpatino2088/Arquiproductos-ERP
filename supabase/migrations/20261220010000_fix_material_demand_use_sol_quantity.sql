-- Fix material demand over-counting: multiply each BOM line by its OWN sales order
-- line quantity (sol.quantity), instead of the MO-level mo.quantity which is just
-- the FIRST line's quantity used as a blanket multiplier (wrong for multi-line MOs).
-- BOMInstances are 1 per sales_order_line (per single unit), so true material need
-- = SUM(bil.qty * sol.quantity).
CREATE OR REPLACE VIEW public.manufacturing_order_material_demand AS
 SELECT bi.manufacturing_order_id,
  ...
  WHERE bi.deleted = false AND bil.deleted = false AND mo.deleted = false AND (mol.status = ANY (ARRAY['reviewed'::text, 'confirmed'::text, 'procurement'::text, 'material_available'::text, 'materials_ready'::text, 'in_production'::text]))
