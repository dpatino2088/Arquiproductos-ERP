-- ============================================================================
-- Consumption Engine: FabricRules extension, dimension_outputs, per_fabric_width
-- Pasos 2-5 del plan Consumption Engine
-- ============================================================================

BEGIN;

-- ============================================================================
-- PASO 2: Ampliar FabricRules con columnas de consumo
-- ============================================================================
...
-- Backfill Dual rules (fix width_multiplier → panel_multiplier)
UPDATE "public"."FabricRules"
SET fabric_width_source = 'tube_width',
    panel_multiplier = 2,
    tube_wrap_mm = 35,
...
-- Backfill Triple rules (fix width_multiplier → panel_multiplier)
UPDATE "public"."FabricRules"
SET fabric_width_source = 'tube_width',
    panel_multiplier = 3,
    tube_wrap_mm = 35,
...
-- ============================================================================
-- PASO 3: dimension_outputs on ConfiguredProducts
-- ============================================================================
...
-- compute_system_dimensions: reads BOM deltas, produces dimension outputs
...
BEGIN
  ...
  -- Compute tube_width: width minus all component deltas that affect 'tube'
  ...
  -- Compute track_width (for drapery)
  SELECT COALESCE(SUM(COALESCE(bc.cut_delta_mm, 0)), 0)
  INTO v_delta_sum
  FROM "BOMComponents" bc
  WHERE bc.bom_template_id = v_cp.bt_id
...
-- ============================================================================
-- PASO 4: Enhanced compute_fabric_pricing_from_rule with dimension_outputs
-- ============================================================================
...
BEGIN
    ...
    fabric_cut_height_mm := NULL;
    fabric_width_used_m := NULL;
    ...
    -- Resolve fabric cut width based on fabric_width_source
    ...
    -- Resolve fabric cut height using panel_multiplier + wraps
...
-- ============================================================================
-- PASO 4b: Enhanced build_bom_preview_snapshot — uses FabricRule for roll_factor
-- ============================================================================
...
BEGIN
  ...
  -- Compute dimension outputs for consumption calculation
  ...
  IF v_cp.roll_catalog_item_id IS NOT NULL THEN
    ...
    -- Try FabricRule-based consumption first
    BEGIN
    ...
    IF v_consumption IS NOT NULL AND v_consumption.qty IS NOT NULL THEN
      ...
      -- Store fabric_width_used_m in dim_outputs for per_fabric_width components
...
'BOM preview JSONB with consumption engine. Uses FabricRule for real consumption, per_fabric_width qty_type. Totals UNIT.';


-- ============================================================================
-- PASO 4c: Enhanced calculate_configured_product_totals (no formula changes)
-- Uses rebuilt snapshot which now includes FabricRule-based roll_factor
-- ============================================================================
-- No changes needed here — the function reads from build_bom_preview_snapshot
-- which already computes correct roll_factor. The pricing ladder (COST->DEALER->MSRP)
-- remains untouched.


-- ============================================================================
-- PASO 5: per_fabric_width + per_spacing in bom_qty_type enum
-- ============================================================================

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'per_fabric_width' AND enumtypid = 'bom_qty_type'::regtype) THEN
    ALTER TYPE bom_qty_type ADD VALUE 'per_fabric_width';
  END IF;
EXCEPTION WHEN undefined_object THEN
  NULL;
END $$;

-- Also add per_fabric_width support to generate_bom_for_manufacturing_order
-- by adding a new ELSIF branch for per_fabric_width in the parent and child loops
...
BEGIN
    ...
    LOOP
        ...
        -- Try to get dimension_outputs from ConfiguredProduct
        BEGIN
