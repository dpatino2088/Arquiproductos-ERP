...
BEGIN
  ...
  IF NOT v_exempt_tax THEN
    SELECT COALESCE(cs.tax_pct, 0.07) INTO v_tax_pct
  ...
  SELECT COALESCE(SUM(
    CASE
      ...
      WHEN pl.line_type = 'from_quote' AND pl.quote_line_id IS NOT NULL THEN (
        SELECT CASE COALESCE(pl.override_mode::text, 'inherit')
        ...
        END
        FROM public."QuoteLines" ql WHERE ql.id = pl.quote_line_id LIMIT 1
      )
      ELSE 0
    END
  ...
  INTO v_discount_pct, v_fee FROM public."Proposals" p WHERE p.id = p_proposal_id;

  v_discount_amount := ROUND(v_subtotal * (v_discount_pct / 100.0), 2);
  v_taxable_base := GREATEST(v_subtotal - v_discount_amount, 0);

  IF v_exempt_tax THEN v_tax_amount := 0;
  ELSE v_tax_amount := ROUND(v_taxable_base * v_tax_pct, 2); END IF;

  v_total := ROUND(v_taxable_base + v_tax_amount + COALESCE(v_fee, 0), 2);

  UPDATE public."Proposals"
  SET subtotal_amount = v_subtotal, installation_amount = v_installation_total,
      discount_amount = v_discount_amount, tax_amount = v_tax_amount, total_amount = v_total
  WHERE id = p_proposal_id;
END;
$$;

COMMENT ON FUNCTION public.recalc_proposal_totals(uuid) IS 'Recalculates Proposals totals. When exempt_tax=true, tax_amount=0. Installation has own discount/fee.';
...
BEGIN
  IF OLD.global_discount_pct IS DISTINCT FROM NEW.global_discount_pct
     ...
     OR OLD.global_installation_fee_pct IS DISTINCT FROM NEW.global_installation_fee_pct
     OR OLD.exempt_tax IS DISTINCT FROM NEW.exempt_tax THEN
...
CREATE TRIGGER trg_proposals_recalc_totals
  AFTER UPDATE OF global_discount_pct, global_fee_amount, global_installation_discount_pct, global_installation_fee_pct, exempt_tax ON public."Proposals"
  FOR EACH ROW EXECUTE FUNCTION public.trg_proposals_recalc_totals();;
