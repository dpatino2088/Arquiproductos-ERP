...
-- Receives goods against a PO: creates InventoryMovement (receipt), updates InventoryBalances,
-- updates PurchaseOrderLines.received_qty, and PurchaseOrders.status.
...
BEGIN
  ...
  LOOP
    ...
    -- Receipt input is in purchase units (vendor-facing). For roll items, normalize to meters
    -- before updating inventory balances and movement lines.
