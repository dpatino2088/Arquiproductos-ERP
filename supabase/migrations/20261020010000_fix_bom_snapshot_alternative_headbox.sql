-- Fix BOM preview snapshot: when a template has MULTIPLE rows for the same
-- user-selectable role (e.g. two headbox alternatives: flat vs round, with/without
-- fabric, etc.), the function used to overwrite component_item_id of EVERY row
-- with the user's selection and emit BOTH rows as selected=true. That duplicated
-- the headbox in the breakdown and inflated bom_total.
--
-- Affected roles (all with a *_item_id user selection):
--   headbox, bottom_bar, side_channel, bottom_channel, tube, motor, drive, track
--
-- Fix: add a guard before processing each component. If the row's component_item_id
-- does NOT match the user's selection AND another row with the same role + matching
-- component_item_id exists in the same template → CONTINUE (skip the alternative).
--
-- Fallback: if no row in the template matches the selection (shouldn't happen with
-- HardwareStep options, but defensive), keep the previous behaviour and let the
-- first row through with item overwritten.
--
-- Also: when the matching row IS the right one, we no longer overwrite its
-- component_item_id with the user's selection (it's already the same), keeping
-- cost / msrp / children intact.
  ...
  IF p_bom_template_id IS NOT NULL THEN
    FOR v_comp IN SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value, bc.qty_delta_mm, bc.qty_spacing_mm, bc.qty_min, bc.uom, bc.parent_component_id, bc.sort_order, bc.per_panel, bc.condition_key, bc.condition_value, bc.is_required FROM public."BOMComponents" bc WHERE bc.bom_template_id = p_bom_template_id AND bc.organization_id = p_org_id AND bc.deleted = false AND bc.archived = false AND bc.parent_component_id IS NULL ORDER BY bc.sort_order ASC LOOP
      ...
      DECLARE
        ...
        v_has_selection_mechanism boolean := false;
      BEGIN
        CASE v_role_lower
        ...
        -- ✅ FIX (2026-10-20): if the template has MULTIPLE rows for this user-selectable
        -- role (e.g. several headbox alternatives), only process the row whose
        -- component_item_id matches the user's selection. Skip the others.
        -- Defensive: if no row matches, fall through to legacy behavior so we still
        -- emit something instead of an empty BOM.
        IF v_has_selection_mechanism
           AND v_selected_id IS NOT NULL
           AND v_comp.component_item_id IS DISTINCT FROM v_selected_id
           AND EXISTS (
             SELECT 1
             FROM public."BOMComponents" bc_alt
             WHERE bc_alt.bom_template_id   = p_bom_template_id
               AND bc_alt.organization_id   = p_org_id
               AND lower(bc_alt.component_role) = v_role_lower
               AND bc_alt.parent_component_id IS NULL
               AND bc_alt.deleted  = false
               AND bc_alt.archived = false
               AND bc_alt.component_item_id = v_selected_id
           ) THEN
          CONTINUE;  -- alternative not chosen by user
        END IF;

        IF v_selected_id IS NOT NULL THEN
          -- Only overwrite if the template row had no item assigned. When the row
          -- already references the user's selection, preserve it untouched so its
          -- own children/cost/msrp stay accurate.
