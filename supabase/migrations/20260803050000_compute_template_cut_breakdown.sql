-- compute_template_cut_breakdown: returns a JSONB array of cuttable breakdowns
-- for a BOM template. Uses the SAME logic as build_bom_preview_snapshot cascade
-- (affects_role + delta_x_mm/delta_y_mm per axis) to ensure a single auditable source of truth.
-- Children of external deductors are aggregated into their parent's combined delta
-- ONLY when the child also has affects_role matching the cuttable.
...
BEGIN
  ...
  LOOP
    ...
    -- Children with matching affects_role are aggregated into parent's combined delta.
    FOR v_deductor IN
