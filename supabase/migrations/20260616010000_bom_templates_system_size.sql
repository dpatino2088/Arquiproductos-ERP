-- ============================================================================
-- Add system_size column to BOMTemplates
-- Represents the track/rail profile size (e.g. 48mm, 60mm) for Drapery systems.
-- NULL = template applies to all system sizes.
-- ============================================================================
...
COMMENT ON COLUMN "public"."BOMTemplates"."system_size"
  IS 'Track/rail profile size (48mm, 60mm, etc.). NULL = applies to all sizes.';

-- 2. Backfill from existing template codes
UPDATE "public"."BOMTemplates"
SET system_size = '48mm'
WHERE system_size IS NULL
  AND code LIKE '%48MM%';

UPDATE "public"."BOMTemplates"
SET system_size = '60mm'
WHERE system_size IS NULL
  AND code LIKE '%60MM%';

-- 3. Update save_bom_template_batch RPC to include system_size
--    Also includes drive_type and installation_location which were missing.
