-- ============================================================
-- FIX: sync_quote_line_pricing_from_configured_product
-- Drop single-param overload, consolidate into (uuid, boolean)
-- Add unit_msrp, net_price, dealer tier fields
-- ============================================================

DROP FUNCTION IF EXISTS public.sync_quote_line_pricing_from_configured_product(uuid);
...
-- ============================================================
-- FIX: commit_configured_product_to_quote_line
-- Add unit_msrp and net_price columns to the INSERT
-- ============================================================
...
BEGIN
  ...
  VALUES (
    ...
    ROUND(COALESCE(v_cp.total_msrp, 0), 4),
...
-- ============================================================
-- BACKFILL: Populate unit_msrp and net_price on existing lines
-- ============================================================
UPDATE public."QuoteLines" ql
SET
  unit_msrp = CASE
    WHEN ql.quantity IS NOT NULL AND ql.quantity > 0 AND ql.msrp IS NOT NULL
    THEN ROUND(ql.msrp / ql.quantity, 4)
    ELSE ql.unit_msrp_total_snapshot
  END,
  net_price = ql.dealer_price_total
WHERE ql.unit_msrp IS NULL
  AND (ql.msrp IS NOT NULL AND ql.msrp > 0);


-- ============================================================
-- FIX: Re-sync QT-00103 anomalous line (stale CP snapshot)
-- ============================================================
DO $$
DECLARE
  v_line_id uuid := 'b1deac76-c89f-439d-8a80-cdf13be6f647';
BEGIN
  UPDATE public."QuoteLines"
  SET pricing_locked = false
  WHERE id = v_line_id;

  PERFORM public.sync_quote_line_pricing_from_configured_product(v_line_id, true);
END;
$$;
