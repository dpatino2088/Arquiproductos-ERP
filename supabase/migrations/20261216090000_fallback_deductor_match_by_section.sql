-- Cut breakdown deductor matching:
-- 1) Keep explicit `affects_role` matching (legacy templates).
-- 2) When a deductor has empty `affects_role` but a non-empty `placement_section`
--    (drive | passive | shared), fall back to the canonical cuttable for the axis:
--    width  -> tube / bottom_bar / bottom_channel
--    height -> side_channel
-- 3) Skip components flagged as `placement_section = 'cuttable'` from the deductor query
--    (they are themselves cuttables, not deductors).
    ...
    FOR v_deductor IN
      SELECT bc2.id, bc2.component_role, ci2.sku,
       ...
       WHERE bc2.bom_template_id = p_bom_template_id
         ...
         AND COALESCE(NULLIF(bc2.placement_section, ''), '') NOT IN ('cuttable')
         AND (
...
IS 'Core DB cut breakdown calculator. Falls back to placement_section + axis when affects_role is empty.';
