...
-- Backfill from legacy pivot table first (line-level intent preserved).
UPDATE public."PurchaseOrderLines" pol
SET
  allocation_type = 'manufacturing_order',
  allocation_mo_id = pomo.manufacturing_order_id
FROM public."PurchaseOrderManufacturingOrders" pomo
WHERE pomo.purchase_order_id = pol.purchase_order_id
  AND pol.allocation_mo_id IS NULL;

-- Fallback backfill from legacy PurchaseOrders header reference.
UPDATE public."PurchaseOrderLines" pol
SET
  allocation_type = 'manufacturing_order',
