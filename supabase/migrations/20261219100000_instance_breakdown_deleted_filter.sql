-- Hardening for compute_instance_cut_breakdown (Phase 4 follow-up):
--  1. Filter BOMInstanceLines by deleted = false on every read. After a BOM is
--     regenerated, old lines are soft-deleted; without this filter the display
--     could read a stale/deleted cut.
--  2. Skip core breakdown roles that the product does not actually include
--     (no produced line), so the work-order view only lists what is really cut
--     (avoids phantom rows like an unselected bottom_channel showing match=false).
...
BEGIN
  ...
  FOR v_elem IN SELECT value FROM jsonb_array_elements(v_core) LOOP
    ...
    -- match: per-panel -> all panels present; single-piece -> exact; multi-piece
    -- (side_channel/chain/brush store combined length) -> nearest integer multiple
    -- of the per-piece resolved cut.
  ...
  IF v_fab.fab_sku IS NOT NULL OR v_fab.cut_length_mm IS NOT NULL THEN
    ...
    IF v_fr IS NOT NULL THEN
      IF COALESCE(v_fr.fabric_width_source, 'finished_width') = 'tube_width' THEN v_fab_w := v_tube_width; v_fab_deds := v_fab_deds || jsonb_build_object('role','width_source','label','Ancho = Tubo resuelto','delta',v_tube_width,'qty',1,'total',v_tube_width,'conditional',false);
