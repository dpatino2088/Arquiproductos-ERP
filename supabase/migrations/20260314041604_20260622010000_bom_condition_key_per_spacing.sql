-- BOM condition_key filtering + per_spacing qty_type support
-- Allows a single template to have multiple glider components conditioned on system_size
-- (e.g., condition_key='system_size', condition_value='48mm' or '60mm')
-- The build_bom_preview_snapshot function will only include components whose condition matches
-- the config_snapshot.system_size, skipping the others.
...
DECLARE
  ...
  v_style_code text;
  v_dim_outputs jsonb;
  v_confection_pct numeric := 0;
  -- condition filtering
  v_config_system_size text;
  v_comp_cond_val text;
  v_skip_comp boolean;
BEGIN
  SELECT * INTO v_cp
  ...
  -- Extract system_size for condition filtering
  v_config_system_size := NULLIF(TRIM(COALESCE(v_config->>'system_size', '')), '');

  -- Fetch confection_pct from the matching FabricRule
  SELECT COALESCE(fr.confection_pct, 0) INTO v_confection_pct
  FROM public."FabricRules" fr
  WHERE fr.organization_id = p_org_id
    AND fr.product_type_id = v_cp.product_type_id
    AND fr.is_active = true
    AND (
      (v_style_code <> '' AND fr.style_code = v_style_code)
      OR (v_style_code = '' AND fr.style_code IS NULL)
    )
  ORDER BY fr.style_code NULLS LAST
  LIMIT 1;

  v_confection_pct := COALESCE(v_confection_pct, 0);
  ...
  IF v_cp.roll_catalog_item_id IS NOT NULL THEN
    ...
    IF v_consumption.qty IS NOT NULL THEN
      ...
      v_fabric_calc := jsonb_build_object(
        ...
        'confection_pct', v_confection_pct
      );
    ELSE
      ...
      v_fabric_calc := jsonb_build_object('source', 'legacy', 'confection_pct', v_confection_pct);
    END IF;
    ...
    v_roll_total_cost := COALESCE(v_roll_total_cost, 0) * (1 + v_confection_pct);
    ...
    v_roll_item := jsonb_build_object(
      ...
      'unit_cost', v_roll_total_cost,
      'cost_total', ROUND(v_roll_total_cost * v_roll_qty, 4),
      'children', '[]'::jsonb,
  ...
  IF p_bom_template_id IS NOT NULL THEN
    FOR v_comp IN
      SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value,
             bc.qty_delta_mm, bc.qty_spacing_mm, bc.uom, bc.parent_component_id, bc.sort_order,
    ...
    LOOP
      -- ── Condition filtering ──────────────────────────────────────────────────
      -- If a component has condition_key set, only include it when the config matches.
      v_skip_comp := false;
      IF v_comp.condition_key IS NOT NULL AND v_comp.condition_key <> '' THEN
        CASE v_comp.condition_key
          WHEN 'system_size' THEN
            v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config_system_size, '')), '');
          WHEN 'installation_type' THEN
            v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config->>'installation_location', '')), '');
          ELSE
            v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config->>v_comp.condition_key, '')), '');
        END CASE;
        IF v_comp_cond_val IS DISTINCT FROM NULLIF(TRIM(COALESCE(v_comp.condition_value, '')), '') THEN
          v_skip_comp := true;
        END IF;
      END IF;
      IF v_skip_comp THEN CONTINUE; END IF;
      -- ────────────────────────────────────────────────────────────────────────
      ...
      CASE COALESCE(v_comp.qty_type, 'fixed')
        ...
        WHEN 'per_spacing' THEN
          -- qty = ceil(width_mm / spacing_mm); qty_min enforced after
          IF COALESCE(v_comp.qty_spacing_mm, 0) > 0 THEN
            v_qty := CEIL(GREATEST(0, v_width_mm + COALESCE(v_comp.qty_delta_mm, 0))
                          / v_comp.qty_spacing_mm::numeric);
          ELSE
            v_qty := COALESCE(v_comp.qty_value, 1);
          END IF;
      ...
      FOR v_child IN
        SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value,
               bc.qty_delta_mm, bc.qty_spacing_mm, bc.uom,
               bc.condition_key, bc.condition_value
        FROM public."BOMComponents" bc
      ...
      LOOP
        ...
        v_skip_comp := false;
        IF v_child.condition_key IS NOT NULL AND v_child.condition_key <> '' THEN
          CASE v_child.condition_key
            WHEN 'system_size' THEN
              v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config_system_size, '')), '');
            WHEN 'installation_type' THEN
              v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config->>'installation_location', '')), '');
            ELSE
              v_comp_cond_val := NULLIF(TRIM(COALESCE(v_config->>v_child.condition_key, '')), '');
          END CASE;
          IF v_comp_cond_val IS DISTINCT FROM NULLIF(TRIM(COALESCE(v_child.condition_value, '')), '') THEN
            v_skip_comp := true;
          END IF;
        END IF;
        IF v_skip_comp THEN CONTINUE; END IF;
          ...
          CASE COALESCE(v_child.qty_type, 'fixed')
            ...
            WHEN 'per_m2', 'area' THEN
              v_child_qty := GREATEST(0, v_area_m2)
                             * COALESCE(v_child.qty_value, 1);
            WHEN 'per_spacing' THEN
              IF COALESCE(v_child.qty_spacing_mm, 0) > 0 THEN
                v_child_qty := CEIL(GREATEST(0, v_width_mm + COALESCE(v_child.qty_delta_mm, 0))
                                    / v_child.qty_spacing_mm::numeric);
              ELSE
                v_child_qty := COALESCE(v_child.qty_value, 1);
              END IF;
            ELSE
      ...
      v_items := v_items || jsonb_build_object(
        ...
        'children', v_children, 'meta', jsonb_build_object(
