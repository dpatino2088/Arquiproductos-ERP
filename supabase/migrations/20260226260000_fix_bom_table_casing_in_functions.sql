-- Fix table name casing: "BomInstances" / "BomInstanceLines" -> "BOMInstances" / "BOMInstanceLines"
-- The actual tables are "BOMInstances" and "BOMInstanceLines" (all-caps BOM).
-- Three functions were referencing them with mixed-case "BomInstances" which fails
-- because PostgreSQL treats double-quoted identifiers as case-sensitive.

-- ============================================================================
-- 1. generate_bom_for_manufacturing_order
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        IF NOT FOUND THEN
            v_warnings := v_warnings || format('No active BOMTemplate found for product_type: %s (SalesOrderLine: %s)', v_sales_order_line.product_type, v_sales_order_line.id);
            CONTINUE;
        END IF;
        ...
        VALUES (v_mo.organization_id, p_manufacturing_order_id, v_sales_order_line.id, v_sales_order_line.quote_line_id, v_bom_template_id, 'draft', false, now(), now(), now()) RETURNING id INTO v_bom_instance_id;
        ...
        LOOP
            IF v_bom_component.component_role = 'fabric' THEN
                ...
                IF NOT FOUND THEN
                    v_warnings := v_warnings || format('Fabric CatalogItem %s does NOT match SalesOrderLine. SalesOrderLine: %s', v_bom_component.component_item_id, v_sales_order_line.id);
                    CONTINUE;
                END IF;
            ELSE
                ...
                IF NOT FOUND THEN
                    v_warnings := v_warnings || format('CatalogItem %s (component_id: %s, role: %s) not found. SalesOrderLine: %s', v_bom_component.component_item_id, v_bom_component.id, v_bom_component.component_role, v_sales_order_line.id);
                    CONTINUE;
                END IF;
            END IF;
            ...
            IF v_unit_cost_exw > 0 THEN
                ...
                BEGIN
                    IF v_cost_from_msrp THEN v_unit_cost_with_taxes := v_unit_cost_exw; ELSE v_unit_cost_with_taxes := v_unit_cost_exw * (1 + (v_shipping_percentage / 100.0) + (v_import_tax_percentage / 100.0)); END IF;
...
-- ============================================================================
-- 2. issue_materials_for_manufacturing_order
-- ============================================================================
...
-- ============================================================================
-- 3. populate_bom_line_base_pricing_fields
-- ============================================================================
CREATE OR REPLACE FUNCTION public.populate_bom_line_base_pricing_fields(p_bom_instance_line_id uuid, p_catalog_item_id uuid, p_component_qty numeric, p_component_uom text, p_component_role text, p_organization_id uuid)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
