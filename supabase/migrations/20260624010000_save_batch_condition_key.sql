-- Fix save_bom_template_batch: persist condition_key/condition_value
-- The previous version only handled condition_json (which doesn't exist as a column).
-- The actual columns are condition_key (text) and condition_value (text), added in 20260611.
-- Also adds them to the return payload so the frontend receives them after save.
