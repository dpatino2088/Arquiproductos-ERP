-- Guard SaleOrderAccessories copy in create_sales_order_from_quote the same
-- way create_sales_order_on_quote_approve does: the table is missing in some
-- environments, and an unconditional INSERT rolled back the whole SO create.
...
BEGIN
  ...
  -- tracking_status is only valid while status = approved (check constraint).
  -- SO insert may have mirrored tracking onto the quote; clear it on convert.
  UPDATE "Quotes"
