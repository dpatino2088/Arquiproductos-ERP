...
AS $function$
DECLARE
    v_mo RECORD; v_mo_line RECORD; v_sol RECORD; v_bi_id uuid; v_bt_id uuid;
    v_bc RECORD; v_child RECORD; v_ci RECORD;
    v_fc integer := 0; v_qty numeric; v_uom text;
    ...
    v_cut_length numeric; v_cut_height numeric; v_dim_mm numeric;
    v_parent_cut_length numeric; v_parent_cut_height numeric;
BEGIN
    ...
    LOOP
        ...
        FOR v_bc IN SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value, bc.uom, bc.qty_spacing_mm, bc.qty_min, bc.cut_delta_mm, bc.depends_on_role FROM "BOMComponents" bc WHERE bc.bom_template_id = v_bt_id AND bc.deleted = false AND bc.component_item_id IS NOT NULL AND bc.parent_component_id IS NULL ORDER BY bc.sort_order, bc.created_at
        LOOP
            IF v_bc.component_role = 'fabric' THEN
                v_fc := v_fc + 1; IF v_fc > 1 THEN CONTINUE; END IF;
                SELECT ci.id, ci.sku, ci.name, ci.description, ci.cost_exw INTO v_ci FROM "CatalogItems" ci INNER JOIN "ItemCategories" ic ON ic.id = ci.item_category_id WHERE ci.id = v_bc.component_item_id AND ci.organization_id = v_mo.organization_id AND ci.is_active = true AND ic.code = 'FABRIC' AND ci.collection_name = v_sol.collection_name AND ((v_sol.variant_name IS NULL AND ci.variant_name IS NULL) OR ci.variant_name = v_sol.variant_name);
                IF NOT FOUND THEN v_warn := v_warn || format('Fabric mismatch %s', v_bc.component_item_id); CONTINUE; END IF;
            ELSE
                SELECT ci.id, ci.sku, ci.name, ci.description, ci.cost_exw INTO v_ci FROM "CatalogItems" ci WHERE ci.id = v_bc.component_item_id AND ci.organization_id = v_mo.organization_id AND ci.is_active = true;
            ...
            ELSIF v_bc.qty_type = 'per_spacing' THEN
                ...
                IF v_bc.depends_on_role IS NOT NULL AND v_bc.depends_on_role = 'height' THEN v_dim_mm := COALESCE(v_sol.height_m, 0) * 1000.0; END IF;
                IF v_dim_mm <= 0 THEN CONTINUE; END IF;
                v_qty := CEIL(v_dim_mm / COALESCE(v_bc.qty_spacing_mm, 500));
                IF v_bc.qty_min IS NOT NULL AND v_qty < v_bc.qty_min THEN v_qty := v_bc.qty_min; END IF;
            ...
            v_cut_length := CASE WHEN v_bc.qty_type='per_width' OR v_bc.component_role IN ('tube','bottom_bar','bottom_rail','bottom_rail_profile') THEN COALESCE(v_sol.width_m,0)*1000.0+COALESCE(v_bc.cut_delta_mm,0) ELSE NULL END;
            v_cut_height := CASE WHEN v_bc.qty_type='per_height' OR v_bc.component_role IN ('side_channel','side_channel_left','side_channel_right') THEN COALESCE(v_sol.height_m,0)*1000.0+COALESCE(v_bc.cut_delta_mm,0) ELSE NULL END;
            v_parent_cut_length := v_cut_length;
            v_parent_cut_height := v_cut_height;

            INSERT INTO "BOMInstanceLines" (organization_id, bom_instance_id, resolved_part_id, part_role, qty, uom, unit_cost_exw, total_cost_exw, unit_msrp, total_msrp, cut_length_mm, cut_height_mm, deleted, created_at, updated_at)
            VALUES (v_mo.organization_id, v_bi_id, v_ci.id, v_bc.component_role, v_qty, v_uom, COALESCE(v_ucx,0)::numeric(12,4), COALESCE(v_tcx,0)::numeric(12,4), COALESCE(v_um,0)::numeric(12,4), COALESCE(v_tm,0)::numeric(12,4), v_cut_length, v_cut_height, false, now(), now());
            v_cl := v_cl + 1; v_tbl := v_tbl + 1;

            FOR v_child IN SELECT cc.id, cc.component_role, cc.component_item_id, cc.qty_type, cc.qty_value, cc.uom, cc.qty_spacing_mm, cc.qty_min, cc.cut_delta_mm, cc.depends_on_role FROM "BOMComponents" cc WHERE cc.parent_component_id = v_bc.id AND cc.deleted = false AND cc.component_item_id IS NOT NULL ORDER BY cc.sort_order, cc.created_at
            LOOP
                SELECT ci.id, ci.sku, ci.name, ci.description, ci.cost_exw INTO v_ci FROM "CatalogItems" ci WHERE ci.id = v_child.component_item_id AND ci.organization_id = v_mo.organization_id AND ci.is_active = true;
                IF NOT FOUND THEN v_warn := v_warn || format('Child CatalogItem %s not found (parent %s)', v_child.component_item_id, v_bc.id); CONTINUE; END IF;

                IF v_child.qty_type = 'per_width' THEN
                    IF v_sol.width_m IS NULL THEN CONTINUE; END IF; v_qty := v_sol.width_m * COALESCE(v_child.qty_value, 1);
                ELSIF v_child.qty_type = 'per_height' THEN
                    IF v_sol.height_m IS NULL THEN CONTINUE; END IF; v_qty := v_sol.height_m * COALESCE(v_child.qty_value, 1);
                ELSIF v_child.qty_type = 'per_area' THEN
                    IF v_sol.width_m IS NULL OR v_sol.height_m IS NULL THEN CONTINUE; END IF; v_qty := v_sol.width_m * v_sol.height_m * COALESCE(v_child.qty_value, 1);
                ELSIF v_child.qty_type = 'per_spacing' THEN
                    v_dim_mm := COALESCE(v_parent_cut_length, COALESCE(v_sol.width_m,0)*1000.0);
                    IF v_child.depends_on_role IS NOT NULL AND v_child.depends_on_role = 'height' THEN v_dim_mm := COALESCE(v_parent_cut_height, COALESCE(v_sol.height_m,0)*1000.0); END IF;
                    IF v_dim_mm IS NULL OR v_dim_mm <= 0 THEN CONTINUE; END IF;
                    v_qty := CEIL(v_dim_mm / COALESCE(v_child.qty_spacing_mm, 500));
                    IF v_child.qty_min IS NOT NULL AND v_qty < v_child.qty_min THEN v_qty := v_child.qty_min; END IF;
                ELSIF v_child.qty_type = 'fixed' THEN v_qty := COALESCE(v_child.qty_value, 1);
                ELSE CONTINUE; END IF;
                IF v_qty IS NULL OR v_qty <= 0 THEN CONTINUE; END IF;

                v_uom := CASE WHEN UPPER(TRIM(v_child.uom)) IN ('PCS','PIECE','PIECES','SET','SETS','EA','EACH') THEN 'ea' WHEN v_child.component_role IN ('tube','bottom_bar','bottom_rail','bottom_rail_profile','chain') OR UPPER(TRIM(v_child.uom)) IN ('FT','FEET','FOOT','MTS','M','METER','METERS') THEN 'm' WHEN UPPER(TRIM(v_child.uom)) IN ('M2','SQM','SQ_M') THEN 'm2' ELSE NULL END;
                IF v_uom IS NULL THEN CONTINUE; END IF;
                ...
                v_cut_length := CASE WHEN v_child.qty_type='per_width' OR v_child.component_role IN ('tube','bottom_bar','bottom_rail','bottom_rail_profile') THEN COALESCE(v_sol.width_m,0)*1000.0+COALESCE(v_child.cut_delta_mm,0) ELSE NULL END;
                v_cut_height := CASE WHEN v_child.qty_type='per_height' OR v_child.component_role IN ('side_channel','side_channel_left','side_channel_right','brush') THEN COALESCE(v_sol.height_m,0)*1000.0+COALESCE(v_child.cut_delta_mm,0) ELSE NULL END;

                INSERT INTO "BOMInstanceLines" (organization_id, bom_instance_id, resolved_part_id, part_role, qty, uom, unit_cost_exw, total_cost_exw, unit_msrp, total_msrp, cut_length_mm, cut_height_mm, deleted, created_at, updated_at)
                VALUES (v_mo.organization_id, v_bi_id, v_ci.id, v_child.component_role, v_qty, v_uom, COALESCE(v_ucx,0)::numeric(12,4), COALESCE(v_tcx,0)::numeric(12,4), COALESCE(v_um,0)::numeric(12,4), COALESCE(v_tm,0)::numeric(12,4), v_cut_length, v_cut_height, false, now(), now());
                v_cl := v_cl + 1; v_tbl := v_tbl + 1;
            END LOOP;
        END LOOP;
    END LOOP;
