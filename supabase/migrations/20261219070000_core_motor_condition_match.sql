-- =====================================================================
-- Fix: compute_cut_breakdown_core skipped conditional deductors keyed on
-- `motor_item_id`.
--
-- BOMComponents store the motor deductor condition as condition_value = the
-- motor SKU / selection_code (e.g. 'CM-06'), but config_snapshot.motor_item_id
-- holds the catalog UUID. The core compared the raw UUID against the SKU, never
-- matched, and therefore dropped the motor's tube deduction (e.g. -15mm),
-- making motorized products resolve ~15mm too long in the preview and diverge
-- from the audited build_bom_preview_snapshot cascade (which already resolves
-- motor_item_id UUID -> SKU).
--
-- This patch resolves motor_item_id the same way the cascade does and uses the
-- resolved code when evaluating the deductor condition, so core == cascade and
-- production geometry closes exactly with the quoted cost.
--
-- Only the condition evaluation changes; all delta / per-panel / chaining logic
