...
-- 1. Add stock_length_mm to CatalogItems for profile stock lengths
ALTER TABLE "CatalogItems" ADD COLUMN IF NOT EXISTS stock_length_mm numeric(12,2);
COMMENT ON COLUMN "CatalogItems".stock_length_mm IS 'Stock bar/tube length in mm for 1D cut optimization';

-- 2. CutPlans - saved cut optimization plans
...
CREATE POLICY cut_plans_org ON "CutPlans" FOR ALL USING (is_org_member(organization_id));

-- 3. CutPlanLines - individual pieces in a cut plan
...
CREATE POLICY cut_plan_lines_via_plan ON "CutPlanLines" FOR ALL USING (
  cut_plan_id IN (SELECT cp.id FROM "CutPlans" cp WHERE is_org_member(cp.organization_id))
);

-- 4. FabricConstructionSpecs - constructive details per fabric type
...
CREATE POLICY fabric_specs_org ON "FabricConstructionSpecs" FOR ALL USING (is_org_member(organization_id));

DO $$ BEGIN RAISE NOTICE 'Cut optimization tables created'; END $$;;
