...
DECLARE
    ...
    -- Legacy BOMComponents variables
...
BEGIN
    ...
    -- STEP 3: AUTO-CREATE ManufacturingOrderLines from SalesOrderLines
    IF v_mo_lines_before = 0 AND v_mo.sales_order_id IS NOT NULL THEN
        INSERT INTO public."ManufacturingOrderLines"(
            manufacturing_order_id, sales_order_line_id, organization_id, status
        )
        SELECT
            p_manufacturing_order_id, sol.id,
    ...
    SELECT COUNT(*) INTO v_mo_lines_after
    FROM public."ManufacturingOrderLines"
    ...
    IF v_mo_lines_after = 0 THEN
        ...
        RETURN jsonb_build_object(
            'ok', true,
            'mo_lines_before', v_mo_lines_before, 'mo_lines_after', v_mo_lines_after,
    ...
    SELECT shipping_percentage, import_tax_percent, min_margin_pct,
           discount_distributor_pct, labor_percentage
    INTO v_cost_settings
    FROM "CostSettings"
    WHERE organization_id = v_mo.organization_id AND deleted = false
    LIMIT 1;
    ...
    FOR v_mo_line IN
        ...
        WHERE mol.manufacturing_order_id = p_manufacturing_order_id
            AND mol.deleted = false AND mol.archived = false
    ...
    LOOP
        ...
        SELECT sol.id, sol.product_type, sol.collection_name, sol.variant_name,
               sol.width_m, sol.height_m, sol.area, sol.drive_type,
               sol.cassette, sol.cassette_type, sol.side_channel, sol.side_channel_type,
               sol.hardware_color, sol.bottom_rail_type, sol.quote_line_id
        ...
        WHERE pt.code = v_sales_order_line.product_type
            AND bt.active = true AND bt.deleted = false
        ORDER BY bt.created_at DESC
        LIMIT 1;

        IF NOT FOUND THEN
            v_warnings := v_warnings || format('No active BOMTemplate for product_type: %s (SOL: %s)',
        ...
        WHERE bil.bom_instance_id IN (
            ...
            WHERE bi.manufacturing_order_id = p_manufacturing_order_id
                AND bi.sales_order_line_id = v_sales_order_line.id AND bi.deleted = false
        );
        ...
        -- STEP 5d: Create BomInstance
        INSERT INTO "BomInstances" (
            organization_id, manufacturing_order_id, sales_order_line_id,
            quote_line_id, bom_template_id, status, deleted,
            created_at, updated_at, generated_at
        ) VALUES (
            v_mo.organization_id, p_manufacturing_order_id, v_sales_order_line.id,
            v_sales_order_line.quote_line_id, v_bom_template_id, 'draft', false,
            now(), now(), now()
        ) RETURNING id INTO v_bom_instance_id;

        IF v_bom_instance_id IS NULL THEN
            v_warnings := v_warnings || format('Failed to create BomInstance for SOL %s', v_sales_order_line.id);
            CONTINUE;
        END IF;

        v_total_bom_instances := v_total_bom_instances + 1;

        -- STEP 5e: Try ConfiguredProducts.bom_preview_snapshot FIRST
        v_configured_product_id := NULL;
        v_snapshot := NULL;

        IF v_sales_order_line.quote_line_id IS NOT NULL THEN
            SELECT ql.configured_product_id INTO v_configured_product_id
            FROM "QuoteLines" ql
            WHERE ql.id = v_sales_order_line.quote_line_id;
        END IF;

        IF v_configured_product_id IS NOT NULL THEN
            SELECT cp.bom_preview_snapshot INTO v_snapshot
            FROM "ConfiguredProducts" cp
            WHERE cp.id = v_configured_product_id
                AND cp.organization_id = v_mo.organization_id
                AND cp.deleted = false;
        END IF;

        v_snapshot_items := CASE
            WHEN v_snapshot IS NOT NULL AND v_snapshot ? 'items'
                 AND jsonb_array_length(v_snapshot->'items') > 0
            THEN v_snapshot->'items'
            ELSE NULL
        END;

        IF v_snapshot_items IS NOT NULL THEN
            -- PRIMARY PATH: Create BomInstanceLines from snapshot items
            v_used_snapshot := true;
            RAISE NOTICE '[BOM-GEN] Using ConfiguredProduct snapshot (cp_id: %, items: %)',
                v_configured_product_id::text, jsonb_array_length(v_snapshot_items);
            ...
            LOOP
                ...
                v_item_sku  := v_item->>'sku';
                v_item_role := COALESCE(v_item->>'role', 'component');
                v_item_qty  := COALESCE((v_item->>'qty')::numeric, 0);
                v_item_uom  := COALESCE(v_item->>'uom', 'ea');
                v_item_name := COALESCE(v_item->>'name', v_item_sku);
                v_item_kind := COALESCE(v_item->>'kind', 'parent');

                IF v_item_qty <= 0 THEN CONTINUE; END IF;

                v_category_code := CASE
                    WHEN v_item_kind = 'roll' OR v_item_role = 'fabric' THEN 'fabric'
                    WHEN v_item_role = 'tube' THEN 'tube'
                    WHEN v_item_role = 'motor' THEN 'motor'
                    WHEN v_item_role = 'bracket' THEN 'bracket'
                    WHEN v_item_role LIKE '%cassette%' THEN 'cassette'
                    WHEN v_item_role LIKE '%side_channel%' THEN 'side_channel'
                    WHEN v_item_role LIKE '%bottom_rail%' OR v_item_role LIKE '%bottom_channel%'
                         OR v_item_role LIKE '%bottom_bar%' THEN 'bottom_channel'
                    WHEN v_item_role = 'drive' THEN 'drive'
                    ELSE 'accessory'
                END;
                ...
                WHERE cim.catalog_item_id = v_item_catalog_id
                    AND cim.organization_id = v_mo.organization_id
                    AND cim.total_cost IS NOT NULL
                LIMIT 1;
                IF FOUND AND v_unit_cost_exw IS NOT NULL THEN
                    v_cost_from_msrp := true;
                ELSE
                    SELECT COALESCE(ci.cost_exw, 0)::numeric(12,4) INTO v_unit_cost_exw
                    FROM "CatalogItems" ci WHERE ci.id = v_item_catalog_id;
                    v_unit_cost_exw := COALESCE(v_unit_cost_exw, 0);
                END IF;
                ...
                IF v_unit_cost_exw > 0 THEN
                    ...
                    BEGIN
                        ...
                        ELSE
                            v_unit_cost_with_taxes := v_unit_cost_exw
                                * (1 + (v_shipping_percentage / 100.0) + (v_import_tax_percentage / 100.0));
                        END IF;
                ...
                DECLARE
                    v_cut_l numeric(12,4) := NULL;
                BEGIN
                    IF v_item_role IN ('tube', 'bottom_bar', 'bottom_rail', 'bottom_rail_profile') THEN
                        v_cut_l := COALESCE(v_sales_order_line.width_m, 0) * 1000.0;
                    END IF;

                    INSERT INTO "BomInstanceLines" (
                        organization_id, bom_instance_id,
                        resolved_part_id, resolved_sku, part_role,
                        qty, uom, description, category_code,
                        unit_cost_exw, total_cost_exw,
                        unit_msrp_sale_out, total_msrp_sale_out,
                        cut_l_mm, deleted, created_at, updated_at
                    ) VALUES (
                        v_mo.organization_id, v_bom_instance_id,
                        v_item_catalog_id, v_item_sku, v_item_role,
                        v_item_qty, v_item_uom, v_item_name, v_category_code,
                ...
                IF v_item ? 'children' AND jsonb_array_length(COALESCE(v_item->'children', '[]'::jsonb)) > 0 THEN
                    ...
                    LOOP
                        ...
                        v_item_sku  := v_child->>'sku';
                        v_item_role := COALESCE(v_child->>'role', 'child');
                        v_item_qty  := COALESCE((v_child->>'qty')::numeric, 0);
                        v_item_uom  := COALESCE(v_child->>'uom', 'ea');
                        v_item_name := COALESCE(v_child->>'name', v_item_sku);

                        IF v_item_qty <= 0 THEN CONTINUE; END IF;

                        v_category_code := CASE
                            WHEN v_item_role LIKE '%chain%' THEN 'chain'
                            WHEN v_item_role LIKE '%end_cap%' THEN 'end_cap'
                            ELSE 'accessory'
                        END;
                        ...
                        IF v_unit_cost_exw > 0 THEN
                            DECLARE
                                v_c_cost_with_taxes numeric(12,4);
                                v_c_msrp_sale_in numeric(12,4);
                            BEGIN
                                IF v_cost_from_msrp THEN
                                    v_c_cost_with_taxes := v_unit_cost_exw;
                                ELSE
                                    v_c_cost_with_taxes := v_unit_cost_exw
                                        * (1 + (v_shipping_percentage / 100.0) + (v_import_tax_percentage / 100.0));
                                END IF;
                                v_c_msrp_sale_in := v_c_cost_with_taxes / (1 - (v_min_margin_pct / 100.0));
                                v_unit_msrp_sale_out := v_c_msrp_sale_in / (1 - (v_max_discount_pct / 100.0));
                                v_total_msrp_sale_out := v_unit_msrp_sale_out * v_item_qty;
                            END;
                        ELSE
                            v_unit_msrp_sale_out := 0;
                        ...
                        INSERT INTO "BomInstanceLines" (
                            ...
                            v_mo.organization_id, v_bom_instance_id,
                            v_item_catalog_id, v_item_sku, v_item_role, v_item_qty,
                            v_item_uom, v_item_name, v_category_code,
                            COALESCE(v_unit_cost_exw, 0)::numeric(12,4),
                            COALESCE(v_total_cost_exw, 0)::numeric(12,4),
                            COALESCE(v_unit_msrp_sale_out, 0)::numeric(12,4),
                            COALESCE(v_total_msrp_sale_out, 0)::numeric(12,4),
        ...
        ELSE
            -- FALLBACK: Legacy BOMComponents-based generation
            v_warnings := v_warnings || format('No ConfiguredProduct snapshot for SOL %s; using legacy BOMComponents', v_sales_order_line.id);
            v_fabric_count := 0;

            FOR v_bom_component IN
                SELECT bc.id, bc.component_role, bc.component_item_id,
                       bc.qty_type, bc.qty_value, bc.qty_formula_code,
                       bc.qty_formula_params, bc.uom
                FROM "BOMComponents" bc
                WHERE bc.bom_template_id = v_bom_template_id
                    AND bc.deleted = false AND bc.component_item_id IS NOT NULL
                ORDER BY bc.created_at
            LOOP
                IF v_bom_component.component_role = 'fabric' THEN
                    v_fabric_count := v_fabric_count + 1;
                    IF v_fabric_count > 1 THEN CONTINUE; END IF;
                    ...
                    WHERE ci.id = v_bom_component.component_item_id
                        ...
                        AND ci.deleted = false AND ci.active = true
                        AND ic.code = 'FABRIC'
                        AND ci.collection_name = v_sales_order_line.collection_name
                        AND ((v_sales_order_line.variant_name IS NULL AND ci.variant_name IS NULL)
                             OR ci.variant_name = v_sales_order_line.variant_name);

                    IF NOT FOUND THEN
                        v_warnings := v_warnings || format('Fabric CatalogItem %s does NOT match SOL collection/variant. SOL: %s',
                            v_bom_component.component_item_id, v_sales_order_line.id);
                        CONTINUE;
                    END IF;
                ELSE
                    SELECT ci.id, ci.sku, ci.item_name, ci.description, ci.cost_exw
                    ...
                    IF NOT FOUND THEN
                        v_warnings := v_warnings || format('CatalogItem %s (role: %s) not found. SOL: %s',
                            v_bom_component.component_item_id, v_bom_component.component_role, v_sales_order_line.id);
                        CONTINUE;
                    END IF;
                END IF;
                ...
                IF v_bom_component.qty_formula_code IS NOT NULL THEN
                    IF v_bom_component.qty_formula_code = 'CHAIN_HEIGHT_FACTOR' THEN
                        ...
                        IF v_formula_params IS NULL OR (v_formula_params->>'height_factor') IS NULL
                           OR (v_formula_params->>'mult') IS NULL THEN CONTINUE; END IF;
                ...
                SELECT cim.total_cost INTO v_unit_cost_exw
                FROM "CatalogItemsMSRP" cim
                WHERE cim.catalog_item_id = v_catalog_item.id
                    AND cim.organization_id = v_mo.organization_id AND cim.total_cost IS NOT NULL
                LIMIT 1;
                IF FOUND AND v_unit_cost_exw IS NOT NULL THEN
                    v_cost_from_msrp := true;
                ELSE
                ...
                IF v_unit_cost_exw > 0 THEN
                    DECLARE
                        v_lc_cost_with_taxes numeric(12,4);
                        v_lc_msrp_sale_in numeric(12,4);
                    BEGIN
                        IF v_cost_from_msrp THEN
                            v_lc_cost_with_taxes := v_unit_cost_exw;
                        ELSE
                            v_lc_cost_with_taxes := v_unit_cost_exw
                                * (1 + (v_shipping_percentage / 100.0) + (v_import_tax_percentage / 100.0));
                        END IF;
                        v_lc_msrp_sale_in := v_lc_cost_with_taxes / (1 - (v_min_margin_pct / 100.0));
                        v_unit_msrp_sale_out := v_lc_msrp_sale_in / (1 - (v_max_discount_pct / 100.0));
                ...
                INSERT INTO "BomInstanceLines" (
                    ...
                    v_mo.organization_id, v_bom_instance_id,
                    v_catalog_item.id, v_catalog_item.sku, v_bom_component.component_role,
                    v_calculated_qty, v_normalized_uom,
        ...
        IF v_created_lines = 0 THEN
            v_warnings := v_warnings || format('BomInstance %s: 0 lines generated (template: %s, SOL: %s, snapshot: %s)',
                v_bom_instance_id, v_bom_template_id, v_sales_order_line.id,
                CASE WHEN v_used_snapshot THEN 'yes' ELSE 'no' END);
        END IF;
        ...
        DECLARE v_bom_msrp_sale_in numeric(12,4);
        BEGIN
    ...
    -- STEP 6: Counts AFTER
    SELECT COUNT(*) INTO v_bom_instances_after
    FROM public."BomInstances"
    WHERE manufacturing_order_id = p_manufacturing_order_id AND deleted = false;

    SELECT COUNT(*) INTO v_bom_lines_after
    FROM public."BomInstanceLines" bil
    JOIN public."BomInstances" bi ON bi.id = bil.bom_instance_id
    WHERE bi.manufacturing_order_id = p_manufacturing_order_id
        AND bi.deleted = false AND bil.deleted = false;
...
COMMENT ON FUNCTION public.generate_bom_for_manufacturing_order IS
    'Generates BOM for a ManufacturingOrder. Primary: reads ConfiguredProducts.bom_preview_snapshot via SalesOrderLine->QuoteLine->ConfiguredProduct. Fallback: legacy BOMComponents. Auto-creates ManufacturingOrderLines if missing.';
