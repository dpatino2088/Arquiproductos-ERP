...
BEGIN
  ...
  v_bottom_hem_override := CASE
    WHEN v_config ? 'bottom_hem_cm'
         AND v_config->>'bottom_hem_cm' IS NOT NULL
         AND v_config->>'bottom_hem_cm' != 'null'
    THEN (v_config->>'bottom_hem_cm')::numeric
    ELSE NULL
  END;
  ...
  IF v_cp.roll_catalog_item_id IS NOT NULL THEN
    ...
    IF v_consumption.qty IS NOT NULL THEN
      ...
      v_fabric_calc := jsonb_build_object(
        ...
        'bottom_hem_cm_override', v_bottom_hem_override
      );
    ELSE
