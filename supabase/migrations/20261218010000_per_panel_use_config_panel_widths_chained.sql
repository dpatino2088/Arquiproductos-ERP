-- Per-panel cut breakdown: respect config_snapshot.panels[i].width_mm and
-- chain depends_on_role through per-panel cuts (not resolved_mm / N).
--
-- Bug fixed: when bottom_bar (depends_on_role = tube) is rendered per-panel,
-- the global resolved_mm correctly inherits from tube_resolved, but each panel
-- previously used `v_width_mm / v_panel_count` (raw total / N) as base. With
-- equal panels this disguises the bug; with unequal panels (e.g. 1000+1200+1500)
-- the sum of panel_cuts diverges from resolved_mm.
--
-- Fix: maintain a v_panel_cuts_map jsonb of `{ role: [cut_p1, cut_p2, ...] }`
-- and prefer the parent's per-panel cut whenever depends_on_role is resolved.
-- Falls back to config_snapshot.panels[i].width_mm, then to total / N.
--
-- See .cursor/rules/per-panel-deductions.mdc for the canonical rules.

CREATE OR REPLACE FUNCTION public.compute_cut_breakdown_core(
  p_org_id uuid,
  p_bom_template_id uuid,
  p_config_snapshot jsonb DEFAULT '{}'::jsonb,
  p_width_mm numeric DEFAULT NULL::numeric,
  p_height_mm numeric DEFAULT NULL::numeric,
...
DECLARE
  ...
  -- Per-panel cuts indexed by role: { "tube": [cut_p1, cut_p2, ...], ... }.
  -- Used so chained cuttables (e.g. bottom_bar depends_on tube) can use the
  -- parent's per-panel cut as their base, not the resolved_mm divided by N.
...
BEGIN
  ...
  LOOP
    ...
    IF v_cuttable.per_panel AND v_panel_count > 1 THEN
      -- Look up parent's per-panel cuts once per cuttable, if depends_on chain.
      ...
      FOR v_p_idx IN 1..v_panel_count LOOP
        ...
        ELSIF v_parent_panels IS NOT NULL THEN
          -- Chained: use parent's per-panel cut as our base
        ...
        ELSIF v_panels IS NOT NULL AND jsonb_typeof(v_panels) = 'array' AND v_p_idx <= jsonb_array_length(v_panels) THEN
          -- Top-level: use this panel's declared width from config_snapshot
          v_p_width := COALESCE(((v_panels -> (v_p_idx - 1)) ->> 'width_mm')::numeric, v_width_mm / v_panel_count);
        ELSE
          -- Fallback: even split (only when no panels declared)
        ...
        -- Note: tolerance is the cuttable's own cut_delta_mm (a fudge factor).
        -- When chained, the parent already applied its own tolerance, so we
        -- only add THIS cuttable's tolerance here.
        ...
        -- Persist this panel cut so chained children can use it.
        v_panel_cuts_map := jsonb_set(
          v_panel_cuts_map,
          ARRAY[v_role],
          COALESCE(v_panel_cuts_map -> v_role, '[]'::jsonb) || to_jsonb(v_p_cut),
          true
        );
      END LOOP;
    END IF;
