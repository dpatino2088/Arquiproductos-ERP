-- =====================================================================
-- Fix: exclusive cuttable/deductor options (e.g. two headboxes in one
-- template) were both included by compute_cut_breakdown_core whenever
-- config_snapshot.headbox_item_id was not literally 'NONE'.
--
-- Assembly Workstation then showed RC3131-W + RC3051-W and double-deducted
-- both from tube/bottom_bar, while BOMInstanceLines (CP snapshot) only had
-- the selected headbox.
--
-- When config carries a UUID for {role}_item_id (and headbox/cassette
-- aliases), only that catalog item participates as cuttable or deductor.
-- Engineering preview without a selection (null) still shows all options.
-- =====================================================================
...
BEGIN
  ...
  IF v_drive_side NOT IN ('left', 'right', 'both') THEN
    v_drive_side := 'left';
  END IF;
  -- Resolve the selected motor's SKU/selection_code so conditional deductors
  -- keyed on motor_item_id (which carry the SKU in condition_value) match the
  -- catalog UUID stored in config_snapshot.motor_item_id.
  v_motor_code := (
    SELECT COALESCE(NULLIF(TRIM(ci_m.selection_code), ''), ci_m.sku, '')
    FROM public."CatalogItems" ci_m
    WHERE ci_m.id = public.try_parse_uuid(v_config_snap ->> 'motor_item_id')
      AND ci_m.organization_id = p_org_id
    LIMIT 1
  );
  FOR v_cuttable IN
  ...
  LOOP
    ...
    -- Optional headbox/cassette off: either shared key may carry NONE.
    IF lower(v_role) IN ('headbox', 'cassette')
       AND COALESCE(
             NULLIF(v_config_snap ->> 'headbox_item_id', ''),
             NULLIF(v_config_snap ->> 'cassette_item_id', '')
           ) = 'NONE' THEN
      CONTINUE;
    END IF;
    IF v_config_snap ->> v_role = 'false' THEN CONTINUE; END IF;
    -- Exclusive option groups (headbox/drive/etc.): only the CP-selected item
    v_selected_id := CASE lower(v_role)
      WHEN 'headbox' THEN COALESCE(
        public.try_parse_uuid(v_config_snap ->> 'headbox_item_id'),
        public.try_parse_uuid(v_config_snap ->> 'cassette_item_id')
      )
      WHEN 'cassette' THEN COALESCE(
        public.try_parse_uuid(v_config_snap ->> 'cassette_item_id'),
        public.try_parse_uuid(v_config_snap ->> 'headbox_item_id')
      )
      ELSE public.try_parse_uuid(v_config_snap ->> (v_role || '_item_id'))
    END;
    IF v_selected_id IS NOT NULL
       AND v_cuttable.component_item_id IS DISTINCT FROM v_selected_id THEN
      CONTINUE;
    END IF;
    ...
    v_total_ded := 0;
    FOR v_deductor IN
      SELECT bc2.id, bc2.component_role, bc2.component_item_id, ci2.sku,
    ...
    LOOP
      IF v_deductor.condition_key IS NOT NULL AND v_deductor.condition_key <> '' THEN
        IF NOT ( v_deductor.condition_key = 'drive_side' AND COALESCE(v_config_snap ->> 'drive_side', '') = 'both' AND COALESCE(v_deductor.condition_value, '') IN ('left', 'right') )
           AND COALESCE(CASE WHEN v_deductor.condition_key = 'motor_item_id' THEN v_motor_code ELSE v_config_snap ->> v_deductor.condition_key END, '') <> COALESCE(v_deductor.condition_value, '') THEN
          CONTINUE;
        END IF;
      END IF;
      IF v_config_snap ->> (v_deductor.component_role || '_item_id') = 'NONE' THEN CONTINUE; END IF;
      IF lower(COALESCE(v_deductor.component_role, '')) IN ('headbox', 'cassette')
         AND COALESCE(
               NULLIF(v_config_snap ->> 'headbox_item_id', ''),
               NULLIF(v_config_snap ->> 'cassette_item_id', '')
             ) = 'NONE' THEN
        CONTINUE;
      END IF;
      v_selected_id := CASE lower(COALESCE(v_deductor.component_role, ''))
        WHEN 'headbox' THEN COALESCE(
          public.try_parse_uuid(v_config_snap ->> 'headbox_item_id'),
          public.try_parse_uuid(v_config_snap ->> 'cassette_item_id')
        )
        ...
        ELSE public.try_parse_uuid(v_config_snap ->> (v_deductor.component_role || '_item_id'))
      END;
      IF v_selected_id IS NOT NULL
         AND v_deductor.component_item_id IS DISTINCT FROM v_selected_id THEN
        CONTINUE;
      END IF;
    ...
    LOOP
      IF v_child_ded.condition_key IS NOT NULL AND v_child_ded.condition_key <> '' THEN
        IF NOT ( v_child_ded.condition_key = 'drive_side' AND COALESCE(v_config_snap ->> 'drive_side', '') = 'both' AND COALESCE(v_child_ded.condition_value, '') IN ('left', 'right') )
           AND COALESCE(CASE WHEN v_child_ded.condition_key = 'motor_item_id' THEN v_motor_code ELSE v_config_snap ->> v_child_ded.condition_key END, '') <> COALESCE(v_child_ded.condition_value, '') THEN
          CONTINUE;
        END IF;
      END IF;
        ...
        LOOP
          IF v_deductor.dinter THEN
            IF v_is_edge_left OR v_is_edge_right THEN
              v_p_deds := v_p_deds || jsonb_build_object('role', v_deductor.drole, 'sku', v_deductor.dsku, 'delta', v_deductor.ddelta / 2, 'qty', 1, 'total', v_deductor.ddelta / 2, 'note', 'half intermediate');
              v_p_total_ded := v_p_total_ded + (v_deductor.ddelta / 2);
            ELSE
              v_p_deds := v_p_deds || jsonb_build_object('role', v_deductor.drole, 'sku', v_deductor.dsku, 'delta', v_deductor.ddelta / 2, 'qty', 2, 'total', v_deductor.ddelta, 'note', 'half left + half right');
