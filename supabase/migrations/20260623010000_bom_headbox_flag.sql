-- BOM Templates: explicit headbox boolean flag
-- Marks whether a template includes a headbox/cassette assembly.
-- Used to pre-filter templates in the configurator (roller=optional, triple=required).

-- 1. Add column
ALTER TABLE "public"."BOMTemplates"
  ADD COLUMN IF NOT EXISTS headbox boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN "public"."BOMTemplates"."headbox"
  IS 'Whether this template includes a headbox/cassette assembly. '
     'Used to pre-filter templates in the configurator. '
     'roller/dual-shade: optional (user picks). triple: always true.';

-- 2. Auto-populate from existing BOMComponents (role = headbox)
UPDATE "public"."BOMTemplates" t
SET headbox = true
WHERE EXISTS (
  SELECT 1 FROM "public"."BOMComponents" bc
  WHERE bc.bom_template_id = t.id
    AND bc.component_role = 'headbox'
    AND bc.deleted = false
    AND bc.archived = false
);

-- 3. Rebuild save_bom_template_batch to include headbox
...
BEGIN
  ...
  LOOP
    ...
    IF v_comp_id IS NOT NULL THEN
      UPDATE "BOMComponents" SET
        ...
        sequence_order      = COALESCE((v_comp ->> 'sequence_order')::int, 0),
        ...
        condition_json      = CASE WHEN v_comp -> 'condition_json' IS NOT NULL THEN v_comp -> 'condition_json' ELSE NULL END,
    ...
    ELSE
      INSERT INTO "BOMComponents" (
        ...
        sort_order, sequence_order,
        is_required, per_panel, auto_select,
        condition_json, deleted, archived,
        ...
        COALESCE((v_comp ->> 'sequence_order')::int, 0),
        COALESCE((v_comp ->> 'is_required')::boolean, true),
        COALESCE((v_comp ->> 'per_panel')::boolean, false),
        COALESCE((v_comp ->> 'auto_select')::boolean, false),
        CASE WHEN v_comp -> 'condition_json' IS NOT NULL THEN v_comp -> 'condition_json' ELSE NULL END,
        false, false,
        v_now, v_now
      )
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'sequence_order', bc.sequence_order,
