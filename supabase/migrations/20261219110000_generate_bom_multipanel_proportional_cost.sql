-- Fase 3b: reparto proporcional de costo en el split multi-panel.
--
-- Antes: al dividir una linea cuttable de UOM 'm' en N paneles, el costo de
-- cada panel se calculaba como (costo_unitario_por_m * metros_del_panel).
-- Como costo_unitario_por_m = total_congelado / qty_congelado, la suma de los
-- paneles solo igualaba al total congelado del quote si qty_congelado == suma
-- de metros de los paneles (cierra dentro de la tolerancia de reconciliacion
-- del 1%, pero podia driftar hasta ~1%).
--
-- Ahora: el costo (y MSRP) congelado de la linea origen se reparte por PESO de
-- cada corte (panel_cut_mm / sum(panel_cut_mm)). El ULTIMO panel absorbe el
-- redondeo, de modo que SUM(total_cost_exw de los paneles) == total_cost_exw
-- congelado de la linea origen EXACTAMENTE. La geometria (qty/cut_length_mm)
-- sigue saliendo de compute_cut_breakdown_core; solo cambia como se distribuye
-- el costo ya congelado del quote. Aplica tanto a roles cuttables genericos
-- como a la tela (fabric) en escenarios multi-panel.
--
-- Single-panel: sin cambios. Solo se sobreescribe cut_length_mm desde core
-- (con guardia multi-pieza y reconciliacion); qty y costos permanecen
-- congelados desde el snapshot.
...
BEGIN
    ...
    LOOP
        v_mlp := v_mlp + 1; v_dim_outputs := '{}'::jsonb;
        ...
        LOOP
            ...
            IF v_panel_cuts IS NOT NULL AND jsonb_typeof(v_panel_cuts) = 'array' AND jsonb_array_length(v_panel_cuts) > 1 THEN
                ...
                LOOP
                    ...
                    END IF;
                    -- reparto proporcional: costo del panel ponderado por su corte; ultimo panel absorbe el redondeo -> suma exacta = costo congelado de la linea origen
        ...
        IF v_panel_cuts IS NOT NULL AND jsonb_typeof(v_panel_cuts) = 'array' AND jsonb_array_length(v_panel_cuts) > 1 THEN
            ...
            LOOP
                ...
                UPDATE public."BOMInstanceLines" SET deleted = true, updated_at = now() WHERE id = v_src_line.id;
                FOR v_pc IN SELECT value FROM jsonb_array_elements(v_panel_cuts) AS value LOOP
                    v_panel_idx := (v_pc->>'index')::integer; v_panel_cut_mm := COALESCE((v_pc->>'tube_width_mm')::numeric, 0);
                    IF v_panel_cut_mm <= 0 THEN CONTINUE; END IF;
                    v_pc_seen := v_pc_seen + 1; v_qty := ROUND(v_panel_cut_mm / 1000.0, 4);
                    IF v_pc_seen = v_pc_count THEN
                        v_panel_cost := ROUND(v_frozen_cost - v_acc_cost, 4); v_panel_msrp := ROUND(v_frozen_msrp - v_acc_msrp, 4);
