-- ============================================================================
-- Allow Work Order generation even when materials are incomplete.
-- The block should be at production start (transition_mo_status), not planning.
-- generate_work_orders_for_mo: remove material readiness check.
-- ============================================================================
...
-- Also update transition_mo_status: only block in_production, not planned
...
BEGIN
  ...
  -- Block transition to in_production only when materials incomplete
  IF lower(trim(p_new_status)) = 'in_production' THEN
    ...
    IF v_has_shortage THEN
      RETURN jsonb_build_object(
        'ok', false,
        'error', 'Materials incomplete. Cover material demand (or receive POs) before starting production.',
        'from', v_from,
        'to', p_new_status,
...
DO $$ BEGIN RAISE NOTICE 'Manufacturing flow: WO gen unblocked, planned unblocked, in_production still requires materials'; END $$;
