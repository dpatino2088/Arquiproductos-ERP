-- Allow cuttable-on-cuttable deductors when affects_role is explicit.
--
-- Real-world example: a Bottom Channel that runs the full width sits BETWEEN
-- two vertical Side Channels. The side channels (which are cuttables themselves)
-- physically take up width that the bottom channel must clear.
-- Templates encode this by setting Side Channel.affects_role = 'bottom_channel'.
--
-- The previous logic excluded `placement_section = 'cuttable'` unconditionally
-- from the deductor query, so the explicit affects_role was silently dropped.
-- Bottom Channel ended up with 0mm deduction even when 2 × 66mm side channels
-- were configured to deduct from it.
--
-- Fix: only apply the "exclude cuttables" rule to the section-based fallback
-- (where affects_role is empty). When affects_role explicitly lists this
-- cuttable's role, the deductor participates regardless of its own section.
-- A self-deduction guard (`bc2.id <> v_cuttable.id`) is added for safety.

CREATE OR REPLACE FUNCTION public.compute_cut_breakdown_core(
  p_org_id uuid,
  ...
  p_panel_count integer DEFAULT 1
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
  ...
  LOOP
    ...
    FOR v_deductor IN
      SELECT bc2.id, bc2.component_role, ci2.sku,
               ...
               CASE
                 WHEN COALESCE(bc2.delta_mode, 'subtract') = 'add' THEN
                   -(CASE WHEN v_axis = 'height'
                    ...
                    END)
                 WHEN COALESCE(bc2.delta_mode, 'subtract') = 'info' THEN 0
                 ELSE
                   (CASE WHEN v_axis = 'height'
                        THEN COALESCE(ci2.delta_y_mm, 0)
                        ELSE COALESCE(ci2.delta_x_mm, 0)
                   END)
               END
             ) * (CASE WHEN COALESCE(bc2.cut_delta_scope, 'per_item') = 'per_side' THEN 2 ELSE 1 END) AS self_delta,
             ...
             (
               COALESCE(bc2.qty_type, 'fixed') = 'per_joint'
               OR lower(COALESCE(bc2.component_role, '')) LIKE '%intermediate%'
               OR lower(COALESCE(bc2.component_role, '')) LIKE '%interconnect%'
               OR lower(COALESCE(bc2.component_role, '')) LIKE '%joint%'
             ) AS is_joint_cut_multiplier,
       ...
       WHERE bc2.bom_template_id = p_bom_template_id
         ...
         AND bc2.id <> v_cuttable.id  -- never self-deduct
         AND COALESCE(bc2.delta_mode, 'subtract') IN ('subtract', 'add', 'info')
         AND (
           -- Explicit affects_role match: ALWAYS allowed (incl. cuttable→cuttable
           -- like side_channel → bottom_channel).
           v_role = ANY(string_to_array(COALESCE(bc2.affects_role, ''), ','))
           OR (
             -- Section + axis fallback: only when the deductor's affects_role
             -- is empty AND it lives in a non-cuttable section (drive/passive/shared).
             COALESCE(bc2.affects_role, '') = ''
             AND COALESCE(NULLIF(bc2.placement_section, ''), '') IN ('drive', 'passive', 'shared')
             AND (
               (v_axis = 'width'  AND v_role IN ('tube', 'bottom_bar', 'bottom_channel'))
               OR (v_axis = 'height' AND v_role IN ('side_channel'))
             )
           )
         )
    ...
    LOOP
      IF v_deductor.condition_key IS NOT NULL AND v_deductor.condition_key <> '' THEN
        IF NOT (
          v_deductor.condition_key = 'drive_side'
          AND COALESCE(v_config_snap ->> 'drive_side', '') = 'both'
          AND COALESCE(v_deductor.condition_value, '') IN ('left', 'right')
        ) AND COALESCE(v_config_snap ->> v_deductor.condition_key, '') <> COALESCE(v_deductor.condition_value, '') THEN
          CONTINUE;
        END IF;
      END IF;
      ...
      IF v_deductor.is_joint_cut_multiplier AND v_panel_count <= 1 THEN
        CONTINUE;
      END IF;

      SELECT COALESCE(SUM(
        CASE
          WHEN COALESCE(bc_ch.delta_mode, 'subtract') = 'add' THEN
            -(CASE WHEN v_axis = 'height'
                   THEN COALESCE(ci_ch.delta_y_mm, 0)
                   ELSE COALESCE(ci_ch.delta_x_mm, 0)
              END) * (CASE WHEN COALESCE(bc_ch.cut_delta_scope, 'per_item') = 'per_side' THEN 2 ELSE 1 END) * COALESCE(NULLIF(bc_ch.qty_value, 0), 1)
          WHEN COALESCE(bc_ch.delta_mode, 'subtract') = 'info' THEN 0
          ELSE
            (CASE WHEN v_axis = 'height'
                  THEN COALESCE(ci_ch.delta_y_mm, 0)
                  ELSE COALESCE(ci_ch.delta_x_mm, 0)
      ...
      IF v_combined_d <> 0 OR v_deductor.delta_mode = 'info' THEN
        v_deductions := v_deductions || jsonb_build_object(
          ...
          'total',         CASE
                             WHEN v_deductor.is_joint_cut_multiplier THEN v_combined_d * v_joint_qty
                             ELSE v_combined_d * v_deductor.qty
                           END,
          'mode',          v_deductor.delta_mode,
          'scope',         v_deductor.cut_delta_scope,
          'qty_type',      v_deductor.qty_type,
          ...
          'position',      CASE
                             WHEN v_deductor.placement_section = 'drive' THEN 'drive_side'
                             WHEN v_deductor.placement_section = 'passive' THEN 'passive_side'
                             WHEN v_deductor.placement_section = 'shared' THEN 'edge'
                             WHEN v_deductor.placement_section = 'cuttable' THEN 'center'
                             WHEN v_deductor.is_joint_cut_multiplier THEN 'shared'
    ...
    FOR v_child_ded IN
      SELECT bc3.component_role, ci3.sku,
               ...
               CASE
                 WHEN COALESCE(bc3.delta_mode, 'subtract') = 'add' THEN
                   -(CASE WHEN v_axis = 'height'
                     ...
                     END)
                 WHEN COALESCE(bc3.delta_mode, 'subtract') = 'info' THEN 0
                 ELSE
                   (CASE WHEN v_axis = 'height'
                         THEN COALESCE(ci3.delta_y_mm, 0)
                         ELSE COALESCE(ci3.delta_x_mm, 0)
                    END)
               END
             ) * (CASE WHEN COALESCE(bc3.cut_delta_scope, 'per_item') = 'per_side' THEN 2 ELSE 1 END) AS delta,
             ...
             (
               COALESCE(bc3.qty_type, 'fixed') = 'per_joint'
               OR lower(COALESCE(bc3.component_role, '')) LIKE '%intermediate%'
               OR lower(COALESCE(bc3.component_role, '')) LIKE '%interconnect%'
               OR lower(COALESCE(bc3.component_role, '')) LIKE '%joint%'
             ) AS is_joint_cut_multiplier,
    ...
    LOOP
      IF v_child_ded.condition_key IS NOT NULL AND v_child_ded.condition_key <> '' THEN
        IF NOT (
          v_child_ded.condition_key = 'drive_side'
          AND COALESCE(v_config_snap ->> 'drive_side', '') = 'both'
          AND COALESCE(v_child_ded.condition_value, '') IN ('left', 'right')
        ) AND COALESCE(v_config_snap ->> v_child_ded.condition_key, '') <> COALESCE(v_child_ded.condition_value, '') THEN
          CONTINUE;
        END IF;
      END IF;

      IF v_child_ded.is_joint_cut_multiplier AND v_panel_count <= 1 THEN
        CONTINUE;
      END IF;
      ...
      IF v_child_ded.delta <> 0 OR v_child_ded.delta_mode = 'info' THEN
        ...
        v_deductions := v_deductions || jsonb_build_object(
          ...
          'total',         CASE
                             WHEN v_child_ded.is_joint_cut_multiplier THEN v_child_ded.delta * v_joint_qty
                             ELSE v_child_ded.delta * v_child_ded.qty
                           END,
          'mode',          v_child_ded.delta_mode,
          'scope',         v_child_ded.cut_delta_scope,
          'qty_type',      v_child_ded.qty_type,
          'affects_cut',   (v_child_ded.delta_mode <> 'info'),
          'conditional',   false,
          'intermediate',  v_child_ded.is_joint_cut_multiplier,
          'position',      CASE
                             WHEN v_child_ded.placement_section = 'drive' THEN 'drive_side'
                             WHEN v_child_ded.placement_section = 'passive' THEN 'passive_side'
                             WHEN v_child_ded.placement_section = 'shared' THEN 'edge'
                             WHEN v_child_ded.placement_section = 'cuttable' THEN 'center'
                             WHEN v_child_ded.is_joint_cut_multiplier THEN 'shared'
    ...
    IF v_cuttable.per_panel AND v_panel_count > 1 THEN
      FOR v_p_idx IN 1..v_panel_count LOOP
        ...
        ELSE
          IF v_panels IS NOT NULL AND jsonb_typeof(v_panels) = 'array' AND v_p_idx <= jsonb_array_length(v_panels) THEN
        ...
        LOOP
          IF v_deductor.dinter THEN
            IF v_is_edge_left OR v_is_edge_right THEN
              v_p_deds := v_p_deds || jsonb_build_object(
                ...
                'delta', v_deductor.ddelta / 2, 'qty', 1, 'total', v_deductor.ddelta / 2,
                'note', '½ intermedio'
              );
            ...
            ELSE
              v_p_deds := v_p_deds || jsonb_build_object(
                'role', v_deductor.drole, 'sku', v_deductor.dsku,
                'delta', v_deductor.ddelta / 2, 'qty', 2, 'total', v_deductor.ddelta,
                'note', '½ izq + ½ der'
              );
              v_p_total_ded := v_p_total_ded + v_deductor.ddelta;
            END IF;
          ELSIF v_deductor.dpos = 'drive_side' THEN
            IF (v_drive_side = 'left' AND v_is_edge_left)
               OR (v_drive_side = 'right' AND v_is_edge_right)
               OR (v_drive_side = 'both' AND (v_is_edge_left OR v_is_edge_right)) THEN
              v_p_deds := v_p_deds || jsonb_build_object(
                'role', v_deductor.drole, 'sku', v_deductor.dsku,
                'delta', v_deductor.ddelta, 'qty', 1, 'total', v_deductor.ddelta
              );
              v_p_total_ded := v_p_total_ded + v_deductor.ddelta;
            END IF;
          ELSIF v_deductor.dpos = 'passive_side' THEN
            IF (v_drive_side = 'left' AND v_is_edge_right)
               OR (v_drive_side = 'right' AND v_is_edge_left) THEN
              v_p_deds := v_p_deds || jsonb_build_object(
                'role', v_deductor.drole, 'sku', v_deductor.dsku,
                'delta', v_deductor.ddelta, 'qty', 1, 'total', v_deductor.ddelta
              );
          ...
          ELSIF v_deductor.dpos = 'per_panel' THEN
            v_p_deds := v_p_deds || jsonb_build_object(
              'role', v_deductor.drole, 'sku', v_deductor.dsku,
              'delta', v_deductor.ddelta, 'qty', v_deductor.dqty,
        ...
        v_panel_cuts := v_panel_cuts || jsonb_build_object(
          ...
          'base_mm', v_p_width,
          'cut_mm', v_p_cut,
          'deduction', GREATEST(0, v_p_width - v_p_cut),
...
COMMENT ON FUNCTION public.compute_cut_breakdown_core(uuid, uuid, jsonb, numeric, numeric, integer)
IS 'Core DB cut breakdown calculator. Explicit affects_role works cuttable→cuttable (e.g. side_channel→bottom_channel). Section+axis fallback only when affects_role is empty AND placement_section is drive/passive/shared.';
