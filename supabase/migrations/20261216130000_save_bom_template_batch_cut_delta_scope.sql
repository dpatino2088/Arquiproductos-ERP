-- Fix: save_bom_template_batch was silently dropping cut_delta_scope and the
-- engineering_* metadata columns. The frontend sends them in the payload, but
-- the previous version of the RPC did not list them in the UPDATE / INSERT
-- statements, so the values stayed at their defaults (NULL / 'fixed').
--
-- This caused the Engineering popup's "Per Side (x2)" toggle to revert to
-- "Per Item (x1)" after Save, and the cut breakdown RPC never saw per_side
-- on parents (only on children, where it was already accidentally working
-- through the placement-section save path).
--
-- We patch all four mutation paths (parent UPDATE, parent INSERT,
-- child UPDATE, child INSERT) and the SELECT that returns the refreshed
-- rows so the frontend re-hydrates with the correct values.
...
BEGIN
  ...
  LOOP
    ...
    IF v_comp_id IS NOT NULL THEN
      UPDATE "BOMComponents" SET
        ...
        cut_delta_scope          = COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'cut_delta_scope', '')), ''), 'per_item'),
        delta_mode               = COALESCE(v_comp ->> 'delta_mode', 'subtract'),
        engineering_delta_source = COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_delta_source', '')), ''), 'fixed'),
        engineering_attr_key     = NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_attr_key', '')), ''),
        engineering_scope        = COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_scope', '')), ''), 'total'),
        engineering_source_role  = NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_source_role', '')), ''),
    ...
    ELSE
      INSERT INTO "BOMComponents" (
        ...
        cut_axis, cut_delta_mm, cut_delta_scope, delta_mode,
        engineering_delta_source, engineering_attr_key,
        engineering_scope, engineering_source_role,
        sort_order,
        ...
        COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'cut_delta_scope', '')), ''), 'per_item'),
        COALESCE(v_comp ->> 'delta_mode', 'subtract'),
        COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_delta_source', '')), ''), 'fixed'),
        NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_attr_key', '')), ''),
        COALESCE(NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_scope', '')), ''), 'total'),
        NULLIF(TRIM(COALESCE(v_comp ->> 'engineering_source_role', '')), ''),
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'cut_delta_scope', bc.cut_delta_scope,
      'delta_mode', bc.delta_mode,
      'engineering_delta_source', bc.engineering_delta_source,
      'engineering_attr_key', bc.engineering_attr_key,
      'engineering_scope', bc.engineering_scope,
      'engineering_source_role', bc.engineering_source_role,
      ...
      'placement_section', bc.placement_section,
...
COMMENT ON FUNCTION public.save_bom_template_batch(uuid, jsonb, jsonb, uuid[]) IS
  'Persists a BOM template + components in one shot. Restored cut_delta_scope and engineering_* fields that were silently dropped before.';
