-- Fix build_bom_preview_snapshot: pass style_code from config_snapshot to
-- compute_fabric_pricing_from_rule instead of NULL.
-- Without this, drapery consumption ignores fullness factor.
...
BEGIN
  ...
  -- Extract style_code from config_snapshot
  ...
  v_width_mm := COALESCE(
    NULLIF((v_config->'measurements'->>'width_total_mm')::numeric, 0),
    NULLIF(v_cp.width_mm, 0),
    (v_config->>'width_mm')::numeric,
    0
  );
  ...
  IF v_cp.roll_catalog_item_id IS NOT NULL THEN
    ...
    -- FabricRule-based consumption with style_code + heatseal/wrap flags
    BEGIN
      SELECT * INTO v_consumption
      FROM public.compute_fabric_pricing_from_rule(
        p_org_id, v_cp.product_type_id, v_style_code,
    ...
    IF v_consumption.qty IS NOT NULL AND v_consumption.qty > 0 THEN
      ...
      v_fabric_calc := jsonb_build_object(
        ...
        'fabric_width_source', COALESCE(
          (SELECT fr.fabric_width_source FROM "FabricRules" fr
           WHERE fr.organization_id = p_org_id AND fr.product_type_id = v_cp.product_type_id
             AND (fr.style_code = v_style_code OR (v_style_code IS NULL AND fr.style_code IS NULL))
        ...
        'openness_factor_pct', (SELECT rs2.openness_factor_pct FROM "CatalogItemRollSpecs" rs2 WHERE rs2.catalog_item_id = v_cp.roll_catalog_item_id AND rs2.organization_id = p_org_id LIMIT 1)
      );
    ...
    ELSE
      ...
      v_fabric_calc := jsonb_build_object('source', 'legacy', 'style_code', v_style_code);
    END IF;
...
'BOM preview JSONB with consumption engine. Uses FabricRule for real consumption with style_code from config_snapshot. Totals UNIT.';
