...
BEGIN
  ...
  IF v_cp.id IS NULL THEN
    RAISE EXCEPTION 'ConfiguredProduct not found';
  END IF;

  -- 🔒 ENFORCE DEALER POLICY (COMMIT)
  ...
  RETURNING id INTO quote_line_id;

  bom_instance_id := NULL;

  RETURN QUERY SELECT quote_line_id, bom_instance_id;
END;
$$;
...
BEGIN
  ...
  IF v_bom_template_id IS NULL THEN
    v_bom_template_id :=
      public.select_best_bom_template_v2_strict(
        p_org_id,
        p_product_type_id,
        p_config_snapshot
      );
  ...
  IF v_bom_template_id IS NULL THEN
    RAISE EXCEPTION 'No matching BOMTemplate found';
  END IF;

  -- 🔒 ENFORCE DEALER POLICY (PREVIEW)
  ...
  v_preview_snapshot :=
    public.build_bom_preview_snapshot(
      p_org_id,
