...
CREATE OR REPLACE FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_options" "jsonb" DEFAULT '{}'::"jsonb") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
declare
  v_ql public."QuoteLines"%rowtype;

  v_roll_cost numeric := 0;
  v_bom_cost  numeric := 0;
  v_total_cost numeric := 0;

  v_new_pricing_version integer := 0;
begin
  -- 1) Cargar QuoteLine
  select *
  into v_ql
  ...
  if not found then
    return jsonb_build_object(
      'ok', false,
      'error', 'QUOTE_LINE_NOT_FOUND',
      'quote_line_id', p_quote_line_id
    );
  end if;

  -- 2) Roll cost (desde QuoteLines)
  -- Preferimos sumar componentes para evitar depender de total_cost previo.
  v_roll_cost :=
      coalesce(v_ql.material_cost, 0)
    + coalesce(v_ql.labor_cost, 0)
    + coalesce(v_ql.shipping_cost, 0)
    + coalesce(v_ql.import_tax_cost, 0);

  -- fallback si por alguna razón esos costos no estaban poblados
  if v_roll_cost = 0 then
    v_roll_cost := coalesce(v_ql.total_cost, 0);
  end if;

  -- 3) BOM cost (sumar BOMInstanceLines)
  select
    coalesce(
      sum(
        coalesce(bil.total_cost_exw, (coalesce(bil.unit_cost_exw,0) * coalesce(bil.qty,0)), 0)
      ),
      0
    )
  into v_bom_cost
  ...
  where bi.quote_line_id = p_quote_line_id
    and coalesce(bi.deleted, false) = false
    and coalesce(bil.deleted, false) = false;

  v_total_cost := coalesce(v_roll_cost,0) + coalesce(v_bom_cost,0);

  -- 4) pricing_version ++ (si era null => 1)
  v_new_pricing_version := coalesce(v_ql.pricing_version, 0) + 1;

  -- 5) Persistir snapshots en QuoteLines
  update public."QuoteLines"
  set
    roll_cost_snapshot = v_roll_cost,
    bom_cost_snapshot  = v_bom_cost,
    total_cost         = v_total_cost,
    pricing_version    = v_new_pricing_version,
    last_priced_at     = now(),
    updated_at         = now()
  where id = p_quote_line_id;
...
begin
  if coalesce(new.is_roll,false) = true then
    select c.cost_exw_per_m, c.cost_exw_per_m2
      into v_per_m, v_per_m2
    from public.compute_roll_conversions(new.cost_exw, new.unit_of_measure, new.roll_width) c;
    ...
    delete from public."CatalogItemConversions"
...
CREATE OR REPLACE TRIGGER "catalogitems_write_conversions" AFTER INSERT OR UPDATE OF "cost_exw", "unit_of_measure", "roll_width", "is_roll" ON "public"."CatalogItems" FOR EACH ROW EXECUTE FUNCTION "public"."trg_catalogitems_write_conversions"();
