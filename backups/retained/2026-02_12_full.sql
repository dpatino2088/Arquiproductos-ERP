...
declare
  v_pt_code text;
  v_allowed text[];
  v_allow_oneoff boolean;
  v_roll_source text;
begin
  -- si no hay dealer => no hay policy que aplicar
  ...
  select
    coalesce(p.allowed_product_type_codes, '{}'::text[]),
    coalesce(p.allow_variants_oneoff, false)
  into
    v_allowed,
    v_allow_oneoff
  ...
  -- sin fila => sin restricciones
  if not found then
    return;
  end if;

  -- fail closed si existe policy pero no tiene product types
  ...
  -- OneOff: esperamos roll_source='oneoff' en config_snapshot (UI)
  v_roll_source := lower(coalesce(p_config_snapshot->>'roll_source', 'catalog'));
  if v_roll_source = 'oneoff' and v_allow_oneoff is not true then
    raise exception 'OneOff variants are not allowed for this dealer'
      using errcode = '42501';
  end if;
end;
$$;
...
COMMENT ON FUNCTION "public"."calculate_configured_product_totals"("p_configured_product_id" "uuid") IS 'Recalculates ConfiguredProduct totals. Roll from catalog (roll_catalog_item_id) only.';
...
begin
  ...
  -- 🚫 OneOff disabled (hard block)
  perform public.reject_oneoff_keys(v_cp.config_snapshot);
  ...
  if v_snapshot->>'version' = '1' and jsonb_array_length(v_snapshot->'items') > 0 then
    ...
    select coalesce(sum(
      ...
      coalesce((select sum((c->>'line_total')::numeric)
               from jsonb_array_elements(coalesce(item->'children', '[]'::jsonb)) c), 0)
    ), 0) into v_bom_total
    from jsonb_array_elements(v_snapshot->'items') as item
  ...
  values (
    ...
    v_unit_msrp, v_total_msrp, v_roll_total_cost + v_bom_total_cost + v_labor_amount,
    true, now(), 1
  )
...
begin
  ...
  -- (si aún quieres enforcement DealerConfiguratorPolicies, déjalo aquí; si no, bórralo)
...
COMMENT ON FUNCTION "public"."create_configured_product_and_bom_preview"("p_org_id" "uuid", "p_product_type_id" "uuid", "p_config_snapshot" "jsonb", "p_quote_id" "uuid", "p_quote_line_id" "uuid") IS 'Creates ConfiguredProduct with pricing. Roll from catalog variant only (roll_catalog_item_id).';
...
COMMENT ON FUNCTION "public"."freeze_proposal_snapshot"("p_proposal_id" "uuid") IS 'Captures QuoteLine + ConfiguredProduct snapshot into ProposalLines.quote_line_snapshot. Idempotent: only updates lines where quote_line_snapshot is null.';
...
COMMENT ON FUNCTION "public"."is_org_user_member"("p_org_id" "uuid") IS 'Returns true if current user is an active/invited member of the organization via OrganizationUsers OR DealerUsers (portal).';
...
BEGIN
  ...
  -- 1) Si vienen ambos NULL, intenta auth.uid()
  IF NEW.created_by_user_id IS NULL AND NEW.created_by_portal_user_id IS NULL THEN
    ...
    -- fallback: copiar desde Quote
    IF NEW.quote_id IS NOT NULL THEN
      SELECT q.created_by_user_id, q.created_by_portal_user_id
      INTO v_quote
      FROM public."Quotes" q
      WHERE q.id = NEW.quote_id;

      IF (v_quote.created_by_user_id IS NOT NULL AND v_quote.created_by_portal_user_id IS NULL)
      OR (v_quote.created_by_user_id IS NULL AND v_quote.created_by_portal_user_id IS NOT NULL) THEN
        NEW.created_by_user_id := v_quote.created_by_user_id;
        NEW.created_by_portal_user_id := v_quote.created_by_portal_user_id;
        RETURN NEW;
      END IF;
    END IF;

    -- si aún no se puede determinar, deja que falle con un error claro
    RAISE EXCEPTION 'Proposal must have exactly one creator. Both are NULL and auth.uid() is NULL. proposal_id=%, quote_id=%',
      COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid), NEW.quote_id;
  END IF;

  -- 2) Si vienen ambos con valor, resolver a EXACTAMENTE uno
  IF NEW.created_by_user_id IS NOT NULL AND NEW.created_by_portal_user_id IS NOT NULL THEN
    -- Si hay quote, preferir el creador del quote si es válido
    IF NEW.quote_id IS NOT NULL THEN
      SELECT q.created_by_user_id, q.created_by_portal_user_id
      INTO v_quote
      FROM public."Quotes" q
    ...
    -- si no, prioriza internal y nulifica portal
    NEW.created_by_portal_user_id := NULL;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Cargar Quote si existe
  IF NEW.quote_id IS NOT NULL THEN
    SELECT
      q.created_by_user_id,
      q.created_by_portal_user_id,
      q.dealer_id
    INTO v_quote
    FROM public."Quotes" q
    WHERE q.id = NEW.quote_id;
  ...
  IF NEW.dealer_id IS NULL THEN
    ...
    ELSE
      RAISE EXCEPTION
        'Proposal requires dealer_id. Not provided and not found on Quote. proposal_id=%, quote_id=%',
  ...
  ------------------------------------------------------------------
  -- 2) Asegurar created_by (exactly one)
  ------------------------------------------------------------------
  IF NEW.created_by_user_id IS NULL AND NEW.created_by_portal_user_id IS NULL THEN
    IF v_uid IS NOT NULL THEN
      NEW.created_by_user_id := v_uid;
    ELSIF v_quote.created_by_user_id IS NOT NULL
       OR v_quote.created_by_portal_user_id IS NOT NULL THEN
      NEW.created_by_user_id := v_quote.created_by_user_id;
      NEW.created_by_portal_user_id := v_quote.created_by_portal_user_id;
    ELSE
      RAISE EXCEPTION
        'Proposal must have exactly one creator. None found. proposal_id=%, quote_id=%',
        COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid),
        NEW.quote_id;
    END IF;
  END IF;
  ...
  IF NEW.created_by_user_id IS NOT NULL
     AND NEW.created_by_portal_user_id IS NOT NULL THEN
    -- prioriza internal
    NEW.created_by_portal_user_id := NULL;
  END IF;
...
BEGIN
  ...
  -- Subtotal = material + addons + global_fee (fee applies to MSRPs, not as separate line)
  SELECT COALESCE(p.global_discount_pct, 0), COALESCE(p.global_fee_amount, 0)
  INTO v_discount_pct, v_fee
  FROM public."Proposals" p
  WHERE p.id = p_proposal_id;

  v_subtotal := v_subtotal + v_addons_total + COALESCE(v_fee, 0);
...
COMMENT ON FUNCTION "public"."recalc_proposal_totals"("p_proposal_id" "uuid") IS 'Recalculates Proposals totals. Uses line_adjustment_pct (not override_mode). Prefers quote_line_snapshot when sent/accepted.';
...
begin
  ...
  -- Detect any key starting with oneoff_
  select exists (
    select 1
    from jsonb_object_keys(p_config) k
    where k like 'oneoff\_%' escape '\'
  ) into v_has_oneoff;
  ...
  -- Also detect nested common place: measurements / variants payload (optional safety)
  -- If you never nested oneoff_* deeper, you can remove this block.
  select exists (
    select 1
    from jsonb_path_query(
      coalesce(p_config,'{}'::jsonb),
      '$.** ? (@key like_regex "^oneoff_.*")'
    )
  ) into v_has_oneoff;

  if v_has_oneoff then
    raise exception 'OneOff is disabled. Remove oneoff_* fields from config_snapshot (nested).';
  end if;
end;
$_$;
...
COMMENT ON FUNCTION "public"."reject_oneoff_keys"("p_config" "jsonb") IS 'Raises exception if config jsonb contains any oneoff_* keys (OneOff disabled).';
...
COMMENT ON FUNCTION "public"."set_quote_line_msrp_from_value"("p_quote_line_id" "uuid", "p_total_msrp" numeric) IS 'Sets QuoteLine msrp and unit_msrp from provided total (e.g. from configurator). Use as fallback when sync_quote_line_pricing_from_configured_product leaves $0.';
...
CREATE OR REPLACE FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) IS 'Upserts one row in DealerConfiguratorPolicies per (org, dealer). Used by Settings → Dealer Profile → Configurator Permissions.';
...
CREATE UNIQUE INDEX "uq_proposals_org_proposal_no" ON "public"."Proposals" USING "btree" ("organization_id", "proposal_no") WHERE (("deleted" = false) AND ("proposal_no" IS NOT NULL) AND ("proposal_no" <> ''::"text"));
...
CREATE POLICY "dealers_select_own_org" ON "public"."Dealers" FOR SELECT USING (("public"."is_org_user_member"("organization_id") AND (("deleted" IS NULL) OR ("deleted" = false))));



COMMENT ON POLICY "dealers_select_own_org" ON "public"."Dealers" IS 'Allow org members (incl. dealer_manager, dealer_member) and DealerUsers to read dealers; needed for logo_url on Proposals.';
...
CREATE POLICY "dealerusers_select_bomtemplates" ON "public"."BOMTemplates" FOR SELECT USING ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "dealerusers_select_configured_products" ON "public"."ConfiguredProducts" FOR SELECT USING ("public"."is_org_user_member"("organization_id"));
...
CREATE POLICY "dircontacts_insert" ON "public"."DirectoryContacts" FOR INSERT TO "authenticated" WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "dircontacts_select" ON "public"."DirectoryContacts" FOR SELECT TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id"))))));



CREATE POLICY "dircontacts_update" ON "public"."DirectoryContacts" FOR UPDATE TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id")))))) WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "dircustomers_insert" ON "public"."DirectoryCustomers" FOR INSERT TO "authenticated" WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "dircustomers_select" ON "public"."DirectoryCustomers" FOR SELECT TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id"))))));



CREATE POLICY "dircustomers_update" ON "public"."DirectoryCustomers" FOR UPDATE TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id")))))) WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));
...
CREATE POLICY "inv_balances_insert_org" ON "public"."InventoryBalances" FOR INSERT TO "authenticated" WITH CHECK ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "inv_balances_select_org" ON "public"."InventoryBalances" FOR SELECT TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "inv_balances_update_org" ON "public"."InventoryBalances" FOR UPDATE TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "inv_profiles_insert_org" ON "public"."InventoryItemProfiles" FOR INSERT TO "authenticated" WITH CHECK ("public"."is_org_user_member"(( SELECT "w"."organization_id"
   FROM "public"."Warehouses" "w"
  WHERE ("w"."id" = "InventoryItemProfiles"."warehouse_id"))));



CREATE POLICY "inv_profiles_select_org" ON "public"."InventoryItemProfiles" FOR SELECT TO "authenticated" USING ("public"."is_org_user_member"(( SELECT "w"."organization_id"
   FROM "public"."Warehouses" "w"
  WHERE ("w"."id" = "InventoryItemProfiles"."warehouse_id"))));



CREATE POLICY "inv_profiles_update_org" ON "public"."InventoryItemProfiles" FOR UPDATE TO "authenticated" USING ("public"."is_org_user_member"(( SELECT "w"."organization_id"
   FROM "public"."Warehouses" "w"
  WHERE ("w"."id" = "InventoryItemProfiles"."warehouse_id"))));
...
CREATE POLICY "po_lines_insert_via_po" ON "public"."PurchaseOrderLines" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."PurchaseOrders" "po"
  WHERE (("po"."id" = "PurchaseOrderLines"."purchase_order_id") AND "public"."is_org_user_member"("po"."organization_id")))));



CREATE POLICY "po_lines_select_via_po" ON "public"."PurchaseOrderLines" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."PurchaseOrders" "po"
  WHERE (("po"."id" = "PurchaseOrderLines"."purchase_order_id") AND "public"."is_org_user_member"("po"."organization_id")))));



CREATE POLICY "po_lines_update_via_po" ON "public"."PurchaseOrderLines" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."PurchaseOrders" "po"
  WHERE (("po"."id" = "PurchaseOrderLines"."purchase_order_id") AND "public"."is_org_user_member"("po"."organization_id")))));



CREATE POLICY "proposal_line_addons_delete" ON "public"."ProposalLineAddOns" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLineAddOns"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));



CREATE POLICY "proposal_line_addons_insert" ON "public"."ProposalLineAddOns" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLineAddOns"."proposal_id") AND ("p"."deleted" = false) AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));



CREATE POLICY "proposal_line_addons_select" ON "public"."ProposalLineAddOns" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
...
CREATE POLICY "proposal_line_addons_update" ON "public"."ProposalLineAddOns" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLineAddOns"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id"))))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLineAddOns"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));
...
CREATE POLICY "proposallines_delete" ON "public"."ProposalLines" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLines"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));



CREATE POLICY "proposallines_insert" ON "public"."ProposalLines" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLines"."proposal_id") AND ("p"."deleted" = false) AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));



CREATE POLICY "proposallines_select" ON "public"."ProposalLines" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLines"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user"("p"."dealer_id")))))));



CREATE POLICY "proposallines_update" ON "public"."ProposalLines" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLines"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id"))))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."Proposals" "p"
  WHERE (("p"."id" = "ProposalLines"."proposal_id") AND ((("p"."organization_id" IS NOT NULL) AND "public"."is_org_member"("p"."organization_id")) OR (("p"."dealer_id" IS NOT NULL) AND "public"."is_dealer_portal_user_with_write"("p"."dealer_id")))))));



CREATE POLICY "proposals_insert" ON "public"."Proposals" FOR INSERT TO "authenticated" WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "proposals_select" ON "public"."Proposals" FOR SELECT TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id"))))));



CREATE POLICY "proposals_update" ON "public"."Proposals" FOR UPDATE TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id")))))) WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "purchase_orders_insert_org" ON "public"."PurchaseOrders" FOR INSERT TO "authenticated" WITH CHECK ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "purchase_orders_select_org" ON "public"."PurchaseOrders" FOR SELECT TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "purchase_orders_update_org" ON "public"."PurchaseOrders" FOR UPDATE TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));
...
CREATE POLICY "quotes_insert" ON "public"."Quotes" FOR INSERT TO "authenticated" WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));



CREATE POLICY "quotes_select" ON "public"."Quotes" FOR SELECT TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id"))))));



CREATE POLICY "quotes_update" ON "public"."Quotes" FOR UPDATE TO "authenticated" USING (((("deleted" = false) OR ("deleted" IS NULL)) AND ("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND ("dealer_id" = "public"."current_dealer_id"("organization_id")))))) WITH CHECK ((("organization_id" IS NOT NULL) AND ("public"."is_org_user_member"("organization_id") OR (("public"."current_dealer_id"("organization_id") IS NOT NULL) AND (("dealer_id" IS NULL) OR ("dealer_id" = "public"."current_dealer_id"("organization_id")))))));
...
CREATE POLICY "salesorders_org_insert" ON "public"."SalesOrders" FOR INSERT WITH CHECK ((("organization_id" IS NOT NULL) AND "public"."is_internal_org_user"("organization_id") AND ("dealer_id" IS NULL)));



CREATE POLICY "salesorders_org_select" ON "public"."SalesOrders" FOR SELECT USING ((("organization_id" IS NOT NULL) AND "public"."is_internal_org_user"("organization_id") AND ("dealer_id" IS NULL)));



CREATE POLICY "salesorders_org_update" ON "public"."SalesOrders" FOR UPDATE USING ((("organization_id" IS NOT NULL) AND "public"."is_internal_org_user"("organization_id") AND ("dealer_id" IS NULL))) WITH CHECK ((("organization_id" IS NOT NULL) AND "public"."is_internal_org_user"("organization_id") AND ("dealer_id" IS NULL)));
...
CREATE POLICY "warehouses_insert_org" ON "public"."Warehouses" FOR INSERT TO "authenticated" WITH CHECK ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "warehouses_select_org" ON "public"."Warehouses" FOR SELECT TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));



CREATE POLICY "warehouses_update_org" ON "public"."Warehouses" FOR UPDATE TO "authenticated" USING ("public"."is_org_user_member"("organization_id"));
...
GRANT ALL ON FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."upsert_dealer_configurator_policy"("p_org_id" "uuid", "p_dealer_id" "uuid", "p_allowed_product_type_codes" "text"[], "p_allow_variants_catalog" boolean, "p_allow_variants_oneoff" boolean, "p_allow_accessories_only" boolean, "p_allow_hardware" boolean, "p_allow_operating_system" boolean) TO "service_role";
