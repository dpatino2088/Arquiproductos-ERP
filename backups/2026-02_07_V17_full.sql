...
BEGIN
  ...
  -- 🔒 ENFORCE DEALER CONFIGURATOR POLICY (COMMIT)
  SELECT COALESCE(
           p_dealer_id,
  ...
  IF v_dealer_id IS NOT NULL THEN
    PERFORM public.assert_dealer_configurator_policy(
      ...
      COALESCE(v_cp.config_snapshot, '{}'::jsonb)
    );
  END IF;
  ...
  VALUES (
    ...
    v_cp.roll_catalog_item_id IS NOT NULL, CASE WHEN v_cp.roll_catalog_item_id IS NOT NULL THEN 'fabric' ELSE NULL END,
    ...
    v_roll_msrp_total, v_bom_total, v_roll_total_cost, v_bom_total_cost, v_unit_msrp, v_total_msrp,
    v_roll_total_cost + v_bom_total_cost + v_labor_amount,
    true, now(), 1
  )
...
BEGIN
  ...
  -- 🔒 ENFORCE DEALER CONFIGURATOR POLICY (PREVIEW)
  -- Si NO hay dealer (org user / preview sin quote) => NO bloquea.
  SELECT q.dealer_id
    INTO v_dealer_id
...
CREATE TABLE IF NOT EXISTS "public"."DealerConfiguratorPolicies" (
    ...
    "allow_variants_oneoff" boolean DEFAULT false NOT NULL,
