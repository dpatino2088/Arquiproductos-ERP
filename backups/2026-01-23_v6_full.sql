...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    -- Sin org no podemos resolver settings
    RETURN;
  END IF;
  ...
  -- Cost model (simple y estable):
  -- shipping = material * shipping_pct
  v_shipping_cost := round(v_material_cost * COALESCE(v_shipping_pct, 0), 6);

  -- import tax = (material + shipping) * import_tax_pct
  ...
  -- MSRP sale-in / sale-out (margin on sale):
...
declare
  ...
  v_min_margin_pct numeric := 0.35;      -- fallback
  v_msrp_pct_sale_out numeric := 0.65;   -- fallback
...
begin
  -- Base cost from CatalogItems (source of truth)
  ...
  -- (Si tu tabla CategoryMargins usa otros nombres, ajusta aquí)
...
CREATE OR REPLACE FUNCTION "public"."sync_catalogitems_cost_to_msrp"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- 1) intenta UPDATE (caso normal: ya existe fila MSRP)
  UPDATE public."CatalogItemsMSRP" m
  SET
    organization_id = NEW.organization_id,
    category_id     = NEW.category_id,
    cost_exw        = COALESCE(NEW.cost_exw, 0),
    updated_at      = now()
  WHERE m.catalog_item_id = NEW.id;

  -- 2) si no existía fila MSRP, crea una mínima pero válida
  IF NOT FOUND THEN
    INSERT INTO public."CatalogItemsMSRP" (
      catalog_item_id,
      organization_id,
...
ALTER FUNCTION "public"."sync_catalogitems_cost_to_msrp"() OWNER TO "postgres";
...
BEGIN
  ...
  IF v_org IS NULL OR v_cat IS NULL THEN
    -- intentar resolver desde CatalogItems si viene vacío
  ...
  -- mantener material_cost alineado a cost_exw si tu regla es esa:
  NEW.material_cost := COALESCE(NEW.cost_exw, NEW.material_cost, 0);
...
CREATE OR REPLACE TRIGGER "trg_sync_catalogitems_cost_to_msrp" AFTER INSERT OR UPDATE OF "cost_exw", "category_id", "organization_id" ON "public"."CatalogItems" FOR EACH ROW EXECUTE FUNCTION "public"."sync_catalogitems_cost_to_msrp"();



CREATE OR REPLACE TRIGGER "trg_sync_catalogitems_to_msrp" AFTER INSERT OR UPDATE OF "cost_exw", "sku", "name", "collection_name", "variant_name", "unit_of_measure", "category_id" ON "public"."CatalogItems" FOR EACH ROW EXECUTE FUNCTION "public"."sync_catalogitems_to_msrp_safe"();
...
GRANT ALL ON FUNCTION "public"."sync_catalogitems_cost_to_msrp"() TO "anon";
GRANT ALL ON FUNCTION "public"."sync_catalogitems_cost_to_msrp"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sync_catalogitems_cost_to_msrp"() TO "service_role";
