...
export default function SOAttachmentsTab({ salesOrderId, organizationId, canEdit = true }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, sales_order_id, organization_id, file_name, file_path, file_size, content_type, uploaded_by, created_at')
      ...
      if (error) {
        ...
        setAttachments((data ?? []) as SOAttachment[]);
      }
      setLoading(false);
    })();
  }, [salesOrderId]);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files?.length || !salesOrderId || !user) return;
    setUploading(true);
    const added: SOAttachment[] = [];
    try {
      for (const file of Array.from(files)) {
        const ext = file.name.split('.').pop() || '';
        const uniqueName = `${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${ext}`;
        const filePath = `${organizationId}/so/${salesOrderId}/${uniqueName}`;
        ...
        added.push(row as SOAttachment);
      }
  ...
  const handleDelete = async (att: SOAttachment) => {
    if (!canEdit) return;
    try {
      await supabase.storage.from(BUCKET).remove([att.file_path]);
      const { error } = await supabase.from('sales_order_attachments').delete().eq('id', att.id);
