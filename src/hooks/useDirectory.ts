    ...
    async function fetchContacts() {
      ...
      try {
        ...
        const { data: contactsData, error: contactsError } = await supabase
          ...
          .select('id, organization_id, customer_id, contact_name, contact_email, contact_phone, deleted, created_at, updated_at')
          ...
          if (customersError) {
            ...
            customersMap = new Map(
              customersData.map((c: any) => [
                c.id, 
                (c.customer_name || '').toString().trim()  // Preferir explícita, fallback a genérica
              ])
            );
          }
        }
        ...
        const transformedContacts = (contactsData || []).map((contact: any) => {
          ...
          const contactName = (contact.contact_name || '').toString().trim();
          const contactEmail = (contact.contact_email || '').toString().trim();
          const contactPhone = (contact.contact_phone || '').toString().trim();
    ...
    async function fetchCustomers() {
      ...
      try {
          ...
          try {
            ...
            if (contactsError) {
              ...
              contactsMap = contactsData.reduce((acc: Record<string, string>, contact: any) => {
                acc[contact.id] = (contact.contact_name || '').toString().trim();
                return acc;
              }, {});
            }
          } catch (err) {
        ...
        const transformedCustomers = data.map((customer: any) => {
          const customerName = (customer.customer_name || '').toString().trim();
