...
const getExpenseTypeIcon = (expenseType: string) => {
  switch (expenseType) {
    ...
    case 'Meals':
      return <Utensils className="w-4 h-4 text-status-green" />;
    case 'Transportation':
      return <Car className="w-4 h-4 text-status-purple" />;
    case 'Office Supplies':
      return <Package className="w-4 h-4 text-status-orange" />;
    case 'Training':
      return <UserPlus className="w-4 h-4 text-status-blue" />;
    case 'Software':
      return <Settings className="w-4 h-4 text-status-blue" />;
    case 'Hardware':
      return <Laptop className="w-4 h-4 text-status-blue" />;
    case 'Other':
      return <Receipt className="w-4 h-4 text-status-blue" />;
    default:
      return <Receipt className="w-4 h-4 text-status-blue" />;
  }
};
...
const getExpenseTypeBadge = (expenseType: string) => {
  const typeConfig = {
    'Travel': { bg: 'bg-blue-50', text: 'text-status-blue' },
    'Meals': { bg: 'bg-green-50', text: 'text-status-green' },
    'Transportation': { bg: 'bg-purple-50', text: 'text-status-purple' },
    'Office Supplies': { bg: 'bg-orange-50', text: 'text-status-orange' },
...
export default function TeamExpenses() {
  ...
  const [expenseTypeSearchTerm, setExpenseTypeSearchTerm] = useState('');
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter(expense => {
      ...
      const matchesExpenseType = selectedExpenseType.length === 0 || selectedExpenseType.includes(expense.expenseType);
      const matchesStatus = selectedStatus.length === 0 || selectedStatus.includes(expense.status);
      const matchesDepartment = selectedDepartment.length === 0 || selectedDepartment.includes(expense.employeeDepartment);
  ...
  const getFilteredExpenseTypeOptions = () => {
    const expenseTypes = [...new Set(expenses.map(expense => expense.expenseType))];
    if (!expenseTypeSearchTerm) return expenseTypes;
    return expenseTypes.filter(type => 
      type.toLowerCase().includes(expenseTypeSearchTerm.toLowerCase())
    );
  };
              ...
              <div className="relative dropdown-container">
                <div className="px-3 py-1 border border-gray-200 rounded text-sm bg-white min-h-[32px] flex items-center justify-between cursor-pointer hover:bg-gray-50" 
                  ...
                  <span className="text-gray-700">
                    {selectedExpenseType.length === 0 ? 'All Expense Types' : 
                     selectedExpenseType.length === 1 ? selectedExpenseType[0] :
                     `${selectedExpenseType.length} selected`}
                  </span>
                    ...
                    <div className="p-2 border-b border-gray-100">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          value={expenseTypeSearchTerm}
                          onChange={(e) => setExpenseTypeSearchTerm(e.target.value)}
                        ...
                        {selectedExpenseType.length > 0 && (
                          ...
                          >
                            Clear ({selectedExpenseType.length})
                          </button>
                        )}
                      </div>
                    </div>
                    {getFilteredExpenseTypeOptions().map((expenseType) => (
                      <div key={expenseType} className="px-3 py-2 hover:bg-gray-50 cursor-pointer flex items-center gap-2"
                           onClick={() => handleExpenseTypeToggle(expenseType)}>
                        <input type="checkbox" checked={selectedExpenseType.includes(expenseType)} readOnly className="w-4 h-4" />
                        <span className="text-sm text-gray-700">{expenseType}</span>
                      </div>
                    ))}
                    {getFilteredExpenseTypeOptions().length === 0 && (
