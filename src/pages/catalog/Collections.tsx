...
import { useCatalogCollectionsCRUD } from '../../hooks/useCatalog';
...
export default function Collections() {
  ...
  const { collections, loading, error, createCollection, updateCollection, deleteCollection, isCreating, isDeleting } = useCatalogCollectionsCRUD();
  ...
  const [sortBy, setSortBy] = useState<'name' | 'code' | 'sort_order'>('sort_order');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc');
  const [showNewModal, setShowNewModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ name: '', code: '', description: '', active: true, sort_order: 0 });
  ...
  }, [collections, searchTerm, sortBy, sortOrder]);
  ...
  const handleNew = () => {
    setFormData({ name: '', code: '', description: '', active: true, sort_order: 0 });
    setEditingId(null);
  ...
  return (
    <div className="py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-title font-semibold text-foreground mb-1">Collections</h1>
          <p className="text-small text-muted-foreground">Manage product collections</p>
        </div>
        <button
          onClick={handleNew}
