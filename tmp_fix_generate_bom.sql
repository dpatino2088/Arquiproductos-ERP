-- Fix generate_bom_for_manufacturing_order function
-- Fixes: SalesOrderLines → SaleOrderLines, remove archived column refs,
-- ON CONFLICT → NOT EXISTS, CostSettings column names
...
BEGIN
    ...
    IF v_mo.sales_order_id IS NULL THEN
        v_warnings := v_warnings || 'MO has no sales_order_id; cannot auto-create MO lines from SaleOrderLines.';
    END IF;
    ...
    IF v_mo_lines_before = 0 AND v_mo.sales_order_id IS NOT NULL THEN
        ...
        WHERE sol.sales_order_id = v_mo.sales_order_id AND COALESCE(sol.deleted, false) = false
          AND NOT EXISTS (
            ...
            WHERE mol2.manufacturing_order_id = p_manufacturing_order_id
              AND mol2.sales_order_line_id = sol.id
          );
    ...
    FROM "CostSettings"
    WHERE organization_id = v_mo.organization_id AND is_active = true
    LIMIT 1;

    IF FOUND THEN
        v_shipping_percentage := COALESCE(v_cost_settings.shipping_pct, 0);
        v_import_tax_percentage := COALESCE(v_cost_settings.import_tax_pct, 0);
        v_min_margin_pct := COALESCE(v_cost_settings.minimum_margin_pct, 35.0);
        v_labor_percentage := COALESCE(v_cost_settings.labor_pct, 0);
        v_max_discount_pct := COALESCE(v_cost_settings.default_msrp_pct, 65.0);
    END IF;
    ...
    LOOP
        ...
        SELECT sol.id, sol.product_type, sol.collection_name, sol.variant_name,
               sol.width_m, sol.height_m, sol.area,
               ql.drive_type, ql.cassette, NULL::text AS cassette_type,
               ql.side_channel, NULL::text AS side_channel_type,
               sol.hardware_color, NULL::text AS bottom_rail_type,
               sol.quote_line_id
        ...
        IF NOT FOUND THEN
            v_warnings := v_warnings || format('SaleOrderLine %s not found', v_mo_line.sales_order_line_id);
            CONTINUE;
        END IF;
        IF v_sales_order_line.product_type IS NULL OR TRIM(v_sales_order_line.product_type) = '' THEN
            v_warnings := v_warnings || format('SaleOrderLine %s has NULL or empty product_type', v_sales_order_line.id);
            CONTINUE;
        END IF;
        ...
        WHERE pt.code = v_sales_order_line.product_type AND bt.active = true AND bt.deleted = false
        ORDER BY bt.created_at DESC LIMIT 1;

        IF NOT FOUND THEN
            v_warnings := v_warnings || format('No active BOMTemplate found for product_type: %s (SaleOrderLine: %s)', v_sales_order_line.product_type, v_sales_order_line.id);
            CONTINUE;
        END IF;
        ...
        IF v_bom_instance_id IS NULL THEN
            v_warnings := v_warnings || format('Failed to create BOMInstance for SaleOrderLine %s', v_sales_order_line.id);
            CONTINUE;
        END IF;
        v_total_bom_instances := v_total_bom_instances + 1;

        FOR v_bom_component IN
            SELECT bc.id, bc.component_role, bc.component_item_id, bc.qty_type, bc.qty_value, bc.qty_formula_code, bc.qty_formula_params, bc.uom
            FROM "BOMComponents" bc
            WHERE bc.bom_template_id = v_bom_template_id AND bc.deleted = false AND bc.component_item_id IS NOT NULL
            ORDER BY bc.created_at
        LOOP
            IF v_bom_component.component_role = 'fabric' THEN
                v_fabric_count := v_fabric_count + 1;
                IF v_fabric_count > 1 THEN
                    v_warnings := v_warnings || format('BOMTemplate %s allows MORE THAN ONE fabric component. Component ID: %s', v_bom_template_id, v_bom_component.id);
                    CONTINUE;
                END IF;
                ...
                WHERE ci.id = v_bom_component.component_item_id
                  AND ci.organization_id = v_mo.organization_id AND ci.deleted = false AND ci.active = true
                  AND ic.code = 'FABRIC'
                  AND ci.collection_name = v_sales_order_line.collection_name
                  AND ((v_sales_order_line.variant_name IS NULL AND ci.variant_name IS NULL) OR ci.variant_name = v_sales_order_line.variant_name);
                IF NOT FOUND THEN
                    v_warnings := v_warnings || format('Fabric CatalogItem %s does NOT match SaleOrderLine. SaleOrderLine: %s', v_bom_component.component_item_id, v_sales_order_line.id);
                    CONTINUE;
                END IF;
            ELSE
                SELECT ci.id, ci.sku, ci.item_name, ci.description, ci.cost_exw INTO v_catalog_item
                ...
                WHERE ci.id = v_bom_component.component_item_id
                  AND ci.organization_id = v_mo.organization_id AND ci.deleted = false AND ci.active = true;
                IF NOT FOUND THEN
                    v_warnings := v_warnings || format('CatalogItem %s (component_id: %s, role: %s) not found. SaleOrderLine: %s', v_bom_component.component_item_id, v_bom_component.id, v_bom_component.component_role, v_sales_order_line.id);
                    CONTINUE;
                END IF;
            END IF;
            ...
            IF v_bom_component.qty_formula_code = 'CHAIN_HEIGHT_FACTOR' THEN
                ...
                IF v_formula_params IS NULL OR (v_formula_params->>'height_factor') IS NULL OR (v_formula_params->>'mult') IS NULL OR v_sales_order_line.height_m IS NULL THEN
                    v_warnings := v_warnings || format('Invalid CHAIN_HEIGHT_FACTOR or NULL height_m. Component: %s', v_bom_component.id);
                    CONTINUE;
                END IF;
            ...
            ELSE
                v_warnings := v_warnings || format('Invalid qty_type for component %s', v_bom_component.id);
                CONTINUE;
            END IF;
            ...
            v_normalized_uom := CASE
                WHEN UPPER(TRIM(v_bom_component.uom)) IN ('PCS', 'PIECE', 'PIECES', 'SET', 'SETS', 'EA', 'EACH') THEN 'ea'
                WHEN v_bom_component.component_role = 'fabric' THEN 'm2'
                WHEN v_bom_component.component_role IN ('tube', 'bottom_bar', 'bottom_rail', 'bottom_rail_profile', 'chain') OR UPPER(TRIM(v_bom_component.uom)) IN ('FT', 'FEET', 'FOOT', 'MTS', 'M', 'METER', 'METERS') THEN 'm'
                WHEN UPPER(TRIM(v_bom_component.uom)) IN ('M2', 'SQM', 'SQ_M') THEN 'm2'
                ELSE NULL
            END;
            IF v_normalized_uom IS NULL THEN v_warnings := v_warnings || format('Invalid UOM for component %s', v_bom_component.id); CONTINUE; END IF;
            ...
            v_category_code := CASE
                ...
                WHEN v_bom_component.component_role LIKE '%bottom_rail%' OR v_bom_component.component_role LIKE '%bottom_channel%' OR v_bom_component.component_role LIKE '%bottom_bar%' THEN 'bottom_channel'
                ELSE 'accessory'
            END;
            ...
            IF v_unit_cost_exw > 0 THEN
                DECLARE v_unit_cost_with_taxes numeric(12,4); v_msrp_sale_in numeric(12,4);
                BEGIN
                    IF v_cost_from_msrp THEN v_unit_cost_with_taxes := v_unit_cost_exw;
                    ELSE v_unit_cost_with_taxes := v_unit_cost_exw * (1 + (v_shipping_percentage / 100.0) + (v_import_tax_percentage / 100.0));
                    END IF;
                ...
                END;
            ELSE
                v_unit_msrp_sale_out := 0; v_total_msrp_sale_out := 0;
            END IF;
