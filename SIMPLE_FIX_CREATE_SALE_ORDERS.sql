-- ====================================================
-- Simple Fix: Create SaleOrders for Approved Quotes
-- ====================================================
-- This script creates SaleOrders for all approved quotes without one
-- ====================================================
...
SELECT 
    ...
    q.status,
    q.organization_id,
    so.id AS existing_sale_order_id
FROM "Quotes" q
...
-- Now create SaleOrders for quotes that don't have one
DO $$
DECLARE
    v_quote RECORD;
    v_so_id uuid;
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            RAISE NOTICE '✅ Created SaleOrder % (%) for Quote % (%)', 
                v_so_id, v_so_no, v_quote.id, v_quote.quote_no;
    ...
    RAISE NOTICE '✅ Created % SaleOrder(s)', v_created_count;
END;
$$;
