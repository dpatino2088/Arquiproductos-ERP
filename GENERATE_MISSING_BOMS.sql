-- Generar BOMs para SO-087295 y SO-006385
DO $$
DECLARE
    v_so_nos text[] := ARRAY['SO-087295', 'SO-006385'];
    v_so_no text;
    v_so_id uuid;
    v_org_id uuid;
    v_quote_line RECORD;
    ...
    v_total integer := 0;
BEGIN
    FOREACH v_so_no IN ARRAY v_so_nos
    LOOP
        RAISE NOTICE '====================================================';
        RAISE NOTICE 'Generando BOM para %', v_so_no;
        RAISE NOTICE '====================================================';
        
        SELECT id, organization_id INTO v_so_id, v_org_id
        FROM "SalesOrders"
        WHERE sale_order_no = v_so_no
        AND deleted = false;
        
        IF v_so_id IS NULL THEN
            RAISE NOTICE '❌ % no encontrado', v_so_no;
            CONTINUE;
        END IF;
        ...
        RAISE NOTICE '✅ % completado', v_so_no;
    END LOOP;
    
    RAISE NOTICE '';
    RAISE NOTICE '✅ Total componentes creados: %', v_total;
END;
$$;
...
WHERE so.sale_order_no IN ('SO-087295', 'SO-006385', 'SO-071890')
