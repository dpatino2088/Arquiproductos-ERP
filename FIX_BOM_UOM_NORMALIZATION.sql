-- ====================================================
-- FIX: Add UOM normalization to BOM generation
-- ====================================================
-- This script updates generate_bom_for_manufacturing_order
-- to normalize UOM based on measure_basis
-- ====================================================

-- First, check if normalize_uom_to_canonical exists
SELECT 
    routine_name,
    routine_type
FROM information_schema.routines
WHERE routine_schema = 'public'
AND routine_name = 'normalize_uom_to_canonical';

-- If it doesn't exist, we'll create a simple normalization logic inline
-- Otherwise, we'll use the existing function

-- ====================================================
-- Update generate_bom_for_manufacturing_order to normalize UOM
-- ====================================================
    ...
    LOOP
        ...
        LOOP
            BEGIN
                ...
                v_normalized_uom := v_component_record.uom;
                
                -- Normalize UOM according to measure_basis
                IF v_measure_basis = 'linear_m' THEN
                    -- Linear components should be 'm' or 'm2'
                    IF v_normalized_uom NOT IN ('m', 'm2', 'mts', 'yd', 'ft') THEN
                        v_normalized_uom := 'm';  -- Default to 'm' for linear
                        RAISE NOTICE '⚠️ Normalized UOM for linear component %: % → m', v_component_record.sku, v_component_record.uom;
                    ELSIF v_normalized_uom IN ('mts', 'yd', 'ft') THEN
                        v_normalized_uom := 'm';  -- Map to canonical 'm'
                    END IF;
                ELSIF v_measure_basis = 'fabric_wxh' OR v_component_record.is_fabric THEN
                    -- Fabric should be 'm2'
                    IF v_normalized_uom NOT IN ('m2', 'yd2', 'ft2', 'sqm') THEN
                        v_normalized_uom := 'm2';  -- Default to 'm2' for fabric
                        RAISE NOTICE '⚠️ Normalized UOM for fabric component %: % → m2', v_component_record.sku, v_component_record.uom;
                    ELSIF v_normalized_uom IN ('yd2', 'ft2', 'sqm') THEN
                        v_normalized_uom := 'm2';  -- Map to canonical 'm2'
                    END IF;
                ELSIF v_measure_basis = 'unit' THEN
                    -- Unit components should be 'ea'
                    IF v_normalized_uom NOT IN ('ea', 'pcs', 'und', 'set', 'pack') THEN
                        v_normalized_uom := 'ea';  -- Default to 'ea' for unit
                        RAISE NOTICE '⚠️ Normalized UOM for unit component %: % → ea', v_component_record.sku, v_component_record.uom;
                    ELSIF v_normalized_uom IN ('pcs', 'und', 'set', 'pack') THEN
