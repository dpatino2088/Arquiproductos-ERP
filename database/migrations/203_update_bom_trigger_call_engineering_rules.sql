-- ====================================================
-- Migration: Update BOM Trigger to Call Engineering Rules
-- ====================================================
-- This migration updates the trigger function to call
-- apply_engineering_rules_to_bom_instance() after BomInstanceLines creation
-- ====================================================

-- Update the trigger function to call engineering rules after BOM creation
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        RAISE NOTICE '✅ Created SaleOrder % for Quote %', v_sale_order_no, NEW.id;
    ELSE
        RAISE NOTICE '⏭️  SaleOrder already exists for Quote %', NEW.id;
    END IF;
    
    -- Step B: Process each QuoteLine
    ...
    LOOP
        ...
        -- Only call if BomInstanceLines were created (v_bom_instance_id is set)
        IF v_bom_instance_id IS NOT NULL THEN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    -- Continue processing even if engineering rules fail
            END;
        END IF;
    END LOOP;
...
COMMENT ON FUNCTION public.on_quote_approved_create_operational_docs IS 
    'Trigger function that creates SaleOrder, SaleOrderLines, BomInstances, and BomInstanceLines when a Quote is approved. Includes base/pricing fields population and engineering rules application. Re-entrant: creates only missing pieces.';
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '✅ Migration 203 completed: Updated BOM Trigger to Call Engineering Rules';
    RAISE NOTICE '';
    RAISE NOTICE '📋 Updated:';
    RAISE NOTICE '   - Function: on_quote_approved_create_operational_docs()';
    RAISE NOTICE '   - Now calls apply_engineering_rules_to_bom_instance() after BomInstanceLines creation';
    RAISE NOTICE '   - Engineering rules compute cut_length_mm, cut_width_mm, cut_height_mm';
    RAISE NOTICE '';
END $$;
