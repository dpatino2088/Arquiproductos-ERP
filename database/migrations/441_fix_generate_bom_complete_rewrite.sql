-- ====================================================
-- Migration 441: Complete rewrite of generate_bom_for_manufacturing_order
-- ====================================================
-- OBJETIVO: Asegurar que la función SIEMPRE genere BOMs completos
-- - Valida MO existe
-- - Obtiene sales_order_id y SalesOrderLines
-- - Crea ManufacturingOrderLines automáticamente si no existen
-- - Crea BomInstances con manufacturing_order_id (obligatorio)
-- - Crea BomInstanceLines desde template
...
-- ====================================================
-- STEP 1: Drop and recreate function
-- ====================================================
...
BEGIN
    -- ====================================================
    -- STEP 1: Validate ManufacturingOrder exists
    -- ====================================================
    SELECT *
    INTO v_mo
    ...
    -- ====================================================
    -- STEP 2: Get sales_order_id from ManufacturingOrder
    -- ====================================================
    v_sales_order_id := v_mo.sales_order_id;
    
    IF v_sales_order_id IS NULL THEN
        RAISE EXCEPTION 'ManufacturingOrder % has no sales_order_id', p_manufacturing_order_id;
    END IF;
    
    RAISE NOTICE '[DEBUG] MO has sales_order_id: %', v_sales_order_id;
    
    -- ====================================================
    -- STEP 3: Get SalesOrderLines from SalesOrder
    -- ====================================================
    DECLARE
    ...
    BEGIN
        ...
        WHERE sol.sales_order_id = v_sales_order_id
            AND COALESCE(sol.deleted, false) = false
            AND COALESCE(sol.archived, false) = false;
        
        IF v_sol_count = 0 THEN
            RAISE EXCEPTION 'No SalesOrderLines found for sales_order_id % (MO: %)', 
                v_sales_order_id, p_manufacturing_order_id;
        END IF;
        
        RAISE NOTICE '[DEBUG] Found % SalesOrderLines', v_sol_count;
    END;
    
    -- ====================================================
    -- STEP 4: Create ManufacturingOrderLines if they don't exist
    -- ====================================================
    DECLARE
        v_mol_count integer := 0;
        v_mol_created integer := 0;
    BEGIN
        SELECT COUNT(*)
        INTO v_mol_count
        ...
        IF v_mol_count = 0 THEN
            ...
            GET DIAGNOSTICS v_mol_created = ROW_COUNT;
            
            RAISE NOTICE '[DEBUG] Created % ManufacturingOrderLines', v_mol_created;
            
            IF v_mol_created = 0 THEN
                RAISE WARNING 'Failed to create ManufacturingOrderLines (may already exist)';
            END IF;
        ELSE
            RAISE NOTICE '[DEBUG] Found % existing ManufacturingOrderLines', v_mol_count;
        END IF;
    END;
    
    -- ====================================================
    -- STEP 5: Load CostSettings
    -- ====================================================
    SELECT 
        shipping_percentage,
        import_tax_percent,
    ...
    IF FOUND THEN
        ...
        RAISE NOTICE '[DEBUG] Loaded CostSettings (labor: %%%)', v_labor_percentage;
    ELSE
        RAISE NOTICE '[DEBUG] No CostSettings found, using defaults';
    END IF;
    
    -- ====================================================
    -- STEP 6: Process each ManufacturingOrderLine
    -- ====================================================
    FOR v_mo_line IN
    ...
    LOOP
        ...
        RAISE NOTICE '[DEBUG] Processing MO Line %/% (sales_order_line_id: %)', 
            v_mo_lines_processed, v_mo_line.sales_order_line_id;
        
        -- ====================================================
        -- STEP 6a: Load SalesOrderLine
        -- ====================================================
        SELECT 
            sol.id,
            sol.product_type,
        ...
        IF NOT FOUND THEN
            ...
            RAISE WARNING 'SalesOrderLine % not found', v_mo_line.sales_order_line_id;
            CONTINUE;
        END IF;
        
        -- ====================================================
        -- STEP 6b: Resolve BOMTemplate
        -- ====================================================
        IF v_sales_order_line.product_type IS NULL OR TRIM(v_sales_order_line.product_type) = '' THEN
            ...
            RAISE WARNING 'SalesOrderLine % has NULL or empty product_type', v_sales_order_line.id;
            CONTINUE;
        END IF;
        ...
        -- ====================================================
        -- STEP 6c: Delete existing BOM for this SalesOrderLine
        -- ====================================================
        ...
        -- ====================================================
        -- STEP 6d: Create BomInstance with manufacturing_order_id
        -- ====================================================
        ...
        IF v_bom_instance_id IS NULL THEN
            ...
            RAISE WARNING 'Failed to create BomInstance for SalesOrderLine %', v_sales_order_line.id;
            CONTINUE;
        END IF;
        ...
        RAISE NOTICE '[DEBUG] Created BomInstance % (manufacturing_order_id: %, sales_order_line_id: %)', 
            v_bom_instance_id, p_manufacturing_order_id, v_sales_order_line.id;
        
        -- ====================================================
        -- STEP 6e: Process BOMComponents and create BomInstanceLines
        -- ====================================================
        ...
        ELSE
            RAISE NOTICE '[DEBUG] Created % BomInstanceLines for BomInstance %', v_created_lines, v_bom_instance_id;
        END IF;
        
        -- ====================================================
        -- STEP 6f: Calculate BOM-level costs and update BomInstance
        -- ====================================================
    ...
    -- ====================================================
    RAISE NOTICE '[DEBUG] BOM generation complete: % instances, % lines', 
        v_total_bom_instances, v_total_bom_lines;
    ...
    RETURN jsonb_build_object(
        ...
        'bom_lines_created', v_total_bom_lines,
...
COMMENT ON FUNCTION public.generate_bom_for_manufacturing_order IS 
    'Generates complete BOM for a ManufacturingOrder. Creates ManufacturingOrderLines if missing, then creates BomInstances with manufacturing_order_id (required) and BomInstanceLines from template.';
