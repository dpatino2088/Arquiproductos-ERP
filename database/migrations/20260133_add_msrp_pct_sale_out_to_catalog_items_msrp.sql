-- ====================================================
-- Migration: Añadir msrp_pct_sale_out a CatalogItemsMSRP
-- Date: 2026-02-01
-- ====================================================
-- Fixes: "column msrp_pct_sale_out of relation CatalogItemsMSRP does not exist"
--
-- msrp_pct_sale_out (= margen Sale Out / precio público) viene de
-- CategoryMargins.msrp_pct_sale_out o CostSettings.default_msrp_pct_sale_out.
-- Se añade, se rellena en backfill y en msrp_compute_for_item (v_sale_out_pct).
--
-- Requiere: 20260132 (msrp_compute_for_item escribe minimum_margin_pct, etc.).
-- ====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public."CatalogItemsMSRP" ADD COLUMN msrp_pct_sale_out numeric(7,4);
    RAISE NOTICE '✅ CatalogItemsMSRP: columna msrp_pct_sale_out añadida';
  END IF;
END $$;

-- 2) Backfill: CategoryMargins.msrp_pct_sale_out por (org, category) o CostSettings.default_msrp_pct_sale_out
UPDATE public."CatalogItemsMSRP" cim
SET msrp_pct_sale_out = COALESCE(
  (SELECT cm.msrp_pct_sale_out FROM public."CategoryMargins" cm
   WHERE cm.organization_id = cim.organization_id AND cm.category_id = cim.category_id
   LIMIT 1),
  (SELECT cs.default_msrp_pct_sale_out FROM public."CostSettings" cs
   WHERE cs.organization_id = cim.organization_id
   LIMIT 1),
  0.65
);

-- 3) msrp_compute_for_item: incluir msrp_pct_sale_out en INSERT y ON CONFLICT
...
'Calcula MSRP para un CatalogItem. Guarda shipping_pct, import_tax_pct, minimum_margin_pct y msrp_pct_sale_out en CatalogItemsMSRP.';
