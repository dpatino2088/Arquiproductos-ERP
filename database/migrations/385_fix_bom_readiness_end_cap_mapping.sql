-- ====================================================
-- Migration 385: Fix BOM Readiness Report - Recognize end_cap Special Case
-- ====================================================
-- Updates bom_readiness_report to recognize that 'end_cap' is a special case
-- that maps to COMP-HARDWARE via get_item_category_codes_from_role,
-- even though it doesn't have an explicit entry in ComponentRoleMap.
-- ====================================================
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            -- Count resolvable auto-select components (have valid role mapping and CatalogItems exist)
            v_auto_select_resolvable_count := 0;
            FOR v_component IN
                SELECT DISTINCT bc.component_role
                FROM "BOMComponents" bc
...
COMMENT ON FUNCTION public.bom_readiness_report(uuid) IS 
    'Analyzes BOM readiness for all ProductTypes in an organization. Returns JSON array with status, issues, and suggested seeds. Updated to recognize end_cap special case mapping.';
