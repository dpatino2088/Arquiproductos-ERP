-- ====================================================
-- Migration: Fix convert_quote_to_sale_order function
-- ====================================================
-- This migration updates the convert_quote_to_sale_order function to:
-- 1. Use get_next_counter_value for safe numbering
-- 2. Return existing Sale Order if already created by trigger
-- 3. Better error handling
-- ====================================================

-- Recreate the function with improvements
...
'Converts an approved Quote to a Sale Order, copying all QuoteLines to SaleOrderLines. Returns the new Sale Order ID. If a Sale Order already exists for this Quote (created by trigger), returns the existing ID.';
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration 180 completed successfully!';
    RAISE NOTICE '📋 Updated:';
    RAISE NOTICE '   - Function: convert_quote_to_sale_order(uuid, uuid, uuid)';
    RAISE NOTICE '   - Now uses get_next_counter_value for safe numbering';
    RAISE NOTICE '   - Returns existing Sale Order if already created by trigger';
