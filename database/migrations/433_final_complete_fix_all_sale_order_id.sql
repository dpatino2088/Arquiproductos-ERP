-- ====================================================
-- Migration 433: FINAL COMPLETE FIX - ALL sale_order_id → sales_order_id
-- ====================================================
-- Esta migración busca y corrige CUALQUIER referencia a sale_order_id (sin 's')
-- en TODAS las funciones relacionadas con ManufacturingOrders
-- ====================================================

SET search_path = public;

-- ====================================================
-- STEP 1: Buscar TODAS las funciones que mencionan ManufacturingOrders
-- ====================================================
...
DECLARE
    ...
    v_func_def text;
    v_problem_functions text[] := ARRAY[]::text[];
BEGIN
    RAISE NOTICE '========================================';
    ...
    LOOP
        ...
        IF v_func_def ~ '\bNEW\.sale_order_id\b' OR 
           ...
           (v_func_def ~ '\bManufacturingOrders' AND v_func_def ~ '\bsale_order_id\b' AND v_func_def !~ '\bsales_order_id\b') THEN
            v_problem_functions := array_append(v_problem_functions, v_func_record.function_name);
            RAISE WARNING '⚠️ Function % contains sale_order_id (without s)', v_func_record.function_name;
        END IF;
    END LOOP;
    
    IF array_length(v_problem_functions, 1) > 0 THEN
        RAISE WARNING 'Funciones problemáticas encontradas: %', array_to_string(v_problem_functions, ', ');
    ELSE
...
-- ====================================================
-- STEP 2: FORCE DROP AND RECREATE ALL CRITICAL FUNCTIONS
-- ====================================================
-- Usamos DROP ... CASCADE para asegurar que se eliminen todas las dependencias
-- ====================================================
...
BEGIN
    -- ✅ FORZADO: sales_order_id (con 's') - NO sale_order_id
...
-- ====================================================
-- STEP 3: DROP AND RECREATE ALL TRIGGERS
-- ====================================================

-- Drop ALL existing triggers on ManufacturingOrders
...
BEGIN
    ...
    ELSE
        RAISE WARNING '========================================';
        RAISE WARNING '⚠️ SE ENCONTRARON % PROBLEMAS - Revisar funciones arriba', v_problem_count;
        RAISE WARNING '========================================';
    END IF;
END $$;
