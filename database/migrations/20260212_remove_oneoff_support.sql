-- ============================================================================
-- Migration: Remove One-Off fabric support
-- Date: 2026-02-12
-- Description: One-Off tab removed from UI. Revert create_configured_product_and_bom_preview
--              and calculate_configured_product_totals to catalog-variant only (no oneoff_*).
-- ============================================================================

-- 1) create_configured_product_and_bom_preview: roll only from catalog variant (remove ELSIF oneoff)
...
COMMENT ON FUNCTION public.create_configured_product_and_bom_preview(uuid, uuid, jsonb, uuid, uuid) IS 'Creates ConfiguredProduct with pricing. Roll from catalog variant only (roll_catalog_item_id).';

-- 2) calculate_configured_product_totals: roll only from roll_catalog_item_id (remove oneoff branch)
...
COMMENT ON FUNCTION public.calculate_configured_product_totals(uuid) IS 'Recalculates ConfiguredProduct totals. Roll from catalog (roll_catalog_item_id) only.';
