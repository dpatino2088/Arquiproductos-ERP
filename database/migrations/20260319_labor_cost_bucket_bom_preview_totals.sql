-- ====================================================
-- Migration 20260319: Labor como bucket explícito en bom_preview_snapshot.totals
-- ====================================================
-- PROBLEMA: CostSettings.labor_pct existe pero labor_cost no se calcula ni persiste.
-- Labor se "pierde" porque unit_labor_cost nunca se calculaba (siempre 0 o legacy).
--
-- REQUERIMIENTO:
-- 1) labor_cost = base_cost_for_labor * labor_pct
--    base_cost_for_labor = roll_cost + bom_cost + accessories_cost (EXCLUIR labor)
-- 2) Persistir en totals: labor_cost, labor_msrp_total, labor_dealer_total
-- 3) total_cost = roll_cost + bom_cost + labor_cost + accessories_cost
-- 4) Option B sin cambios: unit_dealer_price = total_msrp * (1 - discount)
--
-- FUTURE-ONLY: No backfill.
-- ====================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- calculate_configured_product_totals: calcular y persistir labor_cost
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- labor_pct: CP first, then CostSettings
  ...
  -- labor_cost = base_cost_for_labor * labor_pct (base = materiales, EXCLUIR labor)
  ...
  SET
    ...
    bom_preview_snapshot = jsonb_set(
      ...
      jsonb_build_object(
        ...
        'labor_cost_pct', CASE WHEN v_labor_pct <= 1 THEN v_labor_pct * 100 ELSE v_labor_pct END,
...
'Totals from CatalogItemsMSRP. Includes labor_cost bucket: base_cost * labor_pct. Persists labor_cost, labor_msrp_total, labor_dealer_total in bom_preview_snapshot.totals.';

-- ----------------------------------------------------------------------------
-- build_bom_preview_snapshot: incluir labor_cost en totals (para preview inicial)
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- labor_cost bucket: base_cost * labor_pct (base = roll + bom cost)
...
'BOM preview JSONB. Includes labor_cost bucket in totals. Final totals from calculate_configured_product_totals.';
