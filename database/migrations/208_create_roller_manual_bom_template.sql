-- ====================================================
-- Migration: Create BOM Template for Manual Roller Shade
-- ====================================================
-- Creates a BOM template named 'ROLLER_MANUAL_STANDARD' for manual roller shades
-- Based on the diagram provided with components: brackets, tube, fabric, bottom rail, end caps, manual drive, chain
-- ====================================================
...
DECLARE
    ...
    v_required_skus text[] := ARRAY[
        ...
        'RCA04',   -- bottom rail
        'RCA21',   -- bottom rail end cap
        'RC3001',  -- manual drive option 1
        'RC3002',  -- manual drive option 2
        'RC3003',  -- manual drive option 3
        'V15DP',   -- chain option 1
        'V15M'     -- chain option 2
    ];
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Creating BOM Template: ROLLER_MANUAL_STANDARD';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    ...
    IF v_template_id IS NULL THEN
        ...
        VALUES (v_org_id, v_roller_shade_type_id, 'ROLLER_MANUAL_STANDARD', 'Manual Roller Shade - Standard Configuration (Chain/Clutch)', true, false)
    ...
    -- F) Manual Drive - using 'operating_system_drive' (constraint allows this, resolver will filter by drive_type)
    -- Note: Using 'operating_system_drive' instead of 'manual_drive' to match constraint
    -- The resolver should use block_condition to filter for manual drives
    INSERT INTO "BOMComponents" (
        bom_template_id, organization_id, component_role, component_item_id, 
        qty_per_unit, uom, sequence_order, 
        ...
        1, 'ea', v_seq,
        'drive', '{"drive_type": "manual"}'::jsonb,
        false, true, false
    );
    RAISE NOTICE '  ✅ Inserted operating system drive (manual, auto_select=true) at sequence %', v_seq;
    v_seq := v_seq + 10;
    
    -- G) Chain - Note: 'chain' is not in the constraint, using 'operating_system_drive' with metadata or skipping
    -- Since 'chain' is not a valid role, we'll skip it for now or add it to the constraint
    -- For now, commenting out chain component - it may need to be added as an accessory or handled differently
    RAISE NOTICE '  ⚠️  Skipping chain component - "chain" role not in constraint. Consider adding it or handling as accessory.';
    ...
    RAISE NOTICE '      to pick the correct SKU based on hardware_color, width rules, etc.';
    RAISE NOTICE '';
...
BEGIN
    ...
    IF v_template_id IS NULL THEN
        RAISE NOTICE '⚠️  Template ROLLER_MANUAL_STANDARD not found';
        RETURN;
    END IF;
...
-- 3) Missing SKUs report (using required list + LEFT JOIN)
...
SELECT DISTINCT
    ...
    CASE 
        ...
        WHEN rs.sku IN ('RC3001', 'RC3002', 'RC3003') THEN 'manual drive'
        WHEN rs.sku IN ('V15DP', 'V15M') THEN 'chain'
        ELSE 'unknown'
