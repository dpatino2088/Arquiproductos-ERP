-- ====================================================
-- Migration 403: Add debug logs to BOM generation and resolve_auto_select_sku
-- ====================================================
-- Adds RAISE NOTICE logs to help debug BOM generation issues
-- ====================================================

-- Update resolve_auto_select_sku to add debug logs
...
BEGIN
    ...
    RAISE NOTICE '[resolve_auto_select_sku] ENTRY: role=%, rule=%, color=%, org_id=%', 
        p_component_role, p_sku_resolution_rule, p_hardware_color, p_organization_id;
    ...
    -- ✅ DEBUG: Log category code resolution
    RAISE NOTICE '[resolve_auto_select_sku] category_code resolved: % (from role: %)', 
        v_category_code, p_component_role;
    ...
    ELSIF p_sku_resolution_rule = 'CATEGORY_FIRST_MATCH' THEN
        -- ✅ DEBUG: Log CATEGORY_FIRST_MATCH branch
        RAISE NOTICE '[resolve_auto_select_sku] Using CATEGORY_FIRST_MATCH for role=%, category_code=%', 
            p_component_role, v_category_code;
        
        -- ✅ NEW: CATEGORY_FIRST_MATCH - Select first available item in category (ignore hardware_color)
        -- This is useful when the component doesn't depend on color or when color is not specified
        ...
        IF v_resolved_catalog_item_id IS NULL THEN
            RAISE NOTICE '[resolve_auto_select_sku] ❌ CATEGORY_FIRST_MATCH: No items found for category_code=%, organization_id=%', 
                v_category_code, p_organization_id;
            RAISE EXCEPTION 'Could not resolve catalog_item_id for auto-select component: role=%, sku_resolution_rule=CATEGORY_FIRST_MATCH, category_code=%, organization_id=%', 
                p_component_role, v_category_code, p_organization_id;
        ELSE
            RAISE NOTICE '[resolve_auto_select_sku] ✅ CATEGORY_FIRST_MATCH: Resolved to catalog_item_id=%', 
    ...
    ELSE
        -- ✅ DEBUG: Log unsupported rule
        RAISE NOTICE '[resolve_auto_select_sku] ❌ Unsupported sku_resolution_rule: %', p_sku_resolution_rule;
        RAISE EXCEPTION 'Unsupported sku_resolution_rule for auto-select: %. Supported values: EXACT_SKU, CATEGORY_FIRST_MATCH, SKU_SUFFIX_COLOR, ROLE_AND_COLOR', 
...
COMMENT ON FUNCTION public.resolve_auto_select_sku IS 
    'Resolves catalog_item_id for auto-select BOM components using ComponentRoleMap for role→category_code mapping. Supports CATEGORY_FIRST_MATCH (ignores color), SKU_SUFFIX_COLOR, and ROLE_AND_COLOR (considers color). Uses deterministic selection: selection_priority ASC, sku ASC. Prefers HardwareColorMapping table, falls back to SKU pattern matching.';
