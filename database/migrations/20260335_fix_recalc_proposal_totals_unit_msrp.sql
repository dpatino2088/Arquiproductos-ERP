-- ============================================================================
-- Migration: Fix recalc_proposal_totals — ql.unit_msrp does not exist
-- ============================================================================
-- La migración 20260330 reintrodujo referencias a ql.unit_msrp en recalc_proposal_totals.
-- QuoteLines usa unit_msrp_total_snapshot (canónica); unit_msrp puede no existir.
-- Reemplazar ql.unit_msrp por: COALESCE(ql.unit_msrp_total_snapshot, ql.msrp/NULLIF(ql.quantity,0))
-- ============================================================================
...
BEGIN
  ...
  -- Line base: use unit_msrp_total_snapshot (canonical); fallback msrp/quantity. No unit_msrp.
  SELECT COALESCE(SUM(
    CASE
...
COMMENT ON FUNCTION public.recalc_proposal_totals(uuid) IS 'Recalculates Proposals totals. Uses unit_msrp_total_snapshot (not unit_msrp). When exempt_itbms=true, itbms_amount=0.';
