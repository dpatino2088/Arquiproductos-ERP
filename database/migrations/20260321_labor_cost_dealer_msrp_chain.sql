-- ====================================================
-- Migration 20260321: Labor chain cost → dealer → MSRP (margin-on-sale)
-- ====================================================
-- FÓRMULAS (margen sobre venta, no markup sobre costo):
--   labor_cost_base  = materiales × labor_cost_pct         (ej. $1000 × 5% = $50)
--   labor_dealer     = labor_cost_base / (1 - labor_dealer_pct)  (ej. $50 / 0.65 = $76.92 si 35% margen)
--   labor_msrp       = labor_dealer / (1 - labor_msrp_pct)       (ej. $76.92 / 0.35 = $219.77 si 65% margen)
--
-- labor_dealer_pct = margen sobre venta dealer (0.35 = 35%). dealer = cost / (1 - 0.35)
-- labor_msrp_pct   = margen sobre venta MSRP (0.65 = 65%).   msrp = dealer / (1 - 0.65)
-- ====================================================

BEGIN;

ALTER TABLE public."CostSettings"
  ADD COLUMN IF NOT EXISTS labor_dealer_pct numeric(7,4) DEFAULT NULL;

ALTER TABLE public."CostSettings"
  ADD COLUMN IF NOT EXISTS labor_msrp_pct numeric(7,4) DEFAULT NULL;

COMMENT ON COLUMN public."CostSettings".labor_dealer_pct IS
'Labor dealer margin-on-sale (0.35 = 35%). labor_dealer = labor_cost / (1 - labor_dealer_pct). If NULL, falls back to minimum_margin_pct.';

COMMENT ON COLUMN public."CostSettings".labor_msrp_pct IS
'Labor MSRP margin-on-sale (0.65 = 65%). labor_msrp = labor_dealer / (1 - labor_msrp_pct). If NULL, falls back to default_msrp_pct.';

-- ----------------------------------------------------------------------------
-- calculate_configured_product_totals: chain cost → dealer → MSRP
-- ----------------------------------------------------------------------------
...
DECLARE
  ...
  v_denom_dealer numeric;
  v_denom_msrp numeric;
BEGIN
  ...
  -- labor_dealer_pct, labor_msrp_pct: margin-on-sale. Fallback to org defaults.
  v_labor_dealer_pct := COALESCE(v_cs.labor_dealer_pct, v_cs.minimum_margin_pct, 0.35);
  v_labor_msrp_pct   := COALESCE(v_cs.labor_msrp_pct, v_cs.default_msrp_pct, 0.65);
  ...
  -- labor_cost_base = materiales × labor_cost_pct
  ...
  -- Chain: labor_dealer = labor_cost / (1 - labor_dealer_pct), labor_msrp = labor_dealer / (1 - labor_msrp_pct)
  v_denom_dealer := 1 - (CASE WHEN v_labor_dealer_pct <= 1 THEN v_labor_dealer_pct ELSE (v_labor_dealer_pct / 100.0) END);
  v_denom_msrp   := 1 - (CASE WHEN v_labor_msrp_pct <= 1 THEN v_labor_msrp_pct ELSE (v_labor_msrp_pct / 100.0) END);
  IF COALESCE(v_denom_dealer, 0) <= 0 THEN v_denom_dealer := 0.65; END IF;
  IF COALESCE(v_denom_msrp, 0) <= 0 THEN v_denom_msrp := 0.35; END IF;

  v_labor_dealer := ROUND(v_unit_labor_cost / v_denom_dealer, 4);
  v_labor_msrp   := ROUND(v_labor_dealer / v_denom_msrp, 4);

  v_unit_msrp_total := v_roll_msrp_total + v_labor_msrp + v_bom_total + v_accessories_total;
  v_unit_dealer_price := v_roll_dealer_total + v_labor_dealer + v_bom_total + v_accessories_total;
...
'Labor chain: cost = materiales×labor_cost_pct; dealer = cost/(1-dealer_pct); msrp = dealer/(1-msrp_pct). Margin-on-sale.';

-- ----------------------------------------------------------------------------
-- build_bom_preview_snapshot: chain cost → dealer → MSRP
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Chain: labor_cost = base × labor_cost_pct; labor_dealer = labor_cost/(1-dealer_pct); labor_msrp = labor_dealer/(1-msrp_pct)
  ...
  v_labor_dealer := ROUND(v_labor_cost / v_denom_dealer, 4);
  v_labor_amount := ROUND(v_labor_dealer / v_denom_msrp, 4);
...
'BOM preview. Labor chain: cost→dealer→msrp (margin-on-sale).';
