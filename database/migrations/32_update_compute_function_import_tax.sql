-- ====================================================
-- Migration: Update compute_quote_line_cost for REAL Import Tax
-- ====================================================
-- Calculate import tax from real QuoteLineComponents grouped by category
-- Uses ImportTaxRules per category, falls back to CostSettings
-- ====================================================
...
BEGIN
    ...
    -- Step 7: Calculate import_tax_cost from REAL components
    ...
    ELSE
        ...
        -- Loop through all components for this quote line
        ...
        LOOP
            -- Determine unit_cost
            v_unit_cost := COALESCE(v_component_record.unit_cost_exw, v_component_record.catalog_cost_exw, 0);
            
            -- Calculate extended_cost
            v_extended_cost := v_component_record.qty * v_unit_cost;
            ...
            IF v_component_record.item_category_id IS NOT NULL THEN
                ...
                FROM "ImportTaxRules"
                WHERE organization_id = v_quote_line_record.organization_id
                AND category_id = v_component_record.item_category_id
                AND active = true
                AND deleted = false
                LIMIT 1;
            ...
            v_category_tax_amount := v_extended_cost * (v_category_tax_percentage / 100.0);
...
DO $$
BEGIN
    RAISE NOTICE '✅ Updated compute_quote_line_cost for REAL import tax calculation';
    RAISE NOTICE '📋 Features:';
    RAISE NOTICE '   - Calculates import tax from QuoteLineComponents';
    RAISE NOTICE '   - Groups by category and applies category-specific rules';
    RAISE NOTICE '   - Falls back to global default if no category rule exists';
    RAISE NOTICE '   - Respects manual overrides (import_tax_source)';
