-- ============================================================================
-- Migration: QuoteLines – add drop, installation type and location + RPC params
-- Date: 2026-02-04
-- Description: Add fabric_drop, installation_type, installation_location so
--   configurator data (Area, Position, Drop, Installation Type & Location)
--   is persisted in QuoteLines. Update commit_configured_product_to_quote_line
--   to accept and write these fields.
-- ============================================================================

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. ADD COLUMNS TO QuoteLines
-- ═══════════════════════════════════════════════════════════════════════════
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'QuoteLines' AND column_name = 'fabric_drop'
  ) THEN
    ALTER TABLE public."QuoteLines" ADD COLUMN fabric_drop text;
    RAISE NOTICE 'Added QuoteLines.fabric_drop';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'QuoteLines' AND column_name = 'installation_type'
  ) THEN
    ALTER TABLE public."QuoteLines" ADD COLUMN installation_type text;
    RAISE NOTICE 'Added QuoteLines.installation_type';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'QuoteLines' AND column_name = 'installation_location'
  ) THEN
    ALTER TABLE public."QuoteLines" ADD COLUMN installation_location text;
    RAISE NOTICE 'Added QuoteLines.installation_location';
  END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════════════════
-- 2. UPDATE RPC: add p_fabric_drop, p_installation_type, p_installation_location
--    (Function body must match 20260204_quoteline_pricing_from_configured_product
--     plus new params and INSERT columns.)
-- ═══════════════════════════════════════════════════════════════════════════
