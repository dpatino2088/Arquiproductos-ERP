-- ====================================================
-- MIGRATION 59: Migrate roll_width legacy to new schema
-- Date: 2026-02-03
--
-- Purpose:
-- - Migrate data from roll_width (legacy, in meters) to roll_width_value + roll_width_uom
-- - Drop roll_width column after migration
-- - Update triggers and functions to use only new schema
-- ====================================================

BEGIN;

-- Step 1: Migrate existing roll_width data to new schema
-- Note: roll_width legacy is ALWAYS in meters (normalized)
-- unit_of_measure is for PRICE (cost_exw), not dimensions

-- 1a) Migrate roll_width to roll_width_value/uom (only if value is missing)
...
-- 1b) Set roll_length for ALL rolls that don't have it yet
UPDATE public."CatalogItems"
SET
...
-- Step 3: Drop legacy roll_width column
