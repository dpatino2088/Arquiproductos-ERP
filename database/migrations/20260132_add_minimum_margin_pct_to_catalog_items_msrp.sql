-- ====================================================
-- Migration: Añadir minimum_margin_pct a CatalogItemsMSRP
-- Date: 2026-02-01
-- ====================================================
-- Fixes: "column minimum_margin_pct of relation CatalogItemsMSRP does not exist"
--
-- minimum_margin_pct (= margen Sale In) viene de CategoryMargins.msrp_pct_sale_in
-- o CostSettings.minimum_margin_pct. Se añade, se rellena en backfill y en
-- msrp_compute_for_item (v_sale_in_pct).
--
-- Requiere: 20260131 (msrp_compute_for_item escribe shipping_pct, import_tax_pct).
-- ====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'CatalogItemsMSRP' AND column_name = 'minimum_margin_pct'
  ) THEN
    ALTER TABLE public."CatalogItemsMSRP" ADD COLUMN minimum_margin_pct numeric(7,4);
    RAISE NOTICE '✅ CatalogItemsMSRP: columna minimum_margin_pct añadida';
  END IF;
END $$;

-- 2) Backfill: CategoryMargins.msrp_pct_sale_in por (org, category) o CostSettings.minimum_margin_pct
UPDATE public."CatalogItemsMSRP" cim
SET minimum_margin_pct = COALESCE(
  (SELECT cm.msrp_pct_sale_in FROM public."CategoryMargins" cm
   WHERE cm.organization_id = cim.organization_id AND cm.category_id = cim.category_id
   LIMIT 1),
  (SELECT cs.minimum_margin_pct FROM public."CostSettings" cs
   WHERE cs.organization_id = cim.organization_id
   LIMIT 1),
  0.35
);

-- 3) msrp_compute_for_item: incluir minimum_margin_pct en INSERT y ON CONFLICT
...
'Calcula MSRP para un CatalogItem. Guarda shipping_pct, import_tax_pct y minimum_margin_pct en CatalogItemsMSRP.';
