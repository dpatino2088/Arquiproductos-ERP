-- ============================================================================
-- 2026-03-27
-- Remove labor_cost_pct: column was dropped from CostSettings.
-- Use labor_pct only for labor cost calculation.
-- ============================================================================

BEGIN;

-- Re-apply calculate_configured_product_totals without cs.labor_cost_pct
...
BEGIN
  ...
  -- Rebuild snapshot when cost data is missing (critical for commit RPC)
  ...
  -- FIX: snapshot roll costs/MSRP include product_qty; normalize to per-unit
  IF v_qty > 1 THEN
  ...
  -- Cost inputs: labor_pct only (labor_cost_pct column removed)
...
'Pricing ladder: COST -> DEALER -> MSRP. labor_pct only (labor_cost_pct removed). unit_dealer_price = total_cost/(1-minimum_margin_pct).';
