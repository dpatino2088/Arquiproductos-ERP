-- ====================================================
-- Migration 380: Fix bom_readiness_report array concatenation
-- ====================================================
-- Fix: v_result is initialized as array '[]' but we try to concat objects with ||
-- Solution: Build array properly using jsonb_build_array() or append elements correctly
-- ====================================================
...
DECLARE
    ...
    v_items jsonb[] := ARRAY[]::jsonb[];  -- Use array to collect items
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            -- C) Validate component roles (canonical roles)
            ...
            AND bc.component_role NOT IN (
                ...
                'hardware', 'accessory', 'service', 'window_film', 'end_cap', 'operating_system'
            );
            ...
            IF v_invalid_role_count > 0 THEN
                v_issues := v_issues || jsonb_build_object(
                    'type', 'INVALID_ROLES',
                    'severity', 'BLOCKER',
                    'message', format('%s component(s) have invalid/legacy roles. Please migrate to canonical roles.', v_invalid_role_count)
                );
            END IF;
            
            -- Collect unique component roles used in templates
            SELECT ARRAY_AGG(DISTINCT bc.component_role) INTO v_category_codes
            FROM "BOMComponents" bc
            INNER JOIN "BOMTemplates" bt ON bt.id = bc.bom_template_id
            WHERE bt.product_type_id = v_product_type.id
            AND bt.organization_id = p_organization_id
            ...
            -- D & E) Check ComponentRoleMap for each role used
            IF v_category_codes IS NOT NULL THEN
                FOR v_role_mapping IN
                    SELECT DISTINCT bc.component_role as role
                    FROM "BOMComponents" bc
                    INNER JOIN "BOMTemplates" bt ON bt.id = bc.bom_template_id
                    WHERE bt.product_type_id = v_product_type.id
                    AND bt.organization_id = p_organization_id
                    ...
                    AND bc.component_role IS NOT NULL
                LOOP
                    -- Check if role has mapping in ComponentRoleMap
                    IF NOT EXISTS (
                        SELECT 1
                        FROM "ComponentRoleMap" crm
                        WHERE crm.role = v_role_mapping.role
                        AND crm.active = true
                    ) THEN
                        v_missing_mappings := v_missing_mappings || v_role_mapping.role;
                        
                        v_issues := v_issues || jsonb_build_object(
                            'type', 'MISSING_ROLE_MAPPING',
                            'severity', 'BLOCKER',
                            'message', format('Component role "%s" has no mapping in ComponentRoleMap', v_role_mapping.role)
                        );
                        
                        -- Suggest seed (but don't generate full SQL - needs manual ItemCategory mapping)
                        v_suggested_seeds := v_suggested_seeds || jsonb_build_object(
            ...
            -- F) Validate Fixed Components (CatalogItems exist, have UOM, have item_category_id)
            SELECT 
            ...
            -- G) Validate Auto-Select Components
            ...
            -- Check if auto-select components are resolvable (simplified check)
            -- This checks if there are any CatalogItems in the mapped categories
            FOR v_component IN
            ...
            IF v_auto_select_count > 0 AND v_auto_select_resolvable_count < v_auto_select_count THEN
                v_issues := v_issues || jsonb_build_object(
                    'type', 'AUTO_SELECT_VALIDATION',
                    'severity', 'WARN',
                    'message', format('%s auto-select component(s), %s are resolvable', 
        ...
        v_product_type_result := jsonb_build_object(
            ...
            'stats', jsonb_build_object(
                ...
                'invalid_roles_count', COALESCE(v_invalid_role_count, 0)
            )
        );
        
        -- FIX: Append to array properly using array_append, then convert to jsonb array
...
'Analyzes BOM readiness for all ProductTypes in an organization. Returns JSON array with status, issues, and suggested seeds for each ProductType.';
