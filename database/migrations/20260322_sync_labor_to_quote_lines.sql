-- ====================================================
-- Migration 20260322: Sincronizar labor a QuoteLines
-- ====================================================
-- sync_quote_line_pricing_from_configured_product y commit_configured_product_to_quote_line
-- deben escribir labor_cost_snapshot, labor_msrp_snapshot en QuoteLines para que el UI
-- muestre Labor Cost y Labor MSRP correctamente (ReviewStep lee de totals o cpDirect).
-- ====================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- sync_quote_line_pricing_from_configured_product: añadir labor_cost_snapshot, labor_msrp_snapshot
-- ----------------------------------------------------------------------------
...
'Refreshes QuoteLine pricing from ConfiguredProduct. Includes labor_cost_snapshot, labor_msrp_snapshot from labor chain.';

-- ----------------------------------------------------------------------------
-- commit_configured_product_to_quote_line: añadir labor_cost_snapshot, labor_msrp_snapshot al INSERT
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_unit_cost = 0 THEN
    v_unit_cost := COALESCE((v_totals->>'total_cost')::numeric,
      COALESCE(v_cp.roll_total_cost, 0) + COALESCE(v_cp.bom_total_cost, 0)
        + COALESCE(v_cp.accessories_total_cost, 0) + v_labor_cost, 0);
  END IF;
  ...
  VALUES (
    ...
    COALESCE(v_cp.roll_msrp_total, 0), COALESCE(v_cp.bom_total, 0), v_labor_msrp,
    COALESCE(v_cp.roll_total_cost, 0), COALESCE(v_cp.bom_total_cost, 0), v_labor_cost,
...
'Creates QuoteLine from ConfiguredProduct. Includes labor_cost_snapshot, labor_msrp_snapshot from labor chain.';
