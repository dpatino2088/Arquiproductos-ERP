-- ============================================================================
-- Migration: QuoteLines add unit_msrp (sub-total / unit price)
-- Date: 2026-02-05
-- Description:
--   Add unit_msrp to QuoteLines so UI can show:
--   - unit_msrp = precio unitario (sub-total por unidad)
--   - msrp = total de línea (unit_msrp * quantity)
--   UI only reads these from backend; no client-side calculation.
-- ============================================================================

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. ADD unit_msrp COLUMN TO QuoteLines
-- ═══════════════════════════════════════════════════════════════════════════
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'unit_msrp'
  ) THEN
    ALTER TABLE public."QuoteLines"
    ADD COLUMN unit_msrp numeric(12,4) NULL;
    COMMENT ON COLUMN public."QuoteLines".unit_msrp IS
    'Precio unitario MSRP (sub-total por unidad). Line total = msrp = unit_msrp * quantity. Set by commit_configured_product_to_quote_line.';
  END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════════════════
-- 2. BACKFILL: unit_msrp = msrp / qty for existing rows
--    Use quantity if column exists, else qty (QuoteLines may have either)
-- ═══════════════════════════════════════════════════════════════════════════
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'QuoteLines' AND column_name = 'quantity') THEN
    UPDATE public."QuoteLines"
    SET unit_msrp = msrp / NULLIF(GREATEST(COALESCE(quantity, 1), 0.001), 0)
    WHERE unit_msrp IS NULL AND msrp IS NOT NULL;
  ELSE
    UPDATE public."QuoteLines"
    SET unit_msrp = msrp / NULLIF(GREATEST(COALESCE(qty, 1), 0.001), 0)
    WHERE unit_msrp IS NULL AND msrp IS NOT NULL;
  END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════════════════
-- 3. UPDATE RPC commit_configured_product_to_quote_line: set unit_msrp
--    Same signature and logic as 20260204_quotelines_add_drop_installation_columns,
--    plus: v_unit_msrp and INSERT unit_msrp.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Specify full signature so PostgreSQL can target the 8-param version (others may exist with 5 params)
COMMENT ON FUNCTION public.commit_configured_product_to_quote_line(uuid, uuid, uuid, uuid, text, text, text, text, text) IS
'Creates QuoteLine from ConfiguredProduct. Sets unit_msrp (unit price) and msrp (line total). UI reads these only; no client-side calculation.';
