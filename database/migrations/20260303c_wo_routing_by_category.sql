-- ============================================================================
-- Fix WO routing: use CatalogCategories parent name instead of measure_basis
-- ============================================================================
-- Routes BOM lines to work centers based on the parent category name:
--   Parent "Profiles" → CUT-PROFILE  (Head Box, Side Channels, Bottom Bars, etc.)
--   Parent "Rolls"    → CUT-ROLL     (fabrics, screens, films)
--   Everything else   → ASSEMBLY     (Hardware, Accessories, Systems, Drives, etc.)
-- ============================================================================

SET search_path = public;

-- --------------------------------------------------------------------------
-- 1. Update WorkCenters routing rules to use category_parent_names
-- --------------------------------------------------------------------------
UPDATE "WorkCenters"
...
-- --------------------------------------------------------------------------
-- 2. Drop old function signature, recreate with p_regenerate + category logic
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    -- ── Extract arrays from routing_rule ──
    ...
    -- ── Iterate BOM lines, join to category hierarchy ──
    FOR v_line IN
      SELECT
        bil.id           AS bil_id,
    ...
    LOOP
      ...
      -- Match 1: parent category name (case-insensitive)
      ...
      -- Match 2: part_role fallback (e.g. fabric items without category)
      ...
      -- Legacy fallback: measure_basis / is_roll (only if no new-style rules)
      ...
      IF v_matched THEN
        ...
        VALUES
          (v_task_id, v_line.bil_id, v_line.resolved_part_id, v_line.sku,
           ...
           v_line.cut_length_mm, v_line.cut_height_mm);
...
DO $$ BEGIN RAISE NOTICE 'WO routing updated to category-based (parent name) matching'; END $$;
