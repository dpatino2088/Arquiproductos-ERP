...
CREATE OR REPLACE FUNCTION "public"."catalogitemsmsrp_guard_not_null"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  NEW.msrp_sale_in  := COALESCE(NEW.msrp_sale_in, 0);
  NEW.msrp_sale_out := COALESCE(NEW.msrp_sale_out, 0);
  RETURN NEW;
END;
$$;
...
CREATE OR REPLACE FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_context" "jsonb" DEFAULT '{}'::"jsonb") RETURNS TABLE("quote_line_id" "uuid", "quantity" numeric, "cost_exw" numeric, "material_cost" numeric, "labor_pct" numeric, "labor_cost" numeric, "shipping_pct" numeric, "shipping_cost" numeric, "import_tax_pct" numeric, "import_tax_cost" numeric, "total_cost" numeric)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $_$
DECLARE
...
BEGIN
  -- Detectar columnas disponibles en QuoteLines
  SELECT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema='public' AND table_name='QuoteLines' AND column_name='qty'
  ) INTO v_has_qty;

  SELECT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema='public' AND table_name='QuoteLines' AND column_name='quantity'
  ) INTO v_has_quantity;

  SELECT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema='public' AND table_name='QuoteLines' AND column_name='deleted'
  ) INTO v_has_deleted;

  IF NOT v_has_qty AND NOT v_has_quantity THEN
    RAISE EXCEPTION 'QuoteLines must have qty or quantity column';
  END IF;

  -- Leer QuoteLine (dinámico)
  v_sql :=
    'SELECT organization_id, catalog_item_id, ' ||
    CASE
      WHEN v_has_qty THEN 'COALESCE(qty, 1)'
      ELSE 'COALESCE(quantity, 1)'
    END ||
    ' FROM public."QuoteLines" WHERE id = $1 ' ||
    CASE WHEN v_has_deleted THEN ' AND deleted = false ' ELSE '' END;

  EXECUTE v_sql INTO v_org_id, v_catalog_item_id, v_qty USING p_quote_line_id;

  IF v_org_id IS NULL OR v_catalog_item_id IS NULL THEN
    -- No encontrado (o deleted=true si existe esa columna)
  ...
  -- CostSettings (si existe fila)
  -- OJO: ajusta aquí SOLO si tus columnas se llaman distinto.
  -- (Esto ya viene bien con lo que se ve en tus resultados: labor_pct/shipping_pct/import_tax_pct)
  SELECT
    COALESCE(cs.labor_pct, 0),
    COALESCE(cs.shipping_pct, 0),
    COALESCE(cs.import_tax_pct, 0)
  INTO v_labor_pct, v_shipping_pct, v_import_tax_pct
  FROM public."CostSettings" cs
  WHERE cs.organization_id = v_org_id
  LIMIT 1;

  -- Cálculos
  v_material_cost := ROUND(v_cost_exw * v_qty, 4);
  v_labor_cost := ROUND(v_material_cost * v_labor_pct, 4);
  v_shipping_cost := ROUND(v_material_cost * v_shipping_pct, 4);
  v_import_tax_cost := ROUND((v_material_cost + v_shipping_cost) * v_import_tax_pct, 4);

  quote_line_id := p_quote_line_id;
  quantity := v_qty;
  cost_exw := ROUND(v_cost_exw, 4);
  material_cost := v_material_cost;
  labor_pct := v_labor_pct;
  labor_cost := v_labor_cost;
  shipping_pct := v_shipping_pct;
  shipping_cost := v_shipping_cost;
  import_tax_pct := v_import_tax_pct;
  import_tax_cost := v_import_tax_cost;
  total_cost := ROUND(v_material_cost + v_labor_cost + v_shipping_cost + v_import_tax_cost, 4);
...
ALTER FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_context" "jsonb") OWNER TO "postgres";
...
COMMENT ON FUNCTION "public"."msrp_compute_for_item"("p_item_id" "uuid") IS 'Calcula MSRP para un CatalogItem. Usa CatalogItems.cost_exw como base. Escribe en CatalogItemsMSRP: cost_exw, shipping_pct, import_tax_pct, minimum_margin_pct, msrp_pct_sale_out, shipping_cost, import_tax_cost, total_cost, msrp_sale_in, msrp_sale_out.';
...
CREATE OR REPLACE FUNCTION "public"."recompute_catalogitems_msrp_for_category"("p_org_id" "uuid", "p_category_id" "uuid") RETURNS "void"
  ...
  select
    ...
    ci.unit_of_measure,
    cs.shipping_pct,
    coalesce(itr.import_tax_pct, cs.global_import_tax_pct) as import_tax_pct,
    coalesce(cm.msrp_pct_sale_in, cs.minimum_margin_pct) as minimum_margin_pct,
    coalesce(cm.msrp_pct_sale_out, cs.default_msrp_pct_sale_out) as msrp_pct_sale_out,

    round(ci.cost_exw * coalesce(itr.import_tax_pct, cs.global_import_tax_pct), 4) as import_tax_cost,
    round(ci.cost_exw * cs.shipping_pct, 4) as shipping_cost,

    round(
      ci.cost_exw
      + (ci.cost_exw * coalesce(itr.import_tax_pct, cs.global_import_tax_pct))
      + (ci.cost_exw * cs.shipping_pct),
      4
    ) as total_cost,
    ...
    round(
      (
        ...
        + (ci.cost_exw * coalesce(itr.import_tax_pct, cs.global_import_tax_pct))
        + (ci.cost_exw * cs.shipping_pct)
      ) / nullif(1 - coalesce(cm.msrp_pct_sale_in, cs.minimum_margin_pct), 0),
      4
    ) as msrp_sale_in,
    ...
    round(
      ...
      ) / nullif(1 - coalesce(cm.msrp_pct_sale_out, cs.default_msrp_pct_sale_out), 0),
      4
    ) as msrp_sale_out,

    now() as updated_at
  from public."CatalogItems" ci
  join public."CostSettings" cs
    on cs.organization_id = ci.organization_id
  left join public."CategoryMargins" cm
    on cm.organization_id = ci.organization_id
   and cm.category_id = ci.category_id
   and cm.is_active = true
  left join public."ImportTaxRules" itr
    on itr.organization_id = ci.organization_id
...
begin
  ...
  select
    ...
    cs.shipping_pct,
    coalesce(itr.import_tax_pct, cs.global_import_tax_pct, 0),
    coalesce(cm.msrp_pct_sale_in, cs.minimum_margin_pct),
    coalesce(cm.msrp_pct_sale_out, cs.default_msrp_pct_sale_out),

    round(ci.cost_exw * cs.shipping_pct, 4),
    round(ci.cost_exw * coalesce(itr.import_tax_pct, cs.global_import_tax_pct, 0), 4),

    round(
      ci.cost_exw
      + (ci.cost_exw * cs.shipping_pct)
      + (ci.cost_exw * coalesce(itr.import_tax_pct, cs.global_import_tax_pct, 0)),
      4
    ),
...
COMMENT ON FUNCTION "public"."sync_catalogitems_to_msrp"() IS 'Sync identidad (sku, name, collection_name, variant_name, unit_of_measure) desde CatalogItems a CatalogItemsMSRP. Si no existe fila, INSERT con msrp_sale_in=0, msrp_sale_out=0 para cumplir NOT NULL.';
...
COMMENT ON FUNCTION "public"."sync_catalogitems_to_msrp_safe"() IS 'Sync identidad (sku, name, collection_name, variant_name, unit_of_measure) de CatalogItems a CatalogItemsMSRP. Si no existe fila, INSERT con valores mínimos. En UPDATE solo toca identidad, NO cost_exw ni total_cost (eso lo maneja msrp_compute_for_item).';
...
BEGIN
  ...
  -- ROLL AREA ($/m2)
  -- =========================
  IF coalesce(NEW.is_roll,false) = true AND NEW.roll_width IS NOT NULL AND v_per_m IS NOT NULL THEN
    v_per_m2 := v_per_m / NEW.roll_width;
  END IF;
...
CREATE OR REPLACE TRIGGER "catalogitems_write_conversions" AFTER INSERT OR UPDATE OF "cost_exw", "unit_of_measure", "roll_width", "is_roll", "units_per_purchase_unit" ON "public"."CatalogItems" FOR EACH ROW EXECUTE FUNCTION "public"."trg_catalogitems_write_conversions"();
...
GRANT ALL ON FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_context" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_context" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."compute_quote_line_cost"("p_quote_line_id" "uuid", "p_context" "jsonb") TO "service_role";
