...
CREATE OR REPLACE FUNCTION "public"."catalogitemsmsrp_sync_prices"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- Keep dealer_price <-> msrp_sale_in
  IF NEW.dealer_price IS NULL AND NEW.msrp_sale_in IS NOT NULL THEN
    NEW.dealer_price := NEW.msrp_sale_in;
  ELSIF NEW.msrp_sale_in IS NULL AND NEW.dealer_price IS NOT NULL THEN
    NEW.msrp_sale_in := NEW.dealer_price;
  END IF;

  -- Keep msrp <-> msrp_sale_out
  IF NEW.msrp IS NULL AND NEW.msrp_sale_out IS NOT NULL THEN
    NEW.msrp := NEW.msrp_sale_out;
  ELSIF NEW.msrp_sale_out IS NULL AND NEW.msrp IS NOT NULL THEN
    NEW.msrp_sale_out := NEW.msrp;
  END IF;
...
ALTER FUNCTION "public"."catalogitemsmsrp_sync_prices"() OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "trg_catalogitemsmsrp_sync_prices" BEFORE INSERT OR UPDATE ON "public"."CatalogItemsMSRP" FOR EACH ROW EXECUTE FUNCTION "public"."catalogitemsmsrp_sync_prices"();
...
GRANT ALL ON FUNCTION "public"."catalogitemsmsrp_sync_prices"() TO "anon";
GRANT ALL ON FUNCTION "public"."catalogitemsmsrp_sync_prices"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."catalogitemsmsrp_sync_prices"() TO "service_role";
